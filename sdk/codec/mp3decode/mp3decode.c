...
#endif
{
    ...
    MMIO_Write(AUDIO_DECODER_WRITE_DECODE_ERROR, 0);
#endif
                ...
                else
                {
...
#endif                     // !defined(DUMP_PCM_DATA) && !defined(FORCE_FINDSYNCWORD_ON_FILLBUFFER)
                    {
...
#endif
                        {
                            ...
                            if(combodec<5) combodec++; 
                            
                            if (init == 0 && combodec >=5)
                            {  //continuous 5 times succese dec  , init i2s
#if defined(VOICEOFF)
...
/*#if defined(AUDIO_PLUGIN_MESSAGE_QUEUE)
            ...
            if (decstate & DECODER_STATE_WAIT_I2S_BUF)
            {
                ...
                if (keep_sr != mp3DecInfo.samprate && mp3DecInfo.samprate > 0 && init) // change new sampling rate
                {
                    keep_sr = mp3DecInfo.samprate;
                }
                if (keep_nch != SampBuf.nch && SampBuf.nch > 0 && init) // change new ch
                {
                    keep_nch = SampBuf.nch;
                }
