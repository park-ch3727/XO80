...
static void _internal_DAC_reset(void)
{
   ...
   /*ADC control gain*/
    ...
    data = 0x0  |(0x0 << 11)  //chopper stabilize chontrol(L) :(0x0) off
                ...
                |(CFG_SINGLE_END_MODE << 7 )  //mic mode(R)   :(0x1)single-end ,(0x0)differential
                |(CFG_SINGLE_END_MODE << 6 )  //mic mode(L)   :(0x1)single-end ,(0x0)differential
...
{   // precent to step :ex  82% -> 0x90 
    ...
    current_volpercRL[0]=current_volpercRL[1]=target_volperc;
    current_volstepRL[0]=current_volstepRL[1]=current_volstep;
...
{   // precent to step :ex  82% -> 0x90 
    ...
    if(current_volpercRL[0]==0) _faraday_out=1;//mute R:
    if(current_volpercRL[1]==0) _faraday_out=0;//mute L
    if(current_volpercRL[0]==current_volpercRL[0]==0) {
        current_volperc = 0;
        current_volstep = MIN_OUT_VOLUME;
        _faraday_out=2;//mute RL
    }
    itp_codec_playback_unmute();
        
    printf("FARADAY# target_volperc[%d] = %d(0x%02x) out:%d\n",RL,target_volperc,volstep,_faraday_out);
}
...
{   // precent to step :ex  82% -> 0x90 
    ...
    if(current_micpercRL[0]==0) _faraday_in=1;//mute R:
    if(current_micpercRL[1]==0) _faraday_in=0;//mute L
    if(current_micpercRL[0]==current_micpercRL[0]==0) {
        current_micperc = 0;
        current_micstep = MIN_IN_VOLUME;
        _faraday_in=2;//mute RL
    }
    
    itp_codec_rec_unmute();
    
    printf("FARADAY# target_micperc[%d] = %d(0x%02x) in:%d\n",RL,target_micperc,micstep,_faraday_in);
}
