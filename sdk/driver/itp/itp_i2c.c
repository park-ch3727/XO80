...
static int I2cIoctl(int file, unsigned long request, void *ptr, void *info)
{
    ...
    uint32_t    init_clock  = 400 * 1024;    /*400 * 1000*/

    switch (request)
    {
    case ITP_IOCTL_INIT:
#ifdef CFG_SENSOR_ENABLE
		if(strncmp(CFG_CAPTURE_SENSOR_NAME,"TP2825.c",strlen("TP2825.c"))==0) {
			mmpIicInitialize(iic_port, iic_mode, 0, 0, init_clock, 40);
		}
		else if(strncmp(CFG_CAPTURE_SENSOR_NAME,"TP2825_2.c",strlen("TP2825_2.c"))==0) {
			mmpIicInitialize(iic_port, iic_mode, 0, 0, init_clock, 40);
		}
		else {
