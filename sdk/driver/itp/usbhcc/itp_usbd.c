...
#ifdef CFG_USBD_CD_PRINTER
    /* Init Printer class driver module */
    if ((rc=usbdprn_init())!=USBD_SUCCESS)
    {
        LOG_ERR "usbdprn_init()=%d\n", rc LOG_END
        goto end;
    }

    /* Start Printer class driver module */
    if ((rc=usbdprn_start())!=USBD_SUCCESS)
    {
        LOG_ERR "usbdprn_start()=%d\n", rc LOG_END
        goto end;
    }

    if ((rc=usbd_start((usbd_config_t *)&device_cfg_printer))!=USBD_SUCCESS) {
        LOG_ERR "usbd_start(&device_cfg_printer)=%d\n", rc LOG_END
        goto end;
    }
#endif
...
static int usb_device_exit(void)
{
    ...
    if ((rc=usbdprn_stop())!=USBD_SUCCESS)
    {
        LOG_ERR "usbdprn_stop()=%d\n", rc LOG_END
        goto end;
    }

    /* Delete Printer class driver module */
    if ((rc=usbdprn_delete())!=USBD_SUCCESS)
    {
        LOG_ERR "usbdprn_delete()=%d\n", rc LOG_END
        goto end;
    }
#endif
