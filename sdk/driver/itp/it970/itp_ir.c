...
static void IrTxSend(uint16_t *ptr)
{
    ...
    printf("## DO RX clear ##\n");
    ithIrCtrlEnable(ITH_IR0, ITH_IR_EN);
    for(i=0; i<IrTxCnt; i++)
    {
        printf("irTx send:[%x]=%x\n",i,ptr[i]);
        ithIrTxTransmit(ITH_IR0, ptr[i]);
    }
}
...
static void IrTxIntrHandler(void *arg)
{
	...
	if(xQueueReceiveFromISR(irTxQueue, &code, &xHigherPriorityTaskWoken) == pdTRUE) {
		//enable dma to SEND IR CODE
		//1.parse event to get IR TX code(code table)
		IrTxSend(g_IrTxBuf);

		ithIrTxCtrlDisable(ITH_IR0, ITH_IR_INT);
...
static void IrInit(void)
{
    ...
    ithIrInit(CFG_GPIO_IR0_RX, 0, SAMP_DUR, PRECISION);
    IrCalcThresholds();
    ...
    ithIrTxInit(CFG_GPIO_IR0_TX, 0, SAMP_DUR, PRECISION);
...

    {
	    ...
	    ithIrCtrlEnable(ITH_IR0, ITH_IR_INT);
	    ithIrIntrCtrlEnable(ITH_IR0, ITH_IR_DATA);

		ithIrRxSetModFilter(ITH_IR0, 0x535, 0xFA0); //20~60k freq.(80M/freq.)
		//ithIrRxSetModFilter(ITH_IR0, 0xCE4, 0x26AC); //20~60k (WCLK/freq.), WCLK=198MHz Minfreq=20KHz Maxfreq=60KHz
...

	{
		...
		ithIntrDisableIrq(ITH_INTR_IRTX);
		ithIntrClearIrq(ITH_INTR_IRTX);
		ithIntrRegisterHandlerIrq(ITH_INTR_IRTX, IrTxIntrHandler, NULL);
		ithIntrEnableIrq(ITH_INTR_IRTX);

		ithIrTxIntrCtrlEnable(ITH_IR0, ITH_IR_EMPTY);
		
		ithIrTxSetModFreq(ITH_IR0, 0x839); //(0x1e);
	    //ithIrTxSetModFreq(ITH_IR0, 0x13D5);  //39kHz (WCLK/SampleRate), WCLK=198MHz SampleRate=39KHz

		//Enable IR Sender
		ithIrTxCtrlEnable(ITH_IR0, ITH_IR_EN);
	}
...

    {
        ...
        ithIrRxSetModFilter(ITH_IR0, 0x535, 0xFA0); //20~60k freq.(80M/freq.)
        //ithIrRxSetModFilter(ITH_IR0, CE4, 0x26AC); //20~60k (WCLK/freq.), WCLK=198MHz Minfreq=20KHz Maxfreq=60KHz
...
#ifdef  ENABLE_IR_INTR			
		ithIrTxCtrlEnable(ITH_IR0, ITH_IR_INT);
#endif
	}
	else
	{
...
static void IrTxSetCarrier(uint32_t Carrier)
{
    uint32_t ModDiv = 0x0, BusClk = 0x0;
        
    printf("IR TX Set Carrier=%d\n", Carrier);

    ithIrTxClear(ITH_IR0); // Clear TX FIFO

    BusClk = ithGetBusClock();
    ModDiv = BusClk / Carrier;
    
    ithIrTxSetModFreq(ITH_IR0, ModDiv);  // Set TX Carrier
}
...
static int IrIoctl(int file, unsigned long request, void* ptr, void* info)
{
    ...
    case ITP_IOCTL_SET_IRTX_CARRIER:
        IrTxSetCarrier((uint32_t) ptr);
        break;
