...
static int ScreenIoctl(int file, unsigned long request, void* ptr, void* info)
{
    ...
    case ITP_IOCTL_RESET:
        ...
        ithLcdSetBaseAddrB(lcdBaseB);                
	        ...
	        printf("LCD width:%d height:%d pitch:%d baseA:0x%X baseB:0x%X baseC:0x%X\n", ithLcdGetWidth(), ithLcdGetHeight(), ithLcdGetPitch(), lcdBaseA, lcdBaseB, lcdBaseC);
	    #else
	        printf("LCD width:%d height:%d pitch:%d baseA:0x%X baseB:0x%X\n", ithLcdGetWidth(), ithLcdGetHeight(), ithLcdGetPitch(), lcdBaseA, lcdBaseB);
	    #endif
        
	#else
		printf("LCD width HongJeongPyo_1 :%d height:%d pitch:%d\n", ithLcdGetWidth(), ithLcdGetHeight(), ithLcdGetPitch());
        ithLcdLoadScriptFirst(lcdScript, ITH_COUNT_OF(lcdScript));
        printf("LCD width HongJeongPyo_3 :%d height:%d pitch:%d\n", ithLcdGetWidth(), ithLcdGetHeight(), ithLcdGetPitch());
	#endif // CFG_LCD_MULTIPLE
