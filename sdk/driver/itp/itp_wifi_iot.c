...
static void itpWifiIOTLwipInit(struct netif *netif)
{
    ...
    printf("[ITP WIFI IOT] MT7682 Netif(0x%X)\n", netif);
...
static int WifiIoctl(int file, unsigned long request, void* ptr, void* info)
{
    switch (request)
    {
        ...
        case ITP_IOCTL_INIT:
            printf("[ITP WIFI IOT][SDIO Type] ITP_IOCTL_INIT \n");
            return mmpMT7682WifiDriverOn();
        ...
        case ITP_IOCTL_WIFIAP_DISABLE:
            printf("[ITP WIFI IOT][SDIO Type] ITP_IOCTL_WIFIAP_DISABLE is useless\n");
            break;
