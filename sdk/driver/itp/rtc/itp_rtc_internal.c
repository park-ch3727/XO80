...
#define RTC_YEAR_2100_TIMESTAMP (0xf48656c4)	//(0xF485E680)		//JPHong 2023-05-22
...
long itpRtcGetTime(long* usec)
{
        ...
        ((unsigned long)sec1 > (unsigned long)RTC_YEAR_2100_TIMESTAMP) )
    {
        ...
        ithRtcSetTime(sec1);	//JPHong 2023-05-22
    }
...
void itpRtcSetTime(long sec, long usec)
{
    ...
    if( ((unsigned long)sec < (unsigned long)CFG_RTC_DEFAULT_TIMESTAMP) || 
        ((unsigned long)sec >= (unsigned long)RTC_YEAR_2100_TIMESTAMP) )			//JPHong 2023-05-22
    {
        /* SW handle internal RTC low bettery power issue */
        /* set default timestemp when sec < default or sec > year 2100 */
        LOG_ERR "[RTC] set time ERROR(ori=%x, default=%x , %d):[#%d]\n", sec, CFG_RTC_DEFAULT_TIMESTAMP,(usec / 1000000), __LINE__ LOG_END
        ithRtcSetTime(CFG_RTC_DEFAULT_TIMESTAMP + (usec / 1000000));
    }
    else
    {
    	
  //  	LOG_ERR "[RTC] itpRtcSetTime(default=%x):[#%d]\n", sec + (usec / 1000000), __LINE__ LOG_END
