...
#if 0 //def CFG_RTC_INTERNAL  // JPHong
    {
 ...
 */
    ...
    itpRegisterDevice(ITP_DEVICE_USBHACM, &itpDeviceUsbhAcm);
#endif
    ...
    itpRegisterDevice(ITP_DEVICE_ECM_EX, &itpDeviceUsbEcmex);
    ioctl(ITP_DEVICE_ECM_EX, ITP_IOCTL_INIT, NULL);
#endif

    // init ethernet device
#if defined(CFG_NET_ETHERNET) || defined(CFG_USB_ECM) || defined(CFG_USBH_CD_CDCECM) || defined(CFG_USBD_NCM)
    ...
    // init socket device
#if defined(CFG_NET_ENABLE) && !defined(CFG_NET_WIFI_REDEFINE) && !defined(CFG_USBD_NCM) && !defined(CFG_NET_WIFI_SDIO_NGPL_AP6256) && !defined(CFG_NET_WIFI_SDIO_NGPL_AP6236) && !defined(CFG_NET_WIFI_SDIO_NGPL_AP6181)
...
#ifdef CFG_TUYA_CLOUD_ENABLE//FOR TUYA project only need modified
    // register sys param device
    itpRegisterDevice(ITP_DEVICE_SYS_PARAM, &itpDeviceSysParam);
    ioctl(ITP_DEVICE_SYS_PARAM, ITP_IOCTL_INIT, NULL);
#endif
    // init headset device
    ...
    // init usb device: must follow "init drive table"
#if defined(CFG_USB_DEVICE)|| defined(CFG_USBHCC_DEVICE)
...
#ifdef CFG_RTC_6_8P
    ioctl(ITP_DEVICE_RTC, ITP_IOCTL_INIT_TASK, NULL);
#endif
