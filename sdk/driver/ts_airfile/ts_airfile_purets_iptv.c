    ...
    TSAF_CRTL_INFO  *pTsafCtrlInfo)
{
    ...
    //duration_time = itpTimevalDiff(&buff_chk_startT, &currT);
    duration_time = itpGetTickDuration(prevTime);
    if(duration_time > 20)
    {
    ...
    void            *extraDat)
{
    ...
    do{
        ...
        else
        {
            ...
            rc = pthread_create(&pInitParam->thread, &attr, _routine_func, (void *)pInitParam);
            if( rc )
            {
