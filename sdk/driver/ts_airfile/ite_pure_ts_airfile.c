...
#define CACHE_THREAD_TIMER_GAP              1
    ...
    while( IS_THREAD_RUNNING(cache_index, pCacheBufMgr) )
    {
        if( tsaf_get_duration(&pCacheBufMgr->lastT) > CACHE_THREAD_TIMER_GAP )
        {
            ...
            while( IS_START_CACHE(cache_index, pCacheBufMgr) )
            {
                ...
                else if(pCacheBufMgr->cacheBuf_RPtr == pCacheBufMgr->cacheBuf_WPtr)
                {
                    readywriteSize = CACHE_BUFFER_SIZE;
                }
                else
                {   
                ...
                if( getTsStreamFunc )       getTsStreamFunc((void *)pInitParam->tsi_index, &pSampleAddr, &sampleSize, 0);
    ...
    void            *extraData)
{
    ...
    if( pTsafDev && pTsafDev->situation != TSAF_SITUATION_FAIL )
    {
        ...
        pTsafDev->initParam.getTsStreamFunc = (GET_TS_STREAM_FUNC)_get_Ts_Stream;
