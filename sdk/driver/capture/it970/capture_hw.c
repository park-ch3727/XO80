...
ithCapHWICInit(void)
{
   if ((ithReadRegA(ITH_HOST_BASE + ITH_REVISION_ID_REG) & 0xFFF) == 0)
   {
       ithClearRegBitA(ITH_HOST_BASE + ITH_CAP_CLK_REG, ITH_EN_M12CLK_BIT);//turn off M12clk
       usleep(1000);
       ithWriteRegMaskA(_GetAddress(0, CAP_SCALING_SETTING_REGISTER0), 0x1FFF, 0x1FFF);//assign nonzero value to  cap0
       ithWriteRegMaskA(_GetAddress(1, CAP_SCALING_SETTING_REGISTER0), 0x1FFF, 0x1FFF);//assign nonzero value to  cap1
       usleep(1000);
...
void 
ithCap_Set_Delay(CAPTURE_DEV_ID DEV_ID, MMP_UINT32 value)
{
    ithWriteRegMaskA(_GetAddress(DEV_ID, CAP_CLOCK_SETTING_REGISTER), 0x0, 0x1 << 3);
    /*write Dlystage to UCLKDly*/
    ithWriteRegMaskA(_GetAddress(DEV_ID, CAP_CLOCK_SETTING_REGISTER), value << 8, 0xf << 8);
    ithWriteRegMaskA(_GetAddress(DEV_ID, CAP_CLOCK_SETTING_REGISTER), 0x1 << 3, 0x1 << 3);
}
