...
#include <malloc.h>
#include "spinfdrv.h"
#include "../driver/spi/it970/axispi/axispi_hw.h"
...
static SPINF_CFG g_SpiNfCfgArray[] = 
{
	...
	{ "GD_5F1GQ5UE", 0xC8, 0x51, 0x00, 1, 2048, 64, 1024, 128 }, 
	{ "GD_5F2GQ5UE", 0xC8, 0x52, 0x00, 1, 2048, 64, 2048, 128 }, 
	{ "GD_5F4GQ6UE", 0xC8, 0x55, 0x00, 1, 2048, 64, 4096, 128 }, 		//4Gb NAND flash U:3.3V R:1.8V
	...
	{ "FM25S01A", 0xA1, 0xE4, 0x00, 5, 2048, 64, 1024, 64 },	
	{ "XT26G01C", 0x0B, 0x11, 0x00, 4, 2048, 64, 1024, 128 },	
...
*/
static uint8_t gGdSprMapTable[] = { 0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,
...
11b = reserved.
*/
static uint8_t _checkEccStatus_FM(uint32_t blk, uint32_t ppo, uint8_t s)
{
    switch(s>>4)
    {
        case 0x2:
            printf("NfRdFmE2(%x,%x,%x)\n",blk,ppo,s);
...
*/
static uint8_t _checkEccStatus_PRG(uint32_t blk, uint32_t ppo, uint8_t s)
{
    switch(s>>4)
    {
        case 0x3:
...
static uint8_t _checkEccStatus_FS(uint32_t blk, uint32_t ppo, uint8_t s)
{
    ...
    {
        case 0x2:
            printf("NfRdFsE2(%x,%x,%x)\n",blk,ppo,s);
            //means More than 4-bit error and not corrected.
            return (23);
        case 0x1:
            //printf("NfRdFsE1(%x,%x,%x)\n",blk,ppo,s);
            //means 1~4 bits error and corrected
            break;
        case 0x0:
            //printf("NfRdFsE0(%x,%x,%x)\n",blk,ppo,s);
            //mean no bit error
            break;          
        default:
            printf("incorrect status of feature register!!!reg[C0]=%x\n",s);
	...
	{	
	    ...
	    switch(gSpiNfInfo.MID)
	    {
	    	case 0xC8:
	   		...
	   		case 0xA1:
	   			if(gPARAGON_MID_0xA1)
	   			{
	   				rst = _checkEccStatus_PRG(b,p,status);
	   			}
	   			else
	   			{
	   				rst = _checkEccStatus_FM(b,p,status);
	   			}
	   			break;
	   		...
	   		case 0xCD:
	   			rst = _checkEccStatus_FS(b,p,status);
	   			break;
...
static void _reMapSpare(uint8_t *ptr1,uint8_t *ptr2, uint8_t mapType)
{
	...
	if(mapType == SPINF_REMAP_SPARE_FOR_READ)
	{
		...
		if( (gSpiNfInfo.MID==0xC8) && (ptr2[0x10]==0x57) && (ptr2[0x11]==0x4C) && (ptr2[0x12]==0x54) && (ptr2[0x13]==0x46) )
		{			
			//check old format and cover it for compatibility
			if(printCnt++ < 10)	printf("[Warning]: Got old GD's Spr format, use original remapping\n");
			directMap = 1;
		}
			
		for(i=0; i<24; i++)
		{			
			if( directMap)	ptr1[i] = ptr2[i];
			else	ptr1[i] = ptr2[reMap[i]];
...
uint8_t spiNf_setQuadIoOperation(unsigned char value)
{
    ...
    if( !spiNf_getFeature(0xB0, &B0reg) )
    {
        ...
        if( !spiNf_setFeature(0xB0, (B0reg & 0xFE) | setBit) );    //0 is OK
        {
            ...
            if( !spiNf_getFeature(0xB0, &B0reg) )
            {
                if( (gSpiNfInfo.MID != 0xA1) && ((B0reg & 0x01) != setBit) )
...
uint8_t spiNf_Initial(SPI_NF_INFO *info)
{
	...
	if(gSpiNfInfo.MID==0xA1)
	{
		...
		if(gSpiNfInfo.DID0==0xE1)	gPARAGON_MID_0xA1 = 1;
			
	    if(!spiNf_getFeature(0xA0, &status))   printf(" get feature(A0)=%x!!\n",status);
	    if(!spiNf_getFeature(0xB0, &status))   printf(" get feature(B0)=%x!!\n",status);
	    if( !(status&0x10) )	spiNf_setFeature(0xB0, (status|0x10) );		//ENABLE ECC function
	    if(!spiNf_getFeature(0xC0, &status))   printf(" get feature(C0)=%x!!\n",status);
	...
	if(gSpiNfInfo.MID==0x0B)
	{
	    ...
	    if((gSpiNfInfo.MID==0x0B) && ((gSpiNfInfo.DID0 & 0xF0) == 0x10))	gXTX_TYPE_C = 1;
	...
	if( (gSpiNfInfo.MID==0xEF) || (gPARAGON_MID_0xA1) || (gSpiNfInfo.MID==0x0B) || (gSpiNfInfo.MID==0xE5) )
	{
	    gNeedReMapSpare = 1;
        printf("[Spinfdrv.c] MID=%x, gNeedReMapSpare = %x\n", gSpiNfInfo.MID, gNeedReMapSpare);
	}
	
	if( (gSpiNfInfo.MID==0xC8) && gUseDummyReadId ) 
	{
	    gNeedReMapSpare = 1;
            printf("[Spinfdrv.c]2 MID=%x, gNeedReMapSpare = %x\n", gSpiNfInfo.MID, gNeedReMapSpare);
	}
...
uint8_t spiNf_ByteRead(uint32_t blk, uint32_t ppo, uint8_t *dBuf, uint32_t offset, uint32_t rLen)
{
	...
	{
		if ((gSpiNfInfo.MID == 0x98) && tsbRdOffset)  spiret = spi_read(SpiCmd, 4, tmpData, tsbRdOffset + gSpiNfInfo.FtlSprSize);
	...
	{
		if( (gSpiNfInfo.MID == 0x98) && tsbRdOffset )
	    {
...
uint8_t spiNf_PageProgram(uint32_t blk, uint32_t ppo, uint8_t *dBuf)
{
    ...
    //02H(program load)
    #ifdef ENABLE_QUAD_OPERATION_MODE
    if(g_TOSHIBA_QuadMode)	SpiCmd[0] = 0x02;    //Toshiba's 1st gen SPI NAND does NOT support QUAD-IO program
    else	SpiCmd[0] = 0x32;	
    #else
	SpiCmd[0] = 0x02;	
    #endif
...
uint8_t spiNf_BlockErase(uint32_t blk)
{
	...
	{
		printf("Erase Error: block Num Out of Range, b=%x, max=%x\n",blk,gSpiNfInfo.TotalBlk);
		//while(1);
		return (15);
	}
