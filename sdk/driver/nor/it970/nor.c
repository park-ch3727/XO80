...
NOR_VENDOR_CONTEXT  nor_support_vendor[] = {
    ...
    {0xC2, 0x2019, 0x18, "MX_25L25645G",     MX_25L25645G    },
    ...
    {0x0B, 0x4017, 0x16, "XT_25F64F",        XT_25F64F       },
    {0x0B, 0x4018, 0x17, "XT_25F128F",       XT_25F128F      },
    {0x0B, 0x4019, 0x18, "XT_25F256B",       XT_25F256B      },
    {0x20, 0x4018, 0x17, "XM_25QH128C",      XM_25QH128C     },
    ...
    NOR_VENDOR_CONTEXT  *context)
{
    ...
    case XT_25F128F:
        ...
        if (QuadmodeEnable)
        {
            norObject->context.page_program_4PP = 0x32;
            norObject->context.status           = 0x200;
            norObject->context.dtr_dummy_clock  = 8;
        }
        break;
/*
    case MX_25L25735F:
    ...
    case READ:
        ...
        if (norObject->context.use4BytesAddress == true)
        {
            if (QuadmodeEnable)
            {
                ...
                if (DTRmodeEnable && norObject->context.dtr_dummy_clock)
                    mmpAxiSpiSetDummyByte(NOR_BUS, norObject->context.dtr_dummy_clock);
                else
                    mmpAxiSpiSetDummyByte(NOR_BUS, 6);
            }
            else
    ...
    uint32_t    inDataSize)
{
    ...
    do
    {
        ...
        {
            // Doesn't need copy.
            NOR_LOG_MSG("blockStartAddress1 = %d\n",         blockStartAddress);
        ...
        else
        {
            ...
            uint8_t     *readBackBuffer     = (uint8_t *)itpVmemAlignedAlloc(32, norObject->context.bytesPerSector);
						...
						NOR_LOG_MSG("blockStartAddress2 = %d\n",         blockStartAddress);
    ...
    uint32_t    eraseLength)
{
    ...
    do
    {
        ...
        NOR_LOG_MSG("currNorAddress = 0x%08X\n",         currNorAddress);
