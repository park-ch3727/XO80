    ...
    uint32_t     delay)
{
    ...
    {
        IIC_ERROR_MSG("Device(%d) already init!\n", port);
        return false;
    }
...
{
    printf("IT970 not support this function!!!\n");
    return 0;
}
...
#endif
#endif
        //Reset HW, but IT970/IT960 not use, they have another reset mode.
    ...
    uint32_t    outDataSize)
{
    ...
    while(remainderSize)
    {
        ...
        {
            IIC_ERROR_MSG("Wait RX FIFO timeout(status=0x%x)!  ID:0x%x,PORT:%d\n", ithReadRegA(IicDevices[port].regAddrBase + REG_I2C_STATUS),slaveAddr,port);
