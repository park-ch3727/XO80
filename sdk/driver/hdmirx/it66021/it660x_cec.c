...
//******************************************/
#include "hdmirx/it66021/it660x_cec.h"
//#include "stdio.h"
//#include <string>
...
#define	Enable_IT6602_CECB0
#define	ENABLE_LED

#ifdef	DEBUG_IT6602_CEC
#define	IT6602_CEC_PRINT(x) printf x
#else
#define	IT6602_CEC_PRINT(x)
#endif
...
void DumpIT6602_CECReg(void)
{
    ...
    for(i = 0; i < 0x4F; i+=16 )
    {
         IT6602_CEC_PRINT(("[%02X]  ",(int) i));
        for( j = 0; j < 16; j++ )
        {
            ucData = IT6602_CEC_ReadI2C_Byte((int)((i+j)&0xFF));
              IT6602_CEC_PRINT((" %02X",(int) ucData));
            if( (j == 3)||(j==7)||(j==11))
            {
                 IT6602_CEC_PRINT((" -"));
            }
...
EnQueue(void)
{
    ...
    tail += 1;
    i=tail % Q_SIZE;
    queue[i].id.SIZE=IT6602_CEC_ReadI2C_Byte(REG_FOLLOWER_RX_NUM);
    queue[i].id.HEADER=IT6602_CEC_ReadI2C_Byte(REG_RX_HEADER);
    queue[i].id.OPCODE=IT6602_CEC_ReadI2C_Byte(REG_RX_OPCODE);
    queue[i].id.OPERAND1=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND1);
    queue[i].id.OPERAND2=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND2);
    queue[i].id.OPERAND3=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND3);
    queue[i].id.OPERAND4=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND4);
    queue[i].id.OPERAND5=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND5);
    queue[i].id.OPERAND6=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND6);
    queue[i].id.OPERAND7=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND7);
    queue[i].id.OPERAND8=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND8);
    queue[i].id.OPERAND9=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND9);
    queue[i].id.OPERAND10=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND10);
    queue[i].id.OPERAND11=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND11);
    queue[i].id.OPERAND12=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND12);
    queue[i].id.OPERAND13=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND13);
    queue[i].id.OPERAND14=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND14);
    queue[i].id.OPERAND15=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND15);
    queue[i].id.OPERAND16=IT6602_CEC_ReadI2C_Byte(REG_RX_OPERAND16);
...
void cecclrint( void )
{
        IT6602_CEC_WriteI2C_Byte(REG08,  B_INT_CLR|B_DBGCEC_CLR|B_CEC_SMT); 	//Reset all interrupt
        IT6602_CEC_WriteI2C_Byte(REG08, B_CEC_SMT|B_DBGCEC_CLR);
}
...
//#ifdef
    ...
    //while(1)
    {
        ...
        IT6602_CEC_WriteI2C_Byte(REG09,B_EN100ms_CNT);
        delay1ms(99);
        IT6602_CEC_WriteI2C_Byte( REG09,0x00);
#ifdef ENABLE_LED
        LED2=~LED2;
        LED1=~LED1;
#endif
    ...
    T100us = ( IT6602_CEC_ReadI2C_Byte(REG_MSCOUNT_H)*0x10000 + IT6602_CEC_ReadI2C_Byte(REG_MSCOUNT_M)*0x100 + IT6602_CEC_ReadI2C_Byte(REG_MSCOUNT_L) )/0x3E8;



    IT6602_CEC_PRINT(("\n------------------------------\n"));
    IT6602_CEC_PRINT(("T100us = %X\n", (int) T100us));
    IT6602_CEC_PRINT(("H=%X\n",(int) IT6602_CEC_ReadI2C_Byte(REG_MSCOUNT_H)));
    IT6602_CEC_PRINT(("M=%X\n",(int) IT6602_CEC_ReadI2C_Byte(REG_MSCOUNT_M)));
    IT6602_CEC_PRINT(("L=%X\n",(int) IT6602_CEC_ReadI2C_Byte(REG_MSCOUNT_L)));
...
void cecConfigure( BYTE LogicAdr, BYTE TimeUnit, BYTE CECRst, BYTE EnNAck )
{
    ...
    if( RxSelf_Dis==1 )
#if 0
            IT6602_CEC_WriteI2C_Byte(REG09, B_RXSELF_SEL); 	//Reg_RxSelf_Dis[1]
#else
            IT6602_CEC_WriteI2C_Byte(REG09, B_RXSELF_SEL|B_REGION_SEL); 	//Reg_RxSelf_Dis[1]
#endif
        else
            IT6602_CEC_WriteI2C_Byte(REG09, 0x00);

    IT6602_CEC_WriteI2C_Byte(0x0B,0x14); 					//REG_Data_Min[7:0]
//	IT6602_CEC_WriteI2C_Byte(REG_DATA_MIN, 0x00); 		//REG_Data_Min[7:0]
        IT6602_CEC_WriteI2C_Byte(REG_TIMER_UNIT, TimeUnit); 	//REG_Timer_unit[7:0]
        IT6602_CEC_WriteI2C_Byte(REG_CEC_TARGET_ADDR, LogicAdr); 	//REG_Logical_addr[3:0]

        //IT6602_CEC_WriteI2C_Byte(0x07, 0x37); 		//Debug_Reg_Sel

        if( CECRst==1 )
            IT6602_CEC_WriteI2C_Byte(REG08,B_CEC_RST|B_CEC_SMT); 	//REG_CEC_Rst
    else
            IT6602_CEC_WriteI2C_Byte(REG08,B_CEC_SMT); 	//REG_CEC_Rst


    uc=IT6602_CEC_ReadI2C_Byte(REG09);
        if( EnNAck==1 )
            IT6602_CEC_WriteI2C_Byte(REG09, uc|B_NACK_EN); 	//REG_NAck_En
...
BYTE ReadyFire, BusFree;
     ...
     reg0x44 = IT6602_CEC_ReadI2C_Byte(REG_SYS_STATUS);
    ...
    BusFree = (reg0x44&B_BUS_STATUS)>>1;
        ...
        if( ReadyFire==1 && BusFree==1 )  // Ready Fire && Bus Free
            ...
            {
                //printf("IT6602_CEC[44]SYS_STATUS=%X\n",reg0x44);
...
void IT6602_ResetCEC(void)
{

    unsigned char u8_LogicAddress=DEVICE_ID_TV;
    unsigned char i = 0;

    #ifdef Enable_IT6602_CECB0
    IT6602_CEC_WriteI2C_Byte(0xF8,0xC3);
    IT6602_CEC_WriteI2C_Byte(0xF8,0xA5);
    #endif

    // initial Queue for save CEC Rx command
    initQ();

    // Set Logic Address and 100ms timer for IT6602_CEC CEC function	 for IT6602
    cecConfigure(u8_LogicAddress,timeunit,FALSE,FALSE);


    // Reg07[6]='1' for enable CEC interrupt pin to MCU
    IT6602_CEC_WriteI2C_Byte(REG07,IT6602_CEC_ReadI2C_Byte(REG07)|0x40);

    //2013-0509 change IT6602 CEC default value for enable ALL interrupt mask
    IT6602_CEC_WriteI2C_Byte(REG06, 0x00);


    #ifdef Enable_IT6602_CECB0
        // Reg0F[4] IO pull up enable
        IT6602_CEC_WriteI2C_Byte(REG0F,0x50);
    #endif
//xxxxx 2014-0128
    ...
    //IT6602_CECPhysicalAddressReset();
...
void IT6602_CECInit(void)
{
    ...
    //// get 100ms timer from 8051 to synchronous IT6602_CEC capture timer
...
//xxxxx 2014-0128
    ...
    IT6602CEC_RegisterCallBack(&CECReceivePowerOff_Callback, &CECReceivePowerOn_Callback);
//xxxxx

}


void IT6602_CECReceiver(void)
{
...
//if(CEC_INT_Pin)
{
    ...
    CECInt   = (IT6602_CEC_ReadI2C_Byte(REG_INT_STATUS) & 0x3F);

    //IT6602_CEC_PRINT(("CECint = %X\n",CECInt));
        ...
        {
            RegTxOutState=eFail;
            IT6602_CEC_PRINT((" B_TXFAIL_INT => TX cmd %X\n",(int) OldTxCECcmd));
        }

        if( CECInt & B_TXDONE_INT)
        {
            RegTxOutState=eSucessful;
            IT6602_CEC_PRINT((" B_TXDONE_INT => TX cmd %X\n",(int) OldTxCECcmd));
        }

    IT6602_CEC_WriteI2C_Byte(REG_INT_STATUS,CECInt);	// Write '1' clear interrupt
}
#else

    // [0]:TxInt, [1]:RxInt, [2]:RxFail [3]:TxDone [4]:RxDone
    CECInt   = IT6602_CEC_ReadI2C_Byte(REG_INT_STATUS);
...
void IT6602_CECCheckFollower(void)	//for double check follower address that is correct address !!
{
    BYTE uc;
...
void IT6602_CECTransmitter(void)
{
    BYTE i;
    switch(CECTxState)
    {
        case sCECTransfer:
        {
            ...
            //if( cecfirerdy()==TRUE)// for IT6602_CEC old  chip
            if(RegTxOutState!=ePending)		// for IT6602_CECB0 chip only
            {
                IT6602_CEC_WriteI2C_Byte(REG_CEC_OUT_NUM, (TxCmdBuf.array[0]));  	//OutNum

                for(i=0;i<(TxCmdBuf.id.SIZE)+1;i++)
                            IT6602_CEC_WriteI2C_Byte(REG_TX_HEADER+i, TxCmdBuf.array[i+1]);  	//Header



                //FireCmd --> Reg08[7]= 1 -> 0 !!!
                    IT6602_CEC_WriteI2C_Byte(REG08, B_FIRE_FRAME|B_CEC_SMT|B_DBGCEC_CLR);
                    IT6602_CEC_WriteI2C_Byte(REG08, B_CEC_SMT|B_DBGCEC_CLR);

//			IT6602_CEC_WriteI2C_Byte(REG08, B_CEC_SMT);         	//FireCmd
//			IT6602_CEC_WriteI2C_Byte(REG08, B_FIRE_FRAME|B_CEC_SMT);         	//FireCmd




                OldTxCECcmd= IT6602_CEC_ReadI2C_Byte(REG_TX_OPCODE);
                RegTxOutState=ePending;		// for IT6602_CECB0 chip only
...
//0503 not yet!!!				IT6602_CEC_PRINT(("111111111111111111111111111111111111111111111\n"));
//0503 not yet!!!				IT6602_CEC_PRINT((" TimeOutCheck(eCECTxTimeOut)==TRUE\n"));
//0503 not yet!!!				IT6602_CEC_PRINT(("111111111111111111111111111111111111111111111\n"));
...
//0503 not yet!!!			}
        ...
        case sCECCheckResult:
        {
            ...
            if(RegTxOutState==eFail)					// for IT6602_CECB0 chip only
                CECTxState=sCECReceiverFail;
            else if(RegTxOutState==eSucessful)		// for IT6602_CECB0 chip only
                CECTxState=sCECOk;
...
//0503 not yet!!!				CECTxState=sCECReceiverFail;
//0503 not yet!!!				RegTxOutState=eFail;
//0503 not yet!!!				IT6602_CEC_PRINT(("222222222222222222222222222222222222222222222\n"));
//0503 not yet!!!				IT6602_CEC_PRINT((" TimeOutCheck(eCECTxTimeOut)==TRUE\n"));
//0503 not yet!!!				IT6602_CEC_PRINT(("222222222222222222222222222222222222222222222\n"));
//0503 not yet!!!
//0503 not yet!!!			}
            ...
            //uc=(IT6602_CEC_ReadI2C_Byte(REG_SYS_STATUS)&B_OUT_STATUS);
            uc=(IT6602_CEC_ReadI2C_Byte(REG_SYS_STATUS));

            if((uc&B_ReceiverFail)==B_ReceiverFail)
            {
                IT6602_CEC_PRINT((" B_ReceiverFail[%X] => TX cmd %bX\n",uc,OldTxCECcmd));
                CECTxState=sCECReceiverFail;
            }
            else if((uc&B_ReceiverNACK)==B_ReceiverNACK)
            {
                IT6602_CEC_PRINT((" B_ReceiverNACK[%X] => TX cmd %bX\n",uc,OldTxCECcmd));
                //IT6602_CEC_PRINT(("Receiver NACK\n");
                //CECTxState=sCECReceiverNack;
            }
            else if((uc&B_ReceiverRetry)==B_ReceiverRetry)
            {
                //IT6602_CEC_PRINT(("Receiver Retry\n");
            }
            else
            {
                //IT6602_CEC_PRINT(("Receiver ACK\n");
                if( IT6602_CEC_ReadI2C_Byte(REG_INITIATOR_TX_NUM)==TxCmdBuf.SIZE)
                {
                    IT6602_CEC_PRINT((" sCECOk [%X]=> TX cmd %bX\n",uc,OldTxCECcmd));
                    CECTxState=sCECOk;
                }
//				else
//					{
//						MHLRX_DEBUG_PRINTF(("TX size error => Reg Tx Num=%X , TxcmdBufSize=%bX\n",
//							IT6602_CEC_ReadI2C_Byte(REG_INITIATOR_TX_NUM),TxCmdBuf.SIZE));
//					}
            }
#endif
        }
...
// for IT6602+iT6623void CEC_InactiveSource(BYTE port)
// for IT6602+iT6623{
// for IT6602+iT6623
// for IT6602+iT6623#ifdef 	_CEC_DEVICE_AVR_
// for IT6602+iT6623		CECCmdSet(DEVICE_ID_TV,eInactiveSource,txphyadr[0],txphyadr[1]);
// for IT6602+iT6623#else
// for IT6602+iT6623	if(rxcurport==port)
// for IT6602+iT6623		CECCmdSet(DEVICE_ID_TV,eInactiveSource,rxphyadr[rxcurport][0],rxphyadr[rxcurport][1]);
// for IT6602+iT6623#endif
// for IT6602+iT6623}

// for IT6602+iT6623void CEC_RoutingChange(BYTE port)
// for IT6602+iT6623 {
// for IT6602+iT6623	//when Source un-plug(w/o +5v ) then broadcast AVR routing change.
// for IT6602+iT6623
// for IT6602+iT6623	BYTE	adr0,adr1;
// for IT6602+iT6623
// for IT6602+iT6623 	if(Myself_LogicAdr==DEVICE_ID_AUDIO)
// for IT6602+iT6623 	{
// for IT6602+iT6623
// for IT6602+iT6623		if(port<4)
// for IT6602+iT6623		{
// for IT6602+iT6623	 		adr0=rxphyadr[rxcurport][0];
// for IT6602+iT6623	 		adr1=rxphyadr[rxcurport][1];
// for IT6602+iT6623			rxcurport=port;
// for IT6602+iT6623			CECCmdSet(DEVICE_ID_BROADCAST,eRoutingChange,adr0,adr1);
// for IT6602+iT6623		}
// for IT6602+iT6623 	}
// for IT6602+iT6623 }
 ...
 void CECCmdSet(u8_t Follower,eCEC_CMD TxCommand,u8_t cOperand1,u8_t cOperand2)
{
    ...
    BYTE i;
     ...
     IT6602_CEC_PRINT((" follower =%X op1=%bX op2=%bX\n",(int) Follower,(int) cOperand1,(int) cOperand2));
...
#ifdef DEBUG_IT6602_CEC
                IT6602_CEC_PRINT(("PollingMessage Header=%X\n",TxCmdBuf.id.HEADER));
#endif
                 TxCmdBuf.id.SIZE=1;
                 break;
...
IT6602_CEC_PRINT(("CECCmdSet --> Tx "));

for(i=0;i<((TxCmdBuf.array[0])+1);i++)
{
    IT6602_CEC_PRINT(("TXCmd[%X]=%bX ",i,TxCmdBuf.array[i]));
    if((i%6)==5) IT6602_CEC_PRINT(("\n"));
}
IT6602_CEC_PRINT(("\n\n\n"));
...
void CECDecoder(void)
{
    ...
    {
        IT6602_CEC_PRINT(("RxCmd[%X]=%bX ",i,pCECRxBuf->array[i]));
        if((i%6)==5) IT6602_CEC_PRINT(("\n"));
    }
    IT6602_CEC_PRINT(("\n"));
#endif
        ...
        switch(LookUpTable_CEC[index].header)
        {
            case eDirectly:
                if(i==DEVICE_ID_BROADCAST)
                {
                    bInvalidCmd=TRUE;
#ifdef DEBUG_IT6602_CEC
                    IT6602_CEC_PRINT(("!!!Invalid Direct Cmd !!!\n"));
#endif
                }
                break;
            ...
            case eBroadcast:
                if(i!=DEVICE_ID_BROADCAST)
                {
                    bInvalidCmd=TRUE;
#ifdef DEBUG_IT6602_CEC
                    IT6602_CEC_PRINT(("!!!Invalid Broadcast Cmd !!!\n"));
#endif
                }
    ...
    IT6602_CEC_PRINT(("initator addr =%X\n", FollowerAddr));
...
#ifdef DEBUG_IT6602_CEC
                IT6602_CEC_PRINT(("Receive !!!Feature Abort !!!\n"));
#endif
...
//=== 12.4  Abort  ====
        case eAbort :		// <MS><0xff> , //example: [0x45 0xff] playback device to AVR
                ...
                {
                    IT6602_CEC_PRINT(("Receive !!!About !!!\n"));
...
//=== 13.1 One Touch Play ===
        case eImageViewOn :	// <MS><0x04> , //example: [0x40 0x04] , Playback device to TV
            ...
            IT6602_CEC_PRINT(("Receive !!!Image View On !!!\n"));
            ...
            //		}else
            //			{
            //				IT6602_CEC_PRINT(("CECList[uc].Active -> FALSE\n");
            //			}
            //	}
            //}
        ...
        case eTextViewOn :	// <MS><0x0D>, example: [0x40 0x0D] , Playback device to TV
            ...
            IT6602_CEC_PRINT(("Receive !!!Text View On !!!\n"));
            break;
        ...
        case eActiveSource :    //<DB> <0X82> <Physical Address of initator> ,example:[0x4f 0x82 AB CD] playback device[AB CD] broadcast acitve source
            {
                IT6602_CEC_PRINT(("Receive !!!Active Source !!!\n"));
...
//xxxxx 2014-0128
                ...
                if(pCECRxBuf->id.SIZE==4)
                {
                    ...
                    if(Myself_LogicAdr==uc || uc==0xf)
                    {
                        ...
                        IT6602_CEC_PRINT(("===CEC Active Port = %X ===\n",rxcurport));
...
//=== 13.2.1 Routing Control ====
        case eRoutingChange :		//<DB><0x80><Original Adr><New Adr>  , size=6		// send by switch device to manually change port
            ...
            IT6602_CEC_PRINT(("Receive !!!Routing Change> !!!\n"));
...
//			}
        ...
        case eRoutingInformation :	//<DB> <0x81> <Physical Address> ,example:[0x5F 0x81 AB CD]  AVR get active route below the switch
            ...
            IT6602_CEC_PRINT(("Receive !!!Rounting Information !!!\n"));

            if( txphyadr[0]==pCECRxBuf->id.OPERAND1 && txphyadr[1]==pCECRxBuf->id.OPERAND2 )
                CECCmdSet(DEVICE_ID_BROADCAST, eRoutingInformation,rxphyadr[rxcurport][0],rxphyadr[rxcurport][1]);
            else
                IT6602_CEC_PRINT(("Invalid -->eRoutingInformation\n"));
        ...
        case eInactiveSource :    	//<MS> <0x9D> <Physical Address>		example:<0x40> <0x9D> <AB> <CD>
            {
                    ...
                    IT6602_CEC_PRINT(("Receive !!!Inactive Source !!!\n"));
                ...
                //xxxxx 2014-0128
                    ...
                    if(Myself_LogicAdr==uc)
                    {
                        ...
                        if(CECList[uc].Active==TRUE)
                        {
                            ...
                            IT6602_CEC_PRINT(("CECList[%X].Active -> FALSE\n",uc));
                            ...
                            if(uc==15)
                            {
                                IT6602_CEC_PRINT(("No CEC device active\n"));
        ...
        case eRequestActiveSource :	// <DB> <0x85> , need to reply <active source> by CEC active device
            IT6602_CEC_PRINT(("Receive !!!Request Active Source !!!\n"));
            {
                //uc=(pCECRxBuf->id.HEADER & 0xf0)>>4;
        ...
        case eSetStreamPath :	// <DB> <0x86> <Physical Address> , example:[0x0F 0x86 AB CD] TV request a streaming path from the specified Physical Address
            {
                ...
                IT6602_CEC_PRINT(("Receive !!!Set Stream Path !!!\n"));
...
// for IT6602+iT6623					if(t_IIC_Buffer.I2cRegisterMap.CEC_Power_Status==0)
// for IT6602+iT6623					{
// for IT6602+iT6623						// Receiver CEC command Queue for I2C slave protocal
// for IT6602+iT6623						t_IIC_Buffer.I2cRegisterMap.CEC_Power_Status=1;		// power on
// for IT6602+iT6623						I2C_CECAddQ(0x35);	//2011/06/09 for ACR cmd
// for IT6602+iT6623					}
...
//=== 13.3 System Stand by ===
        case eStandBy :	// <MS> <0X36> or <DB> <0x36> ,
            IT6602_CEC_PRINT(("nReceive !!!Standby !!!\n"));

// for IT6602+iT6623			// Receiver CEC command Queue for I2C slave protocal
// for IT6602+iT6623			t_IIC_Buffer.I2cRegisterMap.CEC_Power_Status=0;		// power off
// for IT6602+iT6623			I2C_CECAddQ(0x36);	//2011/06/09 for ACR cmd
...
//=== 13.6 System Information ===
        ...
        case eGivePhysicalAddress :    //<MS> <0x83> , example:[0x05 0x83] TV request AVR to return Physical address
            IT6602_CEC_PRINT(("Receive !!!Give Physical Address !!!\n"));
...
//					}
        ...
        case eReportPhysicalAddress :	// <DB> <0x84> <Physicall Adress> <Device type>, example:[0x5F 0x84 AB CD 0]
                ...
                IT6602_CEC_PRINT(("Receive !!!Report Physical Address !!!\n"));
...
//=== 13.13 Remote Control Pass Through ===
        case eUserPressed :	//<MS> <0x44> <UI Command>
            IT6602_CEC_PRINT(("Receive !!!User Control Pressed !!!\n"));
            UICommandCode=pCECRxBuf->id.OPERAND1;		// SAVE UI command code
            break;

        case eUserReleased :	//<MS> <0X45>
            IT6602_CEC_PRINT(("Receive !!!User Control Released !!!\n"));
...
//=== 13.14 Device Power Status ===
        case eGiveDevicePowerStatus:{	// <MS> <0x8F Give Device Power Status> --> <MS> <0x90 Report Power Status> < 0~3 >
            IT6602_CEC_PRINT(("Receive !!!Give Device Power Status !!!\n"));
            ...
            break;
        case eReportPowerStatus:
            IT6602_CEC_PRINT(("Receive !!!Report Power Status !!!\n"));
            break;
...
//=== 13.15.1 System Audio Control ====
        case eAudioModeRequest :{			//<MS> <0x70 Audio Mode Request > <Phy Adr> ,example:[0x05 0x70 00 00] TV send Audio mode request ON to AVR
                ...
                IT6602_CEC_PRINT(("Receive !!!System Audio Mode Request !!!\n"));
        ...
        case eGiveAudioStatus:{			//<MS> <0x71>
            IT6602_CEC_PRINT(("Receive !!!Give Audio Status !!!\n"));
            uc=(pCECRxBuf->id.HEADER & 0xf0)>>4;	// Get initator logic address
        ...
        case eSetSystemAudioMode :{    	//<MS> <0X72> <Audio on/off>
            IT6602_CEC_PRINT(("Receive Set System Audio Mode !!!\n"));
        ...
        case eSystemAudIoModeStatus :	{ //<MS> <0x7E> <System Audio Status>
            IT6602_CEC_PRINT(("Receive !!!System Audio Mode Status !!!\n"));
            if(pCECRxBuf->id.OPERAND1==0)
                bSystemAudioMode=1;	// AVR audio mode ON
            else
                bSystemAudioMode=0;	// AVR audio mode OFF
        ...
        case eRequestAudioDescriptor :{	//<MS> <0xA4> <Audio Format ID and Code>
            IT6602_CEC_PRINT(("Receive !!!Request Short Audio Descriptor !!!\n"));
            uc=(pCECRxBuf->id.HEADER & 0xf0)>>4;	// Get initator logic address
...
//=== ARC ====
        case eInitiateARC :
            IT6602_CEC_PRINT(("Receive !!!Initiate ARC !!!\n"));
...
//			if(CheckUpDownPhyAdr()==TRUE)
        ...
        case eReportARCInitiated :
            IT6602_CEC_PRINT(("Receive !!!Report ARC Initiated !!!\n"));
            break;
        case eReportARCTerminated :
            IT6602_CEC_PRINT(("Receive !!!Report ARC Terminated !!!\n"));

//			uc=IT6602_CEC_ReadI2C_Byte(REG_HEAC_CTRL)&(~(B_ARC_TX_EN|B_ARC_RX_EN|B_SPDIFO_EN|B_HEC_EN));
//
...
//
//			IT6602_CEC_WriteI2C_Byte(REG_HEAC_CTRL,uc);
//			DumpIT6602_CECReg();

            break;
        case eRequestARCInitiation :
            IT6602_CEC_PRINT(("Receive !!!Request ARC Initiation !!!\n"));
//			if(CheckUpDownPhyAdr()==TRUE)
                {
        ...
        case eRequestARCTermination :    //Request ARC Termination
            IT6602_CEC_PRINT(("Receive !!!Request ARC Termination !!!\n"));
            if(CheckUpDownPhyAdr()==TRUE){
                switch_ARCState(sTerminateARC);
                }
            break;
        case eTerminateARC :    //Termiate ARC
            IT6602_CEC_PRINT(("Receive !!!Termiate ARC !!!\n"));
...
void Switch_OneTouchPlayState(OneTouchPlay_StateType uState)
{
    ...
    IT6602_CEC_PRINT(("Switch_OneTouchPlayState--> %X\n",uState));
...
void switch_PollingMessage(PollingMessage_StateType state)
{
        ...
        Myself_LogicAdr==DEVICE_ID_PLAYBACK2 || Myself_LogicAdr==DEVICE_ID_PLAYBACK3 )
    {
        ...
        IT6602_CEC_PRINT(("DVD switch_PollingMessage--> %X\n",state));
        switch(state)
        {
            case sPollingMessage1:
                    ...
                    IT6602_CEC_WriteI2C_Byte(REG_CEC_TARGET_ADDR, Myself_LogicAdr);
...
#else
                    if( IT6602_CECTxOutStatusUpdate() == 0)	// fail
                            switch_PollingMessage(sReportPhyAdr);
                        else if( IT6602_CECTxOutStatusUpdate() == 1)	// sucessful
                            switch_PollingMessage(sPollingMessage2);
#endif
        ...
        case sReportPhyAdr:
                    if(OldTxCECcmd==eReportPhysicalAddress)
                    {
                        ...
                        switch(CECTxState)
                        {
                            ...
                            case sCECReceiverFail:
									//CECCmdSet(DEVICE_ID_BROADCAST, eReportPhysicalAddress,txphyadr[0],txphyadr[1]);
                                    //printf("CECTxState=%X ,Reg44=%bX\n",CECTxState,(IT6602_CEC_ReadI2C_Byte(REG_SYS_STATUS)));
                                    //IT6602_CECDeviceStateReset();	// Give up !!!
                                    break;


                            case sCECOk:
                            case sCECNone:
                                    IT6602_CEC_PRINT(("CECTxState=%X\n",CECTxState));
						...
						//    switch_PollingMessage(sReportPhyAdr);
						//else
                        if( IT6602_CECTxOutStatusUpdate() == 1)	// sucessful
                            switch_PollingMessage(sReportDeviceVendorID);
...
void Switch_LogicalAddressingState(LogicAddressing_StateType uState)
{
    ...
    IT6602_CEC_PRINT(("TV Switch_LogicalAddressingState--> %X\n",uState));
    LAState=uState;
}
        ...
        case sPollingMessage:		// polling next CEC device
            {
                ...
                if(cPollingCount<15)
                {
                ...
                }else
                    {
                        ...
                        for(uc=1;uc<15;uc++)
                        {
                            IT6602_CEC_PRINT(("CECList[%X]= %bX , PhysicalAddr=[%bX][%bX]\n",
                                    uc,
                                    CECList[uc].Active,
...
//
//					}
//					IT6602_CEC_PRINT(("-----Rx Header=%X, opcode=%bX , operand1=%bX , operand2=%bX ,cPollingCount=%bX-----\n ",
//					pCECRxBuf->id.HEADER,
//					pCECRxBuf->id.OPCODE,
//					pCECRxBuf->id.OPERAND1,
...
void switch_ARCState(ARC_StateType uState)
{
    ...
    //if(Myself_LogicAdr==DEVICE_ID_AUDIO||Myself_LogicAdr==DEVICE_ID_TV)
    {
        IT6602_CEC_PRINT(("switch ARCState -> %s  !!!\n",ARCStr[uState]));
        ARCState=uState;
...
// for IT6602+iT6623						// Receiver CEC command Queue for I2C slave protocal
// for IT6602+iT6623						t_IIC_Buffer.I2cRegisterMap.CEC_ARC_Status=1;
// for IT6602+iT6623						I2C_CECAddQ(0xC0);	//2011/06/09 for ACR cmd
                        IT6602_CECCheckFollower();
...
// for IT6602+iT6623						// Receiver CEC command Queue for I2C slave protocal
// for IT6602+iT6623						t_IIC_Buffer.I2cRegisterMap.CEC_ARC_Status=0;
// for IT6602+iT6623						I2C_CECAddQ(0xC0);	//2011/06/09 for ACR cmd
                        IT6602_CECCheckFollower();
            ...
            case	sHEC_InquirState:
                {
                    ...
                    if(pCECRxBuf->id.OPERAND3==eHEC_InquirState)
                    {
                        ...
                        IT6602_CEC_PRINT(("-----Header=%X, opcode=%bX , operand1=%bX , operand2=%bX ,FollowerAddr=%bX-----\n ",
                        pCECRxBuf->id.HEADER,
                        pCECRxBuf->id.OPCODE,
                        pCECRxBuf->id.OPERAND1,
                        pCECRxBuf->id.OPERAND2,
...
{
    IT6602_CEC_PRINT(("!!! SwitchSystemInfoFeature -> %X !!!\n",State);
...
void CECManager(void)
{
    ...
    IT6602_CECReceiver();		// Receive CEC command
    //#endif
    ...
    IT6602_CECTransmitter();
}
// for IT6602+iT6623
// for IT6602+iT6623//
// for IT6602+iT6623//void switch_Standby(unsigned char state)
// for IT6602+iT6623//{
// for IT6602+iT6623//
// for IT6602+iT6623//	StandbyState=state;
// for IT6602+iT6623//
// for IT6602+iT6623//	switch(StandbyState)
// for IT6602+iT6623//	{
// for IT6602+iT6623//		case 0:
// for IT6602+iT6623//			// TODO: nothing!!!
// for IT6602+iT6623//			break;
// for IT6602+iT6623//
// for IT6602+iT6623//		case 1:
// for IT6602+iT6623//			bSystemAudioMode=0;
// for IT6602+iT6623//			CECCmdSet(DEVICE_ID_TV,eSetSystemAudioMode,bSystemAudioMode,0 );
// for IT6602+iT6623//			break;
// for IT6602+iT6623//
// for IT6602+iT6623//		case 1:
// for IT6602+iT6623//			CECCmdSet(DEVICE_ID_BROADCAST,eSetSystemAudioMode,bSystemAudioMode,0 );
// for IT6602+iT6623//			break;
// for IT6602+iT6623//
// for IT6602+iT6623//		case 2:
// for IT6602+iT6623//			CECCmdSet(DEVICE_ID_BROADCAST, eStandBy,0,0);
// for IT6602+iT6623//			break;
// for IT6602+iT6623//	}
// for IT6602+iT6623//}
// for IT6602+iT6623//void StandbyHandler(void)
// for IT6602+iT6623//{
// for IT6602+iT6623//	switch(StandbyState)
// for IT6602+iT6623//	{
// for IT6602+iT6623//		case 0:
...
// for IT6602+iT6623					t_IIC_Buffer.I2cRegisterMap.CEC_Audio_Status=(BYTE)uCECAudioStatus.Map.AudioVolumeStatus;
// for IT6602+iT6623					I2C_CECAddQ(0x41);	//2011/06/09 for ACR cmd


                    IT6602_CEC_PRINT(("CEC volume = %X\n",uCECAudioStatus.Map.AudioVolumeStatus));
...
// for IT6602+iT6623						t_IIC_Buffer.I2cRegisterMap.CEC_Audio_Status=(BYTE)uCECAudioStatus.Map.AudioVolumeStatus;
// for IT6602+iT6623						I2C_CECAddQ(0x42);	//2011/06/09 for ACR cmd
// for IT6602+iT6623						IT6602_CEC_PRINT(("CEC volume = %X\n",uCECAudioStatus.Map.AudioVolumeStatus));
...
void IT6602_CECDeviceStateReset(void)
{
    if(Myself_LogicAdr==DEVICE_ID_TV)
    {
        LAState=sLANone;
        IT6602_CEC_PRINT(("IT6602_CECDeviceStateReset ==> DEVICE_ID_TV\n"));
    }
    else if( Myself_LogicAdr==DEVICE_ID_PLAYBACK1 || Myself_LogicAdr==DEVICE_ID_PLAYBACK2
        || Myself_LogicAdr==DEVICE_ID_PLAYBACK3)
    {
        PMState=sPMnone;
        IT6602_CEC_PRINT(("IT6602_CECDeviceStateReset ==> DEVICE_ID_PLAYBACK1\n"));
    }
}


//xxxxx 2014-0128
void IT6602_CECPhysicalAddressReset(void)
{
    if(Myself_LogicAdr==DEVICE_ID_TV)
    {
        IT6602_CEC_PRINT(("IT6602_CECPhysicalAddressReset ==> DEVICE_ID_TV\n"));
        txphyadr[0]=0x10;
        txphyadr[1]=0x00;
        rxphyadr[0][0] = 0x10;
        rxphyadr[0][1] = 0x00;
        ...
        || Myself_LogicAdr==DEVICE_ID_PLAYBACK3)
    {
        IT6602_CEC_PRINT(("IT6602_CECPhysicalAddressReset ==> DEVICE_ID_PLAYBACK1\n"));
        txphyadr[0]=0x11;
        txphyadr[1]=0x00;
    }

    IT6602_CEC_PRINT(("txphyadr[0]=%02X\n",(int) txphyadr[0]));
    IT6602_CEC_PRINT(("txphyadr[1]=%02X\n",(int) txphyadr[1]));
    IT6602_CEC_PRINT(("rxphyadr[0][0]=%02X\n",(int) rxphyadr[0][0]));
    IT6602_CEC_PRINT(("rxphyadr[0][1]=%02X\n",(int) rxphyadr[0][1]));
    IT6602_CEC_PRINT(("rxphyadr[1][0]=%02X\n",(int) rxphyadr[1][0]));
    IT6602_CEC_PRINT(("rxphyadr[1][0]=%02X\n",(int) rxphyadr[1][1]));
...
void IT6602_CECPowerOffByit660x(unsigned char eCECActivePortNum)
{
    if(eCECActivePortNum == 0)
    {
        IT6602_CEC_PRINT(("IT6602_CECPower Off By it660x() active port --> HDMI PORT 0\n"));
    }
    else
    {
        IT6602_CEC_PRINT(("IT6602_CECPower Off By it660x() active port --> HDMI PORT 1\n"));
    }
...
void IT6602_CECPowerOnByit660x(unsigned char eCECActivePortNum)
{
    if(eCECActivePortNum == 0)
    {
        IT6602_CEC_PRINT(("IT6602_CECPowerOn By it660x() active port --> HDMI PORT 0\n"));
    }
    else
    {
        IT6602_CEC_PRINT(("IT6602_CECPowerOn By it660x() active port --> HDMI PORT 1\n"));
    }
...
void CECReceivePowerOn_Callback(unsigned char eCECActivePortNum)
{
    if(eCECActivePortNum ==0)
    {
        IT6602_CEC_PRINT(("CECReceivePowerOn_Callback() active port --> HDMI PORT 0\n"));
    }
    else
    {
        IT6602_CEC_PRINT(("CECReceivePowerOn_Callback() active port --> HDMI PORT 1\n"));
    }
}

void CECReceivePowerOff_Callback(void)
{
    IT6602_CEC_PRINT(("00000 CECReceivePowerOff_Callback()\n"));
}

void IT6602CEC_RegisterCallBack(void * CECReceivePowerOff_Callback,void * CECReceivePowerOn_Callback)
{
