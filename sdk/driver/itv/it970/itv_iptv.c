...
#define ITV_DMA_CH_SIZE         4
#define ITV_DMA_MEM_TIMEOUT     200
//=============================================================================
//                              Macro Definition
//=============================================================================
...
extern void UdpLowLatencyClose();
extern void UdpLowLatencyOpen(char* targetIp, unsigned short port, int width, int height);
...
static const uint32_t tv1400x1050p60VGAScript[] =
{
#include "../../../target/lcd/lcd_tv_1400x1050p60_VGA.inc"
};
...
static const uint32_t tv1080p24Script[] =
{
#include "../../../target/lcd/lcd_tv_1080p24.inc"
};

static const uint32_t tv1080p25Script[] =
{
#include "../../../target/lcd/lcd_tv_1080p25.inc"
};

static const uint32_t tv1080p29Script[] =
{
#include "../../../target/lcd/lcd_tv_1080p29.inc"
};

static const uint32_t tv1080p30Script[] =
{
#include "../../../target/lcd/lcd_tv_1080p30.inc"
};
...
bool gbDisplayFieldMode = false;
static bool gbDisplayTopField = true;
static bool gbISRTopField = true;

static uint8_t gpSessionKey[16] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10};
static int gDpuFd = -1;

//low latency mode
static bool gbLowLatencyMode = false;
static uint8_t gLowLatencyIP[16] = {0};
static int gLowLatencyPort = 0;
static bool gbColorRange = false; // 0: Limited 1:full range

static uint32_t g_fd_buff_addr_TB = 0;
static uint32_t g_fd_buff_addr_0T = 0;
static uint32_t g_fd_buff_addr_0B = 0;
static uint32_t g_fd_buff_addr_1T = 0;
static uint32_t g_fd_buff_addr_1B = 0;
static uint32_t g_fd_buff_addr_2T = 0;
static uint32_t g_fd_buff_addr_2B = 0;
static uint32_t g_fd_buff_w       = 0;
static uint32_t g_fd_buff_h       = 0;
static uint32_t g_fd_buff_p       = 0;
            ...
            if (itv_rcs.ff_mode_isr[i] == 1)
    		{
                ...
                case 0:
                    ...
                    mmpIspSetFrameFunction(
                            ...
                            MMP_PIXEL_FORMAT_RGB565,//MMP_PIXEL_FORMAT_ARGB8888,
                            ff->ui_ridx);
                    break;
    ...
    */

    {
        if (abz == 0) {
            ...
            {
                mmpIspDisable(gIspDev, MMP_ISP_KEEP_LAST_FIELD_MODE);
                gbDisplayTopField = true;
            }
            ...
            /** Sharpness range is -8 ~ 5, 0 is mapping to 1.0f */
            mmpIspSetColorCtrl(gIspDev, MMP_ISP_HUE, 0);
            mmpIspSetColorCtrl(gIspDev, MMP_ISP_CONTRAST, 0);
            mmpIspSetColorCtrl(gIspDev, MMP_ISP_SATURATION, 128);
            mmpIspSetColorCtrl(gIspDev, MMP_ISP_BRIGHTNESS, 0);
            mmpIspSetColorCtrl(gIspDev, MMP_ISP_SHARPNESS, 0);

            mmpIspPlayImageProcess(gIspDev, &isp_share);
        } else {
            if (gbDisplayFieldMode)
            {
                if (!((itv_rcs.video_update == false) && (itv_rcs.ui_update == true)))
                {
                    if (gbDisplayTopField)
                    {
                        mmpIspEnable(gIspDev, MMP_ISP_KEEP_LAST_FIELD_MODE);

                        if (itv_rcs.curr_video != &itv_rcs.blank_yuv_frame)
                        {
                            if ((itv_rcs.disp_buf_ridx - 1) == 0)
                                isp_share.addrU = g_fd_buff_addr_0B;
                            else if ((itv_rcs.disp_buf_ridx - 1) == 1)
                                isp_share.addrU = g_fd_buff_addr_1B;
                            else
                                isp_share.addrU = g_fd_buff_addr_2B;
                        }

                        gbDisplayTopField = false;
                    }
                    else
                    {
                        mmpIspDisable(gIspDev, MMP_ISP_KEEP_LAST_FIELD_MODE);
                        ...
                        {
                            if ((itv_rcs.disp_buf_ridx - 1) == 0)
                                isp_share.addrU = g_fd_buff_addr_0T;
                            else if ((itv_rcs.disp_buf_ridx - 1) == 1)
                                isp_share.addrU = g_fd_buff_addr_1T;
                            else
                                isp_share.addrU = g_fd_buff_addr_2T;
                        }
...
static void itv_lcdISR(void *arg __attribute__((__unused__)))
{
   ...
   //ithPrintf("xTaskGetTickCountFromISR:%lu\n", xTaskGetTickCountFromISR());
    ...
    if (gbDisplayFieldMode)
    {
        if (gInterlaceCount)
        {
            ithLcdIntrSetScanLine1(ithLcdGetHeight()/2 - 10);
        }
        else
        {
            ithLcdIntrSetScanLine1(ithLcdGetHeight() - 10);
        }
        gInterlaceCount = (gInterlaceCount + 1) & 0x1;

        if (gbISRTopField) gbISRTopField = false;
        else gbISRTopField = true;
    }
...
/*nowTime = ithTimerGetCounter(portTIMER);
    ...
    if (abz) {
	    ...
	    if ((itv_rcs.disp_buf_ridx != itv_rcs.disp_buf_widx) && (itv_rcs.video_update == false))
	    {
            if (gbDisplayFieldMode)
            {
                if (!gbISRTopField)
                {
                    ithPrintf("gbISRTopField false, wait bottom field\n");
                }
                else
                {
...
static void itv_change_display_format()
{
    ...
    if (gtFlipCtrl.bInited)
    {
    	...
    	/* enable lcd ISR */
    	{
            ...
            if (gbDisplayFieldMode)
            {
                ithLcdIntrSetScanLine1(lcd_height/2 - 10);
	            ithLcdIntrCtrlEnable(ITH_LCD_INTR_FIELDMODE1);
                gInterlaceCount = 0;
            }
            else
            {
                ithLcdIntrSetScanLine1(lcd_height - 10);
...
int itv_init(void)
{
    ...
    /* sanity check */
    g_blank_buf = itpVmemAlloc(160*160*2);

    if (gbDisplayFieldMode)
    {
        g_fd_buff_addr_0T = itpVmemAlloc(1920 * 540);
        g_fd_buff_addr_0B = itpVmemAlloc(1920 * 540);
        g_fd_buff_addr_1T = itpVmemAlloc(1920 * 540);
        g_fd_buff_addr_1B = itpVmemAlloc(1920 * 540);
        g_fd_buff_addr_2T = itpVmemAlloc(1920 * 540);
        g_fd_buff_addr_2B = itpVmemAlloc(1920 * 540);

        printf("ITV_DMA_BUFFER: %x,%x,%x,%x,%x,%x\n", g_fd_buff_addr_0T, g_fd_buff_addr_0B, g_fd_buff_addr_1T, g_fd_buff_addr_1B, g_fd_buff_addr_2T, g_fd_buff_addr_2B);
    }
    ...
    itv_rcs.blank_yuv_frame.ua       = (uint8_t *)g_blank_buf + 25600;
    itv_rcs.blank_yuv_frame.va       = (uint8_t *)g_blank_buf + 25600 + 6400;
    
    memset((void *) itv_rcs.blank_yuv_frame.ya, 0x10, 25600);
    memset((void *) itv_rcs.blank_yuv_frame.ua, 0x80, 6400);
    memset((void *) itv_rcs.blank_yuv_frame.va, 0x80, 6400);

	ithFlushDCacheRange((void *) g_blank_buf, 160*160*2);
	
    itv_rcs.blank_yuv_frame.src_w    = 160;
    itv_rcs.blank_yuv_frame.src_h    = 160;
    itv_rcs.blank_yuv_frame.pitch_y  = 160;
    itv_rcs.blank_yuv_frame.pitch_uv = 160;
...
int itv_deinit(void)
{
    ...
    if (gbDisplayFieldMode)
    {
        itpVmemFree(g_fd_buff_addr_0T);
        itpVmemFree(g_fd_buff_addr_0B);
        itpVmemFree(g_fd_buff_addr_1T);
        itpVmemFree(g_fd_buff_addr_1B);
        itpVmemFree(g_fd_buff_addr_2T);
        itpVmemFree(g_fd_buff_addr_2B);
...
int itvDmaTx2D(uint32_t srcAddr, uint32_t dstAddr,
    ...
    int16_t srcStride, int16_t dstStride)
{
    ...
    {
        printf("%s: srcStride %d, dstStride %d => need 64-bits alignment \n", __func__, srcStride, dstStride);
        return -1;
    }

    ch = ithDmaRequestCh("m2m", ITH_DMA_CH_PRIO_HIGH, NULL, NULL);
    if (ch < 0)
    {
        printf("%s() no available dma channel \n", __func__);
        return -1;
    }
    ithDmaReset(ch);
...
{
    int dma_ch[ITV_DMA_CH_SIZE] = {0};
    int dma_ch_index = 0;
    uint32_t dma_ch_status = 0;
    uint32_t timeout_ms = ITV_DMA_MEM_TIMEOUT;
    ...
    if (gbDisplayFieldMode)
    {
        g_fd_buff_w = itv_rcs.dbufprop[itv_rcs.disp_buf_widx].src_w;
        g_fd_buff_h = itv_rcs.dbufprop[itv_rcs.disp_buf_widx].src_h / 4;
        g_fd_buff_p = itv_rcs.dbufprop[itv_rcs.disp_buf_widx].pitch_uv * 2;
        g_fd_buff_addr_TB = itv_rcs.dbufprop[itv_rcs.disp_buf_widx].ua;
        //printf("YC2: a = %x, w = %d, h=%d, p=%d\n",g_fd_buff_addr_TB, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p);

        //start_tick = xTaskGetTickCount();

        if (itv_rcs.disp_buf_widx == 0)
        {
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB, g_fd_buff_addr_0T, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB, g_fd_buff_addr_0T + g_fd_buff_w, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB + g_fd_buff_w, g_fd_buff_addr_0B, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB + g_fd_buff_w, g_fd_buff_addr_0B + g_fd_buff_w, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
        }
        else if (itv_rcs.disp_buf_widx == 1)
        {
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB, g_fd_buff_addr_1T, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB, g_fd_buff_addr_1T + g_fd_buff_w, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB + g_fd_buff_w, g_fd_buff_addr_1B, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB + g_fd_buff_w, g_fd_buff_addr_1B + g_fd_buff_w, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
        }
        else
        {
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB, g_fd_buff_addr_2T, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB, g_fd_buff_addr_2T + g_fd_buff_w, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB + g_fd_buff_w, g_fd_buff_addr_2B, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
            dma_ch[dma_ch_index++] = itvDmaTx2D(g_fd_buff_addr_TB + g_fd_buff_w, g_fd_buff_addr_2B + g_fd_buff_w, g_fd_buff_w, g_fd_buff_h, g_fd_buff_p, g_fd_buff_p);
        }
        ...
        while (--timeout_ms)
        {
            ...
            for (dma_ch_index = 0; dma_ch_index < ITV_DMA_CH_SIZE; dma_ch_index++)
                dma_ch_status |= ithDmaIsBusy(dma_ch[dma_ch_index]);

            if (!dma_ch_status) break;
            else usleep(100);
        }

        if (!timeout_ms)
            printf("ITV dma timeout!\n");

        for (dma_ch_index = 0; dma_ch_index < ITV_DMA_CH_SIZE; dma_ch_index++)
            ithDmaFreeCh(dma_ch[dma_ch_index]);
...
int itv_set_output_format(MMP_CAP_INPUT_INFO df, uint32_t audio_sample_rate)
{
	...
	if (bChangeDisplay)
	{
		//ithHDMITXDisable();
		//itv_deinit();
        printf("start change display format.....\n");
		ituFrameFuncExit();
		ithLcdReset();
		...
		switch(df)
		{
			...
			case MMP_CAP_INPUT_INFO_1440X900_60P:
                ...
                gDisplayFrameRate = 60;
				break;
			case MMP_CAP_INPUT_INFO_1400X1050_60P:
				ithLcdLoadScriptFirst(tv1400x1050p60VGAScript, ITH_COUNT_OF(tv1400x1050p60VGAScript));
				ithWriteRegMaskA(0xD1000160, 0x110, 0x110);  //reset i2c gpio (65, 66)
				ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
				ithHDMITXSetDisplayOption(HDMITX_VESA, HDMITX_VESA_1400x1050p60, TRUE, FALSE);
                gDisplayFrameRate = 60;
				break;
			...
			case MMP_CAP_INPUT_INFO_1280X720_60P:
                ...
                gDisplayFrameRate = 60;
				break;
            case MMP_CAP_INPUT_INFO_1920X1080_24P:
                ithLcdLoadScriptFirst(tv1080p24Script, ITH_COUNT_OF(tv1080p24Script));
                ithWriteRegMaskA(0xD1000160, 0x110, 0x110);  //reset i2c gpio (65, 66)
                ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
                ithHDMITXSetDisplayOption(HDMITX_1080p24, HDMITX_CEA_1920x1080p24, TRUE, FALSE);
                gDisplayFrameRate = 24;
                break;
            case MMP_CAP_INPUT_INFO_1920X1080_25P:
                ithLcdLoadScriptFirst(tv1080p25Script, ITH_COUNT_OF(tv1080p25Script));
                ithWriteRegMaskA(0xD1000160, 0x110, 0x110);  //reset i2c gpio (65, 66)
                ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
                ithHDMITXSetDisplayOption(HDMITX_1080p25, HDMITX_CEA_1920x1080p25, TRUE, FALSE);
                gDisplayFrameRate = 25;
                break;
			case MMP_CAP_INPUT_INFO_1920X1080_29P:
                ithLcdLoadScriptFirst(tv1080p29Script, ITH_COUNT_OF(tv1080p29Script));
                ithWriteRegMaskA(0xD1000160, 0x110, 0x110);  //reset i2c gpio (65, 66)
                ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
                ithHDMITXSetDisplayOption(HDMITX_1080p30, HDMITX_CEA_1920x1080p30, TRUE, FALSE);
                gDisplayFrameRate = 30;
                break;	
            case MMP_CAP_INPUT_INFO_1920X1080_30P:
                ithLcdLoadScriptFirst(tv1080p30Script, ITH_COUNT_OF(tv1080p30Script));
                ithWriteRegMaskA(0xD1000160, 0x110, 0x110);  //reset i2c gpio (65, 66)
                ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
                ithHDMITXSetDisplayOption(HDMITX_1080p30, HDMITX_CEA_1920x1080p30, TRUE, FALSE);
                gDisplayFrameRate = 30;
		...
		//itv_init();
		ituFrameFuncInit();
        printf("change display format is done.....\n");
	}
	return 0;
}
				...
				{					
					...
					//if (itv_rcs.df != (msg->data & 0x1F))
					{
						...
						if(mtal_get_low_latency_flag())
						{
							...
							UdpLowLatencyOpen(gLowLatencyIP, gLowLatencyPort, width, height);
							...
							if (mtal_pb_stop() != -1)
							{
								...
								switch(msg->data & 0xFF)
								{
									...
									case MMP_CAP_INPUT_INFO_1440X900_60P:
										...
										break;
									case MMP_CAP_INPUT_INFO_1400X1050_60P:
										mtal_spec.strm_spec[0].src_w = 1400;
...
void mtal_set_source_lock_flag(bool bSourceLock)
{
    gbSourceLockMode = bSourceLock;
}

bool mtal_get_low_latency_flag(void)
{
    if (gbLowLatencyMode)
    {
        return true;
...
void mtal_set_low_latency_flag(bool bLowLatency)
{
    gbLowLatencyMode = bLowLatency;
}

void mtal_set_low_latency_ipaddress(char *ip, int port)
{
	strcpy(gLowLatencyIP, ip);
	gLowLatencyPort = port;
}

void mtal_set_color_info(bool color_range)
{
...
int mtal_pb_InitH264DecodeEnv(int width, int height)
{
    printf("MTAL# %s +\n", __func__);
    if (media_player && media_player->InitH264DecodeEnv)
    {
        media_player->InitH264DecodeEnv(width, height);
    }
...
int mtal_pb_InitAudioDecodeEnv(int samplerate, int num_channels)
{
    printf("MTAL# %s +\n", __func__);
    if (media_player && media_player->InitAudioDecodeEnv)
    {
        media_player->InitAudioDecodeEnv(samplerate, num_channels);
    }
...
int mtal_pb_h264_decode(unsigned char *buf, int size, double timestamp)
{
    //printf("MTAL# %s +\n", __func__);
    if (media_player && media_player->h264_decode)
    {
        int retv = media_player->h264_decode(buf, size, timestamp);
        if (retv < 0)
            SERR();
    }
    //printf("MTAL# %s -\n", __func__);
...
int mtal_pb_audio_decode(unsigned char *buf, int size, double timestamp)
{
    //printf("MTAL# %s +\n", __func__);
    if (media_player && media_player->audio_decode)
    {
        int retv = media_player->audio_decode(buf, size, timestamp);
        if (retv < 0)
            SERR();
    }
    //printf("MTAL# %s -\n", __func__);
