...
static void risc2SwIRProcessCommand(int cmdId)
{
    int i = 0;
...
static int risc2TestDeviceIoctl(int file, unsigned long request, void *ptr, void *info)
{
    ...
    switch (request)
    {
        ...
        case ITP_IOCTL_INIT_SW_IRRX_PARAM:
        {
            SW_IR_INIT_DATA* ptInitData = (SW_IR_INIT_DATA*) ptr;
            memcpy(pExchangeAddress, ptInitData, sizeof(SW_IR_INIT_DATA));
#ifdef CFG_CPU_WB
            ithFlushDCacheRange((void*)pExchangeAddress, sizeof(SW_IR_INIT_DATA));
            ithFlushMemBuffer();
#endif
            risc2SwIRProcessCommand(INIT_CMD_ID);
            break;
        }
...
const ITPDevice itpDeviceSwIRRx =
{
    ":swIRRx",
    itpOpenDefault,
    itpCloseDefault,
