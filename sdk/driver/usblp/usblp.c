...
#define usb_register_dev(intf, x) ({intf->minor=0; 0;})
#define usb_deregister_dev(intf, x) 0
...
#define mutex_lock_interruptible mutex_lock
#define usb_alloc_urb(x,y) usb_alloc_urb(x)
#define usb_submit_urb(x,y) usb_submit_urb(x)
...
#undef  DECLARE_WAITQUEUE
#define DECLARE_WAITQUEUE(name, tsk) do {} while (0)

#define dev_dbg(dev, format, arg...)		\
	ithPrintf("[i] dev=0x%08X, ", dev);      \
	ithPrintf(format, ##arg);

#define dev_err(dev, format, arg...)		\
	ithPrintf("[X] dev=0x%08X, ", dev);      \
	ithPrintf(format, ##arg);

#define printk_ratelimited(fmt, ...)  \
({                                    \
		ithPrintf(fmt, ##__VA_ARGS__);    \
})
...
static void usblp_cleanup(struct usblp *usblp)
{
  ...
  sem_destroy(&usblp->rsem);
  sem_destroy(&usblp->wsem);
	kfree(usblp);
}
...
{
  return 0; // got no way to support poll
}

// linux prototype: int (*ioctl) (struct usb_interface *intf, unsigned int code, void *buf);
// sdk prototype:   int (*ioctl) (struct usb_device    *dev,  uint32_t     code, void *buf);
static int usblp_ioctl(struct usb_interface *intf, unsigned int code, void *buf)
{
	struct usblp *usblp = usb_get_intfdata(intf); //dev_get_drvdata(&udev->dev);
	int length, err, i;
	unsigned char newChannel;
	int status;
	int twoints[2];
	int retval = 0;
  unsigned int cmd=(unsigned int) code;
  unsigned long arg=(unsigned long) buf;
	...
	if (_IOC_TYPE(cmd) == 'P')	/* new-style ioctl number */
		...
		case IOCNR_GET_VID_PID:
			...
			dev_dbg(&usblp->intf->dev,
				"usblp%d is VID=0x%04X, PID=0x%04X\n",
				usblp->minor, twoints[0], twoints[1]);
			break;
...
static int usblp_wwait(struct usblp *usblp, int nonblock)
{
    ...
    if (itpSemWaitTimeout(&usblp->wsem, 1500)==0) {
...
// linux prototype: int (*probe)(struct usb_interface *intf, const struct usb_device_id *id);
// sdk   prototype: void *(*probe)(struct usb_interface *intf, const struct usb_device_id *id);

static void* usblp_probe(struct usb_interface *intf,
		       const struct usb_device_id *id)
{
  ...
  ithPrintf("[i] +++%s\n", __FUNCTION__);
  ithPrintf("[i] idVendor:%04X idProduct:%04X\n", id->idVendor, id->idProduct);
  ithPrintf("[i] bDeviceClass:%02X bDeviceSubClass:%02X bDeviceProtocol:%02X\n", (unsigned int)id->bDeviceClass, (unsigned int)id->bDeviceSubClass, (unsigned int)id->bDeviceProtocol);
  ithPrintf("[i] bInterfaceClass:%02X bInterfaceSubClass:%02X bInterfaceProtocol:%02X\n", (unsigned int)id->bInterfaceClass, (unsigned int)id->bInterfaceSubClass, (unsigned int)id->bInterfaceProtocol);
  ithPrintf("[i] bInterfaceNumber:%02X\n", (unsigned int)id->bInterfaceNumber);
  ...
  sem_init(&usblp->rsem, 0, 1);
  sem_init(&usblp->wsem, 0, 1);
	usblp->dev = dev;
	...
	if (protocol < 0) {
		dev_dbg(&intf->dev,
			"incompatible printer-class device 0x%04X/0x%04X\n",
			le16_to_cpu(dev->descriptor.idVendor),
			le16_to_cpu(dev->descriptor.idProduct));
		retval = -ENODEV;
		goto abort;
...
/*
	dev_info(&intf->dev,
		"usblp%d: USB %sdirectional printer dev %d if %d alt %d proto %d vid 0x%04X pid 0x%04X\n",
...
*/
  ...
  INIT_LIST_HEAD(&usblp->list);
  list_add_tail(&usblp->list, &usblp_list);
	return usblp; //0;
...
static const struct usb_device_id usblp_ids[] = {
	{ USB_DEVICE(0x0483, 0x5840) },	/* Aclass-Printer */
...
int usblp_register_driver()
{
  //INIT_LIST_HEAD(&usblp_list);
...
static int usblpIsHandleValid(void* h)
{
  struct list_head *tmp;
  
  for (tmp=usblp_list.next; tmp!=&usblp_list; tmp=tmp->next) {
    if (h==list_entry(tmp, struct usblp, list)) return 1;
  }
  return 0;
}
...
void* usblpGetHandle(int pos)
{
  ...
  for (tmp=usblp_list.next; (pos>=0) && (tmp!=&usblp_list); pos--, tmp = tmp->next) {
    if (pos==0) return list_entry(tmp, struct usblp, list);
  }
  return NULL;
}
...
int usblpRead(void* h, char* buffer, unsigned int len)
{
  ...
  if (!usblpIsHandleValid(h)) return -1;
  
  memset(&file, 0, sizeof(file));
  file.private_data=h;

  return (int) usblp_read(&file, buffer, len, NULL);
}
...
int usblpWrite(void* h, char* buffer, unsigned int len)
{
  ...
  return (int) usblp_write(&file, buffer, len, NULL);
}
...
int usblpIoctl(void* h, unsigned int code, void *buf, unsigned int len)
{
  ...
  switch (code) {
    default:
      return -ENOTTY;
    case USBLP_IOCTL_GET_ID:
      code=LPIOC_GET_DEVICE_ID(len);
      break;
    case USBLP_IOCTL_GET_PORT_STATUS:
    code=LPGETSTATUS;
      break;
    case USBLP_IOCTL_SOFT_RESET:
      code=LPIOC_SOFT_RESET;
      break;
    case USBLP_IOCTL_HP_SET_CHANNEL:
      code=LPIOC_HP_SET_CHANNEL;
      break;
    case USBLP_IOCTL_GET_VID_PID:
      code=LPIOC_GET_VID_PID(len);
      break;
    case USBLP_IOCTL_GET_BUS_ADDRESS:
      code=LPIOC_GET_BUS_ADDRESS(len);
      break;
    case USBLP_IOCTL_GET_PROTOCOLS:
      code=LPIOC_GET_PROTOCOLS(len);
      break;
    case USBLP_IOCTL_SET_PROTOCOL:
      code=LPIOC_SET_PROTOCOL;
      break;
  }

  return usblp_ioctl(((struct usblp*)h)->intf, code, buf);
}
