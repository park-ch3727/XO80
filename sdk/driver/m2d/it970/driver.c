...
#ifdef CFG_LCD_PQ_TUNING
    if (GfxDriver.displaySurfaceIndex == 0)
        inputInfo_addrY = ithLcdGetBaseAddrA();
    else if (GfxDriver.displaySurfaceIndex == 1)
        inputInfo_addrY = ithLcdGetBaseAddrB();
    else
        inputInfo_addrY = ithLcdGetBaseAddrC();

    ithLcdVPOnFlyUpdate(inputInfo_addrY);
#else
...
gfxPQTuning(MMP_ISP_COLOR_CTRL colorCtrl, int value)
{
    ...
    if (ithLcdVPOnFlyCCUpdate(colorCtrl, value) == ISP_SUCCESS)
        result = true;
