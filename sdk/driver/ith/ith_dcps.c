    ...
    if (DcpsInfo->IsEnableComQ)
    {
        ...
        if (DcpsInfo->DcpsMode)
        {
            ...
            if (DcpsInfo->DcpsMode == 1)
            {
                ...
                printf("RUN SPEEDY compress cmdQ fire\n");
            }
            else if (DcpsInfo->DcpsMode == 2)
            {
                if (DcpsInfo->LzCpsBytePerPxl == 1)
                {
                    ithWriteRegA(ITH_DCPS_REG_DCR0, 0x0000012E);         //1-bit SPEEDY format
                }
                else if (DcpsInfo->LzCpsBytePerPxl == 2)
                {
                    ithWriteRegA(ITH_DCPS_REG_DCR0, 0x0000022E);         //2-bit SPEEDY format
                }
                else
                {
                    ithWriteRegA(ITH_DCPS_REG_DCR0, 0x0000042E);         //4-bit SPEEDY format
                }
                printf("RUN SPEEDY decompress cmdQ fire\n");
            }
            else
            {
                printf("CMDQ ERROR\n");
