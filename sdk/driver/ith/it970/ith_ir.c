...
void ithIrInit(unsigned int pin, unsigned long extClk, unsigned long sample, unsigned int precision)
{
    ...
    ithIrSetGpio(ITH_IR0, pin);
   	ithIrSetCaptureMode(ITH_IR0, ITH_IR_RISING);
   	//ithIrSetCaptureMode(ITH_IR0, ITH_IR_BOTH);  //Test Tx->Rx Local Case
    ithIrCtrlEnable(ITH_IR0, ITH_IR_DEBOUNCE);
    ithIrCtrlEnable(ITH_IR0, ITH_IR_SIGINVESE);
    ithIrCtrlEnable(ITH_IR0, ITH_IR_TMRST);
    ithIrCtrlEnable(ITH_IR0, ITH_IR_WARP);
    ithIrClear(ITH_IR0);
...
void ithIrTxInit(unsigned int pin, unsigned long extClk, unsigned long sample, unsigned int precision)
{
    ...
    //ithIrTxSetCaptureMode(ITH_IR0, ITH_IRTX_CFG_LENGTH);
    //ithIrTxCtrlEnable(ITH_IR0, ITH_IR_TX_SIGINVESE);
    ithIrTxClear(ITH_IR0);
