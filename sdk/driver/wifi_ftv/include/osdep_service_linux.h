...
#if ITE_EASON_MODIFIED //eason add, replace for linux/version.h
...
#define LINUX_VERSION_CODE KERNEL_VERSION(2, 6, 35)
    ...
    #include <basic_types.h> //eason add for BOOLEAN in osdep_service.h
...
#if ITE_EASON_MODIFIED
	#define _sema	sem_t
    #define sema_init(_sema_, _val_) sem_init(_sema_, 0,_val_)
#else
...
#if ITE_EASON_MODIFIED //(LINUX_VERSION_CODE < KERNEL_VERSION(2,6,22))
...
#else
#if ITE_EASON_MODIFIED
    /* Not implement in RTOS..., so using mutex_lock */
    ret = mutex_lock(pmutex);
#else
...
{
#if ITE_EASON_MODIFIED//(LINUX_VERSION_CODE > KERNEL_VERSION(2,5,41))
	schedule_work(pwork);
#else
	schedule_task(pwork);
#endif
