...
_func_enter_;
    ...
    //DBG_871X("+usb_write_port seqnum(%d) pktlen(%d) case(%d) %s\n", pattrib->seqnum, pattrib->pktlen, addr, RTW_CANNOT_TX(padapter) ? "CANNOT TX":"CAN TX");
    //usleep(1000);
...
void usb_read_port_complete(struct urb *purb, struct pt_regs *regs)
{
	...
	RT_TRACE(_module_hci_ops_os_c_,_drv_err_,("usb_read_port_complete!!!\n"));
    usleep(5*1000); //eason workaround
