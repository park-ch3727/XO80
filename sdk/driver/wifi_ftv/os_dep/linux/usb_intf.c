...
static struct usb_device_id rtw_usb_id_tbl[] ={
	...
	/*=== Realtek demoboard ===*/
	{USB_DEVICE_AND_INTERFACE_INFO(USB_VENDER_ID_REALTEK, 0xF179, 0xff, 0xff, 0xff), .driver_info = (unsigned long)RTL8188F}, /* 8188FU 1*1 */
#endif
...
static int rtw_drv_init(struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
    ...
    for (id_count = 0; id_count < rtw_usb_id_len; id_count++)
	{
	   if (dev->descriptor.idVendor == rtw_usb_id_tbl[id_count].idVendor &&
		   dev->descriptor.idProduct == rtw_usb_id_tbl[id_count].idProduct)
	   {
		   printk("idVendor = 0x%X, idProduct = 0x%X \n",dev->descriptor.idVendor, dev->descriptor.idProduct);
		   break;
	   }
	}
	if (id_count == rtw_usb_id_len) {
	...
	if ((dvobj = usb_dvobj_init(pusb_intf, pdid)) == NULL) {
        ...
        DBG_871X("initialize device object priv Success!\n");
	}
	...
	if ((if1 = rtw_usb_if1_init(dvobj, pusb_intf)) == NULL) {
        ...
        DBG_871X("rtw_usb_if1_init Success!\n");
	}
...
exit:
	return (status == _SUCCESS) ? _SUCCESS:-ENODEV;
}
