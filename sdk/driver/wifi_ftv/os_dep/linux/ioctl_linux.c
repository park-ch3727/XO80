    ...
    if(netDeviceInfo->infoType == WLAN_INFO_WIFI_STATE)
    {
       ...
       // printf("WLAN_INFO_WIFI_STATE 0x%x, 0x%X 0x%x  adapter 0x%x \n",pmlmepriv,get_fwstate(pmlmepriv),pmlmepriv->fw_state,padapter);
        ...
        for (i = 0; i < MAX_CHANNEL_NUM; i++) {
            ...
            if(rfctl->channel_set[i].ChannelNum != 0 && rfctl->channel_set[i].ChannelNum <= 11)
            {
                if(rfctl->channel_set[i].rx_count < rfctl->channel_set[tempindex].rx_count)
                {
                    tempindex = i;
                    bestchannel = rfctl->channel_set[i].ChannelNum;
                }
            }
        }
#endif
...
//      _adapter *padapter = (_adapter *)netdev_priv(dev);
    ...
    else if (netDeviceInfo->infoType == WLAN_INFO_SCAN_GET)
    {
        ...
        while((work - scanResult) < iwReq.data.length && i<63)
        {
            ...
            //printf("====>SSID: %32s SecurityMode(%d)\n", netDeviceInfo->apList[i].ssidName, netDeviceInfo->apList[i].securityMode);
