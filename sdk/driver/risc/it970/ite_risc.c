...
#define RISC3_SRAM_SIZE        (32*1024)
    ...
    int             dataSize)
{
    switch (loadTarget)
    {
        ...
        case RISC2_IMAGE_MEM_TARGET:
            ...
            break;
        case RISC3_IMAGE_MEM_TARGET:
            if (dataSize > RISC3_SRAM_SIZE)
            {
                return INVALID_LOAD_SIZE;
            }
            memset(iteRiscGetTargetMemAddress(loadTarget), 0x0, RISC3_SRAM_SIZE);
            break;
        case AUDIO_MESSAGE_MEM_TARGET:
...
#ifdef CFG_CPU_WB
    if (loadTarget != RISC3_IMAGE_MEM_TARGET)
    {
