...
#endif
//Just prvent RTSP_CLIENT_ENABLE build fail
...
static MMP_CAP_FRAMERATE        gEncode_Framerate = MMP_CAP_FRAMERATE_UNKNOW;
static MMP_UINT32               gFramerateNum     =25;
    ...
    VIDEO_ENCODE_MODE mode)
{
    ...
    for (i = 0; i < gptAVCEncoder[0]->sourceBufCount; i++)
    {
        ...
        gptAVCEncoder[0]->pSourceBufAdrY[i]     = gISPBUUFERINFO.pISPBufAdrY[i];
        gptAVCEncoder[0]->pSourceBufAdrU[i]     = gISPBUUFERINFO.pISPBufAdrU[i];
		gptAVCEncoder[0]->pSourceBufAdrV[i]     = gISPBUUFERINFO.pISPBufAdrU[i];
    ...
    gptAVCEncoder[0]->frameWidth = gptAVCEncoder[0]->framePitchY = cfg0.width = gEncode_Width;
    gptAVCEncoder[0]->frameHeight = cfg0.height = gEncode_Height;
    ...
    if (gEncode_Framerate == MMP_CAP_FRAMERATE_25HZ)
    {
        cfg0.frameRateDenom = 1;
        cfg0.frameRateNum = 25;
		gFramerateNum = 25;
		
    } else if(gEncode_Framerate == MMP_CAP_FRAMERATE_30HZ){
        cfg0.frameRateDenom = 1;
        cfg0.frameRateNum = 30;
		gFramerateNum = 30;
		
    }     
	else if(gEncode_Framerate == MMP_CAP_FRAMERATE_60HZ){
        cfg0.frameRateDenom = 1;
...
void H264SliceReady(H264EncSliceReady *slice)
{
    ...
    for (i = slice->slicesReadyPrev; i < slice->slicesReady; i++)
    {
        ...
        if (slice->slicesReadyPrev == 0 && ((gtAVCEncoderFlow.encodedFrameCount % gFramerateNum) == 1))
    ...
    void *arg)
{
    ...
    while (gbStream0Enable && gbVideoEncoder)
    {
         ...
         if (QUEUE_NOT_EMPTY == gtAVCEncoderFlow.pfEnDequeue(&tEntry) && gbEncodeStart)
         {
             ...
             if ((gtAVCEncoderFlow.encodedFrameCount % gFramerateNum) == 1)
...
ENCODING_PROC :
             ...
             if ((gtAVCEncoderFlow.frameCount[gEncodeIdx] % gFramerateNum) == 1)
                 encIn.codingType = H264ENC_INTRA_FRAME;
             else
                 encIn.codingType = H264ENC_PREDICTED_FRAME;
...
void VideoEncoder_SetISPBUFFERINFO(ISP_BUFFER_INFO buf_addr)
{
	gISPBUUFERINFO = buf_addr;
}
