...
void I2S_DA32_WAIT_RP_EQUAL_WP(int wait)
{
    ...
    if(_dacRPwait){
        ...
        while(GET_DA_RW_GAP>bytes){
            usleep(2000*(GET_DA_RW_GAP/bytes));
            i++;
            if(i>100)  {break; printf("I2S# %d should not be happened\n", __LINE__);}
            if(!_dacRPwait) break;
        }
    }
    _dacRPwait=false;
...
void i2s_frection_set(int data){
    int32_t fractionalPart;
    int sampleR;
    int32_t signBit = (ithReadRegA(0xD800011C) & 0x80000);
    
    itp_codec_get_i2s_sample_rate(&sampleR);
    if(sampleR == 32000 || sampleR == 48000){
        data+=215;
    }else if(sampleR == 44100){
        data+=254;
    }else{
        data+=0;
    } 
    ...
    if(signBit) {
        ithWriteRegMaskA(0xD800011C,1<<19,1<<19);
        data = 1024-data;
    }
    fractionalPart = data*1024;
    ithWriteRegMaskA(0xD800011C,fractionalPart,0x7FFFF);  
    
    /*if(signBit)
        printf("(-)i2s=%x ",fractionalPart);
    else
        printf("(+)i2s=%x ",fractionalPart);
    */
}
...
#ifdef CFG_IPTV_RX
	| (1                                 << 6) /* Output Channel active level, 0:Low for Left; 1:High for Left */
#else
...
void i2s_init_ADC(STRC_I2S_SPEC *i2s_spec)
{
    ...
    ithWriteRegA(I2S_REG_IN2_BASE4, (u32)i2s_spec->base_hdmi[0]);
    ithWriteRegA(I2S_REG_IN2_BASE3, (u32)i2s_spec->base_hdmi[1]);
    ithWriteRegA(I2S_REG_IN2_BASE2, (u32)i2s_spec->base_hdmi[2]);
    ithWriteRegA(I2S_REG_IN2_BASE1, (u32)i2s_spec->base_hdmi[3]);
...
uint32_t i2s_da_data_put(unsigned char *ptr,unsigned int size)
{
    ...
    if ((DA_w + size) > buffer_size){
        int szbuf = buffer_size - DA_w;
        if (szbuf > 0)
        {
            memcpy(base_i2s + DA_w, ptr, szbuf);
            ithFlushDCacheRange(base_i2s+DA_w, szbuf);
        }
        DA_w = size - szbuf;
        memcpy(base_i2s, ptr + szbuf, DA_w);
        ithFlushDCacheRange(base_i2s, DA_w);
    }
    else
    {
        memcpy(base_i2s + DA_w, ptr, size);
        ithFlushDCacheRange(base_i2s+DA_w, size);
        DA_w += size;
    }
    
    if (DA_w == buffer_size) DA_w = 0;
