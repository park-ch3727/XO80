...
static void _i2s_set_clock(u32 demanded_sample_rate , u8 *demanded_sample_width)
{
    ...
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_40, 0x0002B801);   //AMCLK PLL2_OUT2 /1 ,[6:0]:amclk_ratio (AMCLK: 12.288MHz)
   ...
   #else
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_40, 0x0002B801); //AMCLK PLL2_OUT2 /1 ,[9:0]:amclk_ratio (1)

    if(demanded_sample_rate == 32000){
        //12.288 MHz
        ithWriteRegA(0xd800011c,0x000147ae); 
        ithWriteRegA(0xd8000118,0xa02e2d01); 
        ithWriteRegA(0xd800011c,0x800147ae); 
        ithWriteRegA(0xd800011c,0xF04147ae); 
        ithDelay(220);
        ithWriteRegA(0xd800011c,0x800147ae); 

    }else if(demanded_sample_rate == 44100){
        //11.2896 MHz
        ithWriteRegA(0xd800011c,0x00005e35); 
        ithWriteRegA(0xd8000118,0xa04f5401); 
        ithWriteRegA(0xd800011c,0x80005e35); 
        ithWriteRegA(0xd800011c,0xF9005e35); 
        ithDelay(220);
        ithWriteRegA(0xd800011c,0x80005e35); 
    }else{
        //12 MHz
...
static void _i2s_set_clock(u32 demanded_sample_rate , u8 *demanded_sample_width)
{
    ...
    printf("I2S# %s, demanded_sample_rate: %u\n", __func__, demanded_sample_rate);
    //48K init for FPGA wm8960 24.576M
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_40, 0x0002c802);  
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, 0xf2aac808);  
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C, 0x02aac808);  

    switch (demanded_sample_rate)
    {
        case 48000: amclk_ratio = 0x2; zclk_ratio = 0x8; break;
        case 24000: amclk_ratio = 0x4; zclk_ratio = 0x10; break;
        case 12000: amclk_ratio = 0x8; zclk_ratio = 0x10; break;
        case 44100: amclk_ratio = 0x2; zclk_ratio = 0x8; break;
        case 22050: amclk_ratio = 0x4; zclk_ratio = 0x10; break;
        case 11025: amclk_ratio = 0x8; zclk_ratio = 0x20; break;
        case 32000: amclk_ratio = 0x2; zclk_ratio = 0x8; break;
        case 16000: amclk_ratio = 0x4; zclk_ratio = 0x10; break;
        case  8000: amclk_ratio = 0x8; zclk_ratio = 0x30; break;
        default   : printf("not support sampling rate : %d\n",demanded_sample_rate);break;
    }
	
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_40,(0x0002C800|amclk_ratio));//amclk_ratio

    ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C,(0xF2AAC800|zclk_ratio));//zclk_ratio
    ithDelay(10);
    ithWriteRegA(MMP_AUDIO_CLOCK_REG_3C,(0x02AAC800|zclk_ratio));	
...
static void _i2s_set_GPIO(void) /* GPIO settings for CFG2 */
{
    ...
    {
       unsigned int GpioTable[] ={22,23,24,25,26} ;
    ...
    case DOUBLE_HEADSET:
        {
            ...
            itp_codec_rec_init(0);
            ithWriteRegMaskA(I2S_REG_IN_CTRL, 1<<6|0<<4, 1<<6|1<<4);//1 channel ,left high
            ithGpioSetOut(CHANEL_ENB);
            ithGpioSet(CHANEL_SEL);
            break;
        }
...
void i2s_init_DAC(STRC_I2S_SPEC *i2s_spec)
{
    ...
    itp_codec_playback_init(2);// FPGA v7 wm8960
