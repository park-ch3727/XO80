...
typedef enum
{
    ITH_FIFO_DEFAULT = 0,       ///< FIFO default mode
...
{
    ithWriteRegA(port + ITH_IR_MOD_FILTER_REG, (minFreqDiv & 0x7FFF) << 16 | (maxFreqDiv & 0x7FFF));
}
 ...
 */
static inline void ithIrRxMode(ITHIrPort port, ITHIrRXMode mode)
{
    ithWriteRegMaskA(port + ITH_IR_CAP_CTRL_REG, mode << ITH_IR_RXMODE_BIT, ITH_IR_RXMODE_MASK);
}
 ...
 *
 * @param preScale Pre-Scale factor
