 ...
 */
static inline void ithIrSetGpio(ITHIrPort port, unsigned int pin)
{
 ...
 */
static inline void ithIrCtrlEnable(ITHIrPort port, ITHIrCtrl ctrl)
{
    ithSetRegBitA(port + ITH_IR_CAP_CTRL_REG, ctrl);
}
 ...
 */
static inline void ithIrCtrlDisable(ITHIrPort port, ITHIrCtrl ctrl)
{
    ithClearRegBitA(port + ITH_IR_CAP_CTRL_REG, ctrl);
}
 ...
 */
static inline void ithIrSetCaptureMode(ITHIrPort port, ITHIrCaptureMode mode)
{
    ithWriteRegMaskA(port + ITH_IR_CAP_CTRL_REG, mode << ITH_IR_CAPMODE_BIT, ITH_IR_CAPMODE_MASK);
}
 ...
 * @param ctrl the control to enable.
 */
static inline void ithIrIntrCtrlEnable(ITHIrPort port, ITHIrCtrl ctrl)
{
    ithSetRegBitA(port + ITH_IR_CAP_STATUS_REG, ctrl);
}
 ...
 * @param ctrl the control to disable.
 */
static inline void ithIrIntrCtrlDisable(ITHIrPort port, ITHIrCtrl ctrl)
{
    ithClearRegBitA(port + ITH_IR_CAP_STATUS_REG, ctrl);
}

/**
 * Clears the receiver FIFO and reset control logic.
 */
static inline void ithIrClear(ITHIrPort port)
{
    ithSetRegBitA(port + ITH_IR_CAP_CTRL_REG, ITH_IR_CLEAR_BIT);
}
...
{
    ithWriteRegA(port + ITH_IRTX_SEND_DATA_REG, code);
}
...
{
    return (ithReadRegA(port + ITH_IRTX_CAP_CTRL_REG) >> 16);
}
...
{
    return (ithReadRegA(port + ITH_IRTX_CAP_STATUS_REG) & 0x4) >> 2;
}
...
{
    ithWriteRegA(port + ITH_IRTX_MOD_REG, 0x8000 | freqDiv);
}
...
{
    ithSetRegBitA(port + ITH_IRTX_CAP_STATUS_REG, ctrl);
}
...
{
    ithSetRegBitA(port + ITH_IRTX_CAP_CTRL_REG, ctrl);
}
...
{
    ithClearRegBitA(port + ITH_IRTX_CAP_CTRL_REG, ctrl);
}
...
{
    ithClearRegBitA(port + ITH_IRTX_CAP_STATUS_REG, ctrl);
}
...
{
    ithSetRegBitA(port + ITH_IRTX_CAP_CTRL_REG, ITH_IR_CLEAR_BIT);
}
