...
static int video_stream_udp_start_with_source (VideoStream *stream, Call_info *call_list, uint32_t rem_port,
...
int video_stream_udp_start (VideoStream *stream, Call_info *call_list, int rem_port,
    ...
    if(stream->dir == VideoStreamRecvOnly)
        return video_stream_udp_start_with_source(stream, call_list, rem_port, mobile_call, video_from_ipcam, NULL, NULL);
    else
        return video_stream_udp_start_with_source(stream, call_list, stream->ms.udp_port, mobile_call, video_from_ipcam, cam, ms_web_cam_create_reader(cam));
}
