...
typedef struct VIDEO_SAMPLE_TAG
{
    ...
    bool        bIFrame;
    bool        bLastSlice;
    bool        bdata_used[MAX_VIDEO_USER_NUM];
    int         bdata_reused_cnt;
} VIDEO_SAMPLE;
...
static uint32_t     video_inuse_cnt[VIDEO_STREAM_NUM] = {0};
static bool         es_have_startup = false, encoder_have_init = false, encoder2_have_init = false;
    ...
    do
    {
        ...
        for (j = 0; j < MAX_USER_NUM; j++)
        {
            if (video_use_info[j].bused)
                break;
            else if (j == (MAX_USER_NUM - 1) && !video_use_info[j].bused)
            {
                VideoEncoder_Close();
                ...
                for (k = 0; k < VIDEO_STREAM_NUM; k++)
                    ...
                    {
                        if (gptVideoSample[k][i] != NULL && gptVideoSample[k][i]->m_video != NULL)
    ...
    MSFilter *f)
{
    ...
    do
    {
        ...
        {
            for(i = MAX_VIDEO_USER_NUM; i < MAX_USER_NUM ; i++)
		    {
        ...
        {
            for(i = 0; i < MAX_VIDEO_USER_NUM ; i++)
		    {
			    if(video_use_info[i].bused == false)
				{
				    pEnc_info->use_cnt = i;
        ...
        if (pEnc_info->mobile_call && !encoder2_have_init)
        {
			video_inuse_cnt[1]++;
            ...
            for (i = 0; i < VIDEO_SAMPLE_NUM; i++)
            {
                ...
                gptVideoSample[1][i]->bdata_used[0] = false;
                gptVideoSample[1][i]->bdata_reused_cnt = 0;
            }
            video_use_info[pEnc_info->use_cnt].bmobile  = true;
            gVideoSendIdx[1]                            = 0;
            gVideoinitTSIdx[1]                          = 0;
            VideoEncoder_Open(1);
            encoder2_have_init = true;
        }
        else if(pEnc_info->mobile_call && encoder2_have_init)
        {
			video_inuse_cnt[1]++;
			if(video_inuse_cnt[1] == 1)
			{
				for (i = 0; i < VIDEO_SAMPLE_NUM; i++)
				{
					gptVideoSample[1][i]->bGetStream    = false;
				}
				VideoEncoder_Resume(1);    
				gVideoSendIdx[1] = 0;
			}
        }    
        else
        {
            ...
            {
#if defined(CFG_DUAL_STREAM)	
				if(video_inuse_cnt[0] == 1)
				{
					for (i = 0; i < VIDEO_SAMPLE_NUM; i++)
					{
						gptVideoSample[0][i]->bGetStream    = false;
					}
					VideoEncoder_Resume(0);
					gVideoSendIdx[0] = 0;
				}
#endif				
                return;
            ...
            for (i = 0; i < VIDEO_SAMPLE_NUM; i++)
            {
                ...
                for(int j = 0; j < MAX_VIDEO_USER_NUM; j++)
                {
                    gptVideoSample[0][i]->bdata_used[j] = false;
                }
                gptVideoSample[0][i]->bdata_reused_cnt = 0;
            }
            gVideoSendIdx[0]    = 0;
            gVideoinitTSIdx[0]  = 0;
            VideoEncoder_Open(0);
    ...
    MSFilter *f)
{
	...
	if(k == 0)
	{
		check_idx = pEnc_info->use_cnt;
	}else
		check_idx = pEnc_info->use_cnt - MAX_VIDEO_USER_NUM;
    if (gptVideoSample[k][gVideoSendIdx[k]]->bGetStream && !gptVideoSample[k][gVideoSendIdx[k]]->bdata_used[check_idx])
    {
        //lastTick = itpGetTickCount();
        ...
        {
            m = allocb(gptVideoSample[k][gVideoSendIdx[k]]->dataSize, 0);
            if (m != NULL)
            {
                memcpy(m->b_wptr, gptVideoSample[k][gVideoSendIdx[k]]->m_video->b_wptr, gptVideoSample[k][gVideoSendIdx[k]]->dataSize);
                m->b_wptr += gptVideoSample[k][gVideoSendIdx[k]]->dataSize;
#ifdef CORRECT_RFC3984
                m->b_rptr += 4;
#endif
            }
            ...
            gptVideoSample[k][gVideoSendIdx[k]]->bdata_used[check_idx] = true;
            gptVideoSample[k][gVideoSendIdx[k]]->bdata_reused_cnt++;
            //cnt++;
#if !defined(CFG_LEAF_ENABLE)
            }         //else {
#endif
...
#if defined(ENABLE_VIDEO_MULTICAST)
	if(gptVideoSample[k][gVideoSendIdx[k]]->bdata_reused_cnt)
#else
    if(gptVideoSample[k][gVideoSendIdx[k]]->bdata_reused_cnt >= video_inuse_cnt[k])
#endif
    {
        for(i = 0; i < MAX_VIDEO_USER_NUM; i++)
        {
            gptVideoSample[k][gVideoSendIdx[k]]->bdata_used[i] = false;
        }
        gptVideoSample[k][gVideoSendIdx[k]]->bdata_reused_cnt = 0;
        freeb(gptVideoSample[k][gVideoSendIdx[k]]->m_video);
        if(k == 0)
    ...
    MSFilter *f)
{
	...
	{
		video_inuse_cnt[1]--;
		if(!video_inuse_cnt[1])
			VideoEncoder_Pause(1);
    }
	else
#endif 		
    {   
