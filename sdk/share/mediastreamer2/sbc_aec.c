...
extern pvsola_state pv_config[1];
...
mblk_t *audio_flow_controller_process_(AudioFlowController_ *ctl, mblk_t *m)
{
    if (ctl->total_samples > 0 && ctl->target_samples > 0)
    {
        ...
        if (ctl->current_pos >= ctl->total_samples)
            ctl->target_samples = 0; /*stop discarding*/
    }
    return m;
}
...
void AEC_IN_ARM_INIT(int *AECframesize, int *AECdelayms)
{
    ...
    TimeStretch_Init(&sndstrh_config[0]);
...
static void *hw_engine_thread(void *arg)
{
    ...
    while (s->hw_start)
    {
        ...
        if (err1 && err2)
        {
            ...
            //TimeStretch(oecho->b_wptr, oecho->b_wptr, &sndstrh_config[0]);
