...
void leaf_init_audio_streams (LeafCall *call, unsigned short port) {
    ...
    audiostream->use_ng=iniparser_getint(call->cfgini , strcat(strcpy(str,SOUND),"noisegate"), 1);
#endif
#ifdef ENABLE_AUDIO_SPK_EQUALIZER
    audiostream->eq_SPK=iniparser_getint(call->cfgini , strcat(strcpy(str,SOUND),"eq_SPK"), 1);//filter
#endif
#ifdef ENABLE_AUDIO_MIC_EQUALIZER
    audiostream->eq_MIC=iniparser_getint(call->cfgini , strcat(strcpy(str,SOUND),"eq_MIC"), 1);//filter
#endif
#ifdef ENABLE_AEC_ENABLE
    #ifdef ENABLE_AEC_WITH_FILTER
    audiostream->use_ec=iniparser_getint(call->cfgini, strcat(strcpy(str,SOUND),"echocancellation"), 1);
    audiostream->ecdelay=iniparser_getint(call->cfgini, strcat(strcpy(str,SOUND),"ecdelay"), CFG_AEC_DELAY_MS);
    #else
    audiostream->use_ec=iniparser_getint(call->cfgini, strcat(strcpy(str,SOUND),"echocancellation"), 2);
    #endif
#endif
#ifdef ENABLE_SEND_FILTER
    audiostream->use_volsend=iniparser_getint(call->cfgini , strcat(strcpy(str,SOUND),"use_volsend"), 1);//filter
#endif
#ifdef ENABLE_RECV_FILTER     
    audiostream->use_volrecv=iniparser_getint(call->cfgini , strcat(strcpy(str,SOUND),"use_volrecv"), 1);;//filter
#endif
    audiostream->use_mix=iniparser_getint(call->cfgini , strcat(strcpy(str,SOUND),"use_mix"), 0);//filter;
...
void leaf_start_video_stream(LeafCall *call, Call_info *call_list, int port) {
