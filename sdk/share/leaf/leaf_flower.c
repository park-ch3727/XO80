...
#define VIDEOSTREAM_TCP_ENABLE
#define AUDIOSTREAM_TCP_ENABLE
...
#ifndef VIDEOSTREAM_TCP_ENABLE	
    flow_start_udp_videostream(call->stream_flow, call_list, port, dir);
#else
    flow_start_tcp_videostream(call->stream_flow, call_list, port, dir);
#endif
    return 0;
}
...
#ifndef AUDIOSTREAM_TCP_ENABLE	
	flow_start_udp_audiostream(call->stream_flow, 8000, ULAW, call_list, port);
#else
	flow_start_tcp_audiostream(call->stream_flow, 8000, ULAW, call_list, port);
#endif
    return 0;
}
...
#else
		flow_stop_tcp_videostream(call->stream_flow, dir);
#endif
...
void leaf_start_video_memo_partial_record(LeafCall *call, char *file, int width, int height, int fps)
{
    if (call->stream_flow->videostream !=NULL)
    {
		VideoMemoInfo *info = (VideoMemoInfo*)ite_new0(VideoMemoInfo,1);
		...
		info->video_height = height;
		info->video_fps = fps;
		ite_filter_call_method(call->stream_flow->videostream->Frec_avi, ITE_FILTER_REC_AVI_OPEN_PARTIAL, (void*)info);
		if(info)
			free(info);
		call->VideomemoRecording = true;			
    }
