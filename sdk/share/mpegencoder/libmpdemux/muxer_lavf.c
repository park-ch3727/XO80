...
static muxer_stream_t* lavf_new_stream(muxer_t *muxer, int type)
{
    ...
    spriv->avstream = avformat_new_stream_ex(priv->oc, NULL);
    if (!spriv->avstream)
    {
...
static void fix_parameters(muxer_stream_t *stream)
{
    ...
    if (stream->type == MUXER_TYPE_AUDIO)
    {
        ...
        {
            ctx->extradata = av_malloc_ex(stream->wf->cbSize);
    ...
    else if(stream->type == MUXER_TYPE_VIDEO)
    {
        ...
        if (stream->aspect)
            ...
            spriv->avstream->sample_aspect_ratio = av_d2q_ex(stream->aspect * ctx->height / ctx->width, 255);

        if (stream->bih->biSize > sizeof(*stream->bih))
        {
            ctx->extradata_size = stream->bih->biSize - sizeof(*stream->bih);
            ctx->extradata = av_malloc_ex(ctx->extradata_size);
            if (ctx->extradata != NULL)
...
static int write_chunk(muxer_stream_t *stream, size_t len, unsigned int flags, double dts, double pts)
{
    ...
    if (len)
    {
        ...
        if ((result = av_interleaved_write_frame_ex(priv->oc, &pkt)) != 0)
        {
...
static void write_header(muxer_t *muxer)
{
    ...
    avformat_write_header_ex(priv->oc);
...
static void write_trailer(muxer_t *muxer)
{
    ...
    av_write_trailer_ex(priv->oc);

    avformat_close_input_ex(&priv->oc);
}

static uint64_t write_size(muxer_t *muxer)
{
    muxer_priv_t *priv = muxer->priv;
    return av_write_size_ex(priv->oc);
}

static int init_format(muxer_t *muxer)
{
    muxer_priv_t *priv = muxer->priv;
    return avformat_init_format_ex(priv->oc);
}
...
int muxer_init_muxer_lavf(muxer_t *muxer)
{
    ...
    priv->oc = avformat_alloc_context_ex();
    if (!priv->oc)
    {
        trac("");
    ...
    if (conf_format)
        fmt = av_guess_format_ex(conf_format, NULL, NULL);
    priv->oc->oformat = fmt;
    if (muxer->cache_data_in_file)
        priv->oc->flags |= AVFMT_FLAG_CACHE_DATA_IN_FILE;

    priv->oc->pb = avio_alloc_context_ex(priv->buffer, BIO_BUFFER_SIZE, muxer, mp_write, mp_seek, mp_write_size);
