...
static int init_muxer(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (of->codec_tag) {
            codec->codec_tag = av_codec_get_tag_ex(of->codec_tag, codec->codec_id);
        }
    ...
    if (!s->priv_data && of->priv_data_size > 0) {
        s->priv_data = av_mallocz_ex(of->priv_data_size);
        if (!s->priv_data) {
            trac("");
            ret = AVERROR(ENOMEM);
            goto fail;
...
int av_write_trailer_ex(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        av_freep_ex(&s->streams[i]->priv_data);
    }
    av_freep_ex(&s->priv_data);
    return ret;
}
