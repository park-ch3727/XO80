    ...
    unsigned char** ppentry_data)
{
    if (ppc && (*ppc))
    {
        ...
        cache_dump_to_avio_write(*ppc, avio_write_ex, pb);
        free_cache(*ppc);
        *ppc = MMP_NULL;
    }
    else
    {
        avio_write_ex(pb, *ppentry_data, size);

        av_free_ex(*ppentry_data);
...
static int64_t update_size(AVIOContext *pb, int64_t pos)
{
    ...
    avio_seek_ex(pb, pos, SEEK_SET);
    avio_wb32_ex(pb, curpos - pos); /* rewrite size */
    avio_seek_ex(pb, curpos, SEEK_SET);
...
static int mov_write_stco_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32_ex(pb, 0); /* size */
    if (mode64)
        ffio_wfourcc(pb, "co64");
    else
        ffio_wfourcc(pb, "stco");

    avio_wb32_ex(pb, 0); /* version & flags */
    avio_wb32_ex(pb, track->entry_count[MOV_STCO]); /* entry count */
#else
    ...
    if (!mode64)
    {
        ...
        avio_write_ex(pb, data, sizeof(data));
    }
    else
    {
        uint8_t data[] = {
...
static int mov_write_stsz_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    {
        avio_wb32_ex(pb, track->stsz_sample_size); // sample size
        avio_wb32_ex(pb, track->sample_count);            // sample count
    }
    else
    {
        avio_wb32_ex(pb, 0);                       // sample size
        avio_wb32_ex(pb, track->sample_count);            // sample count
...
static int mov_write_stsc_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32_ex(pb, 0); // version & flags
    avio_wb32_ex(pb, track->entry_count[MOV_STSC]); // entry count
#else
...
static int mov_write_stss_tag(AVIOContext *pb, MOVTrack *track, uint32_t flag)
{
    ...
    avio_wb32_ex(pb, 0); // version & flags
    avio_wb32_ex(pb, track->entry_count[MOV_STSS]); // entry count
#else
...
static void put_descr(AVIOContext *pb, int tag, unsigned int size)
{
    ...
    avio_w8_ex(pb, tag);
    for(; i>0; i--)
        avio_w8_ex(pb, (size>>(7*i)) | 0x80);
    avio_w8_ex(pb, size & 0x7F);
}
...
static int mov_write_esds_tag(AVIOContext *pb, MOVTrack *track) // Basic
{
    ...
    avio_wb16_ex(pb, track->track_id);
    avio_w8_ex(pb, 0x00); // flags (= no flags)
    ...
    if ((track->enc->codec_id == AV_CODEC_ID_MP2 ||
        ...
        avio_w8_ex(pb, 0x6B); // 11172-3
    else
        avio_w8_ex(pb, ff_codec_get_tag_ex(ff_mp4_obj_type_ex, track->enc->codec_id));

    // the following fields is made of 6 bits to identify the streamtype (4 for video, 5 for audio)
    // plus 1 bit to indicate upstream and 1 bit set to 1 (reserved)
    if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO)
        avio_w8_ex(pb, 0x15); // flags (= Audiostream)
    else
        avio_w8_ex(pb, 0x11); // flags (= Visualstream)

    avio_w8_ex(pb,  track->enc->rc_buffer_size>>(3+16));      // Buffersize DB (24 bits)
    avio_wb16_ex(pb, (track->enc->rc_buffer_size>>3)&0xFFFF); // Buffersize DB

    avg_bitrate = compute_avg_bitrate(track);
    // maxbitrate (FIXME should be max rate in any 1 sec window)
    avio_wb32_ex(pb, FFMAX3(track->enc->bit_rate, track->enc->rc_max_rate, avg_bitrate));
    avio_wb32_ex(pb, avg_bitrate);

    if (track->vos_len) {
        // DecoderSpecific info descriptor
        put_descr(pb, 0x05, track->vos_len);
        avio_write_ex(pb, track->vos_data, track->vos_len);
    }
...
static int mov_write_wave_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32_ex(pb, 12);    /* size */
    ffio_wfourcc(pb, "frma");
    avio_wl32_ex(pb, track->tag);
#else
...
#ifdef TEST
    avio_wb32_ex(pb, 8);     /* size */
    avio_wb32_ex(pb, 0);     /* null tag */
#else
    {
...
static int mov_write_audio_tag(AVIOContext *pb, MOVTrack *track)
{
...
#ifdef TEST
    avio_wb32_ex(pb, 0); /* size */
    avio_wl32_ex(pb, tag); // store it byteswapped
    avio_wb32_ex(pb, 0); /* Reserved */
    avio_wb16_ex(pb, 0); /* Reserved */
    avio_wb16_ex(pb, 1); /* Data-reference index, XXX  == 1 */

    /* SoundDescription */
    avio_wb16_ex(pb, version); /* Version */
    avio_wb16_ex(pb, 0); /* Revision level */
    avio_wb32_ex(pb, 0); /* Reserved */
#else
    {
        uint8_t data[] = {
    ...
    if (track->mode == MODE_MOV) {
        avio_wb16_ex(pb, track->enc->channels);
        avio_wb16_ex(pb, 16);
        avio_wb16_ex(pb, -2); /* compression ID */
    ...
    avio_wb16_ex(pb, 0); /* packet size (= 0) */
    avio_wb16_ex(pb, track->enc->sample_rate <= UINT16_MAX ?
                  track->enc->sample_rate : 0);
    avio_wb16_ex(pb, 0); /* Reserved */

    if(version == 1) { /* SoundDescription V1 extended info */
        avio_wb32_ex(pb, track->enc->frame_size); /* Samples per packet */
        avio_wb32_ex(pb, track->sample_size / track->enc->channels); /* Bytes per packet */
        avio_wb32_ex(pb, track->sample_size); /* Bytes per frame */
        avio_wb32_ex(pb, 2); /* Bytes per sample */
    }
...
int ff_isom_write_avcc_ex(AVIOContext *pb, const uint8_t *data, int len);
...
static int mov_write_avcc_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    ff_isom_write_avcc_ex(pb, track->vos_data, track->vos_len);
    return update_size(pb, pos);
#else
    int64_t pos = avio_tell(pb);
    uint8_t data[] = {
...
static int mp4_get_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    if (!ff_codec_get_tag_ex(ff_mp4_obj_type_ex, track->enc->codec_id))
        return 0;
...
{
    AVRational sar;
    av_reduce_ex(&sar.num, &sar.den, track->enc->sample_aspect_ratio.num,
              track->enc->sample_aspect_ratio.den, INT_MAX);

    avio_wb32_ex(pb, 16);
    ffio_wfourcc(pb, "pasp");
    avio_wb32_ex(pb, sar.num);
    avio_wb32_ex(pb, sar.den);
    return 16;
}
...
static int mov_write_video_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    avio_wb32_ex(pb, 0); /* size */
    avio_wl32_ex(pb, track->tag); // store it byteswapped
#ifdef TEST
    avio_wb32_ex(pb, 0); /* Reserved */
    avio_wb16_ex(pb, 0); /* Reserved */
    avio_wb16_ex(pb, 1); /* Data-reference index */

    avio_wb16_ex(pb, 0); /* Codec stream version */
    avio_wb16_ex(pb, 0); /* Codec stream revision (=0) */
    ...
    avio_wb16_ex(pb, track->enc->width); /* Video width */
    avio_wb16_ex(pb, track->height); /* Video height */
#ifdef TEST
    avio_wb32_ex(pb, 0x00480000); /* Horizontal resolution 72dpi */
    avio_wb32_ex(pb, 0x00480000); /* Vertical resolution 72dpi */
    avio_wb32_ex(pb, 0); /* Data size (= 0) */
    avio_wb16_ex(pb, 1); /* Frame count (= 1) */

    /* FIXME not sure, ISO 14496-1 draft where it shall be set to 0 */
    avio_w8_ex(pb, strlen(compressor_name));
    avio_write_ex(pb, compressor_name, 31);

    avio_wb16_ex(pb, 0x18); /* Reserved */
    avio_wb16_ex(pb, 0xffff); /* Reserved */
#else
    {
...
#ifdef TEST
    avio_wb32_ex(pb, atom_size); /* size */
    ffio_wfourcc(pb, "stts");
    avio_wb32_ex(pb, 0); /* version & flags */
    avio_wb32_ex(pb, entries); /* entry count */
#else
    {
        uint8_t data[] = {
            (atom_size >> 24),          /* size */
...
{
#ifdef TEST
    avio_wb32_ex(pb, 28); /* size */
    ffio_wfourcc(pb, "dref");
    avio_wb32_ex(pb, 0); /* version & flags */
    avio_wb32_ex(pb, 1); /* entry count */

    avio_wb32_ex(pb, 0xc); /* size */
    //FIXME add the alis and rsrc atom
    ffio_wfourcc(pb, "url ");
    avio_wb32_ex(pb, 1); /* version & flags */
#else
...
static int mov_write_stbl_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    {
        av_free_ex(track->entry[MOV_STSS]);
...
{
#ifdef TEST
    avio_wb32_ex(pb, 16); /* size */
    ffio_wfourcc(pb, "smhd");
    avio_wb32_ex(pb, 0); /* version & flags */
    avio_wb16_ex(pb, 0); /* reserved (balance, normally = 0) */
    avio_wb16_ex(pb, 0); /* reserved */
#else
...
{
#ifdef TEST
    avio_wb32_ex(pb, 0x14); /* size (always 0x14) */
    ffio_wfourcc(pb, "vmhd");
    avio_wb32_ex(pb, 0x01); /* version & flags */
    avio_wb64_ex(pb, 0); /* reserved (graphics mode = copy) */
#else
...
static int mov_write_hdlr_tag(AVIOContext *pb, MOVTrack *track)
{
...
#endif
    avio_write_ex(pb, hdlr, 4); /* handler */
    ...
    if (!track || track->mode == MODE_MOV)
        avio_w8_ex(pb, strlen(descr)); /* pascal string */
    avio_write_ex(pb, descr, strlen(descr)); /* handler description */
    if (track && track->mode != MODE_MOV)
        avio_w8_ex(pb, 0); /* c string */
    return update_size(pb, pos);
}
...
static int mov_write_mdhd_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    (version == 1) ? avio_wb32_ex(pb, 44) : avio_wb32_ex(pb, 32); /* size */
#ifdef TEST
    ffio_wfourcc(pb, "mdhd");
    avio_w8_ex(pb, version);
    avio_wb24_ex(pb, 0); /* flags */
#else
    {
...
#endif
    if (version == 1) {
        avio_wb64_ex(pb, track->time);
        avio_wb64_ex(pb, track->time);
    } else {
        avio_wb32_ex(pb, track->time); /* creation time */
        avio_wb32_ex(pb, track->time); /* modification time */
    }
    avio_wb32_ex(pb, track->timescale); /* time scale (sample rate for audio) */
    if (!track->sample_count)
        (version == 1) ? avio_wb64_ex(pb, UINT64_C(0xffffffffffffffff)) : avio_wb32_ex(pb, 0xffffffff);
    else
        (version == 1) ? avio_wb64_ex(pb, track->track_duration) : avio_wb32_ex(pb, track->track_duration); /* duration */
    avio_wb16_ex(pb, track->language); /* language */
    avio_wb16_ex(pb, 0); /* reserved (quality) */
...
{
#ifdef TEST
    avio_wb32_ex(pb, 1 << 16); /* 16.16 format */
    avio_wb32_ex(pb, 0 << 16); /* 16.16 format */
    avio_wb32_ex(pb, 0);        /* u in 2.30 format */
    avio_wb32_ex(pb, 0 << 16); /* 16.16 format */
    avio_wb32_ex(pb, 1 << 16); /* 16.16 format */
    avio_wb32_ex(pb, 0);        /* v in 2.30 format */
    avio_wb32_ex(pb, 0 << 16); /* 16.16 format */
    avio_wb32_ex(pb, 0 << 16); /* 16.16 format */
    avio_wb32_ex(pb, 1 << 30);  /* w in 2.30 format */
#else
...
{
    int64_t duration = av_rescale_rnd_ex(track->track_duration, MOV_TIMESCALE,
                                      track->timescale, AV_ROUND_UP);
    int version = duration < INT32_MAX ? 0 : 1;

    (version == 1) ? avio_wb32_ex(pb, 104) : avio_wb32_ex(pb, 92); /* size */
#ifdef TEST
    ffio_wfourcc(pb, "tkhd");
    avio_w8_ex(pb, version);
    avio_wb24_ex(pb, 0xf); /* flags (track enabled) */
#else
    {
    ...
    avio_wb32_ex(pb, track->track_id); /* track-id */
    avio_wb32_ex(pb, 0); /* reserved */
    if (!track->sample_count)
        (version == 1) ? avio_wb64_ex(pb, UINT64_C(0xffffffffffffffff)) : avio_wb32_ex(pb, 0xffffffff);
    else
        (version == 1) ? avio_wb64_ex(pb, duration) : avio_wb32_ex(pb, duration);

#ifdef TEST
    avio_wb32_ex(pb, 0); /* reserved */
    avio_wb32_ex(pb, 0); /* reserved */
    avio_wb16_ex(pb, 0); /* layer */
#else
    {
        uint8_t data[] = {0, 0, 0, 0,   /* reserved */
                          0, 0, 0, 0,   /* reserved */
...
#endif
    avio_wb16_ex(pb, st ? st->codec->codec_type : 0); /* alternate group) */
    /* Volume, only for audio */
    if(track->enc->codec_type == AVMEDIA_TYPE_AUDIO)
        avio_wb16_ex(pb, 0x0100);
    else
        avio_wb16_ex(pb, 0);
    avio_wb16_ex(pb, 0); /* reserved */
    ...
    if(st && (track->enc->codec_type == AVMEDIA_TYPE_VIDEO))
    {
        ...
        avio_wb32_ex(pb, sample_aspect_ratio * track->enc->width*0x10000);
        avio_wb32_ex(pb, track->height*0x10000);
    }
    else {
#ifdef TEST
        avio_wb32_ex(pb, 0);
...
static int mov_write_iods_tag(AVIOContext *pb, MOVMuxContext *mov)
{
    ...
    put_descr(pb, 0x10, 7);
#ifdef TEST
    avio_wb16_ex(pb, 0x004f);
    avio_w8_ex(pb, 0xff);
    avio_w8_ex(pb, 0xff);
    avio_w8_ex(pb, audio_profile);
    avio_w8_ex(pb, video_profile);
    avio_w8_ex(pb, 0xff);
#else
    {
...
static int mov_write_mvhd_tag(AVIOContext *pb, MOVMuxContext *mov)
{
    ...
    for (i=0; i<mov->nb_streams; i++) {
        if(mov->tracks[i].sample_count > 0) {
            max_track_len_temp = av_rescale_rnd_ex(mov->tracks[i].track_duration,
                                                MOV_TIMESCALE,
    ...
    (version == 1) ? avio_wb32_ex(pb, 120) : avio_wb32_ex(pb, 108); /* size */
#ifdef TEST
...
#endif
    if (version == 1) {
        avio_wb64_ex(pb, mov->time);
        avio_wb64_ex(pb, mov->time);
    } else {
        avio_wb32_ex(pb, mov->time); /* creation time */
        avio_wb32_ex(pb, mov->time); /* modification time */
    }
    avio_wb32_ex(pb, MOV_TIMESCALE);
    (version == 1) ? avio_wb64_ex(pb, max_track_len) : avio_wb32_ex(pb, max_track_len); /* duration of longest track */

#ifdef TEST
    avio_wb32_ex(pb, 0x00010000); /* reserved (preferred rate) 1.0 = normal */
    avio_wb16_ex(pb, 0x0100); /* reserved (preferred volume) 1.0 = normal */
    avio_wb16_ex(pb, 0); /* reserved */
    avio_wb32_ex(pb, 0); /* reserved */
    avio_wb32_ex(pb, 0); /* reserved */
#else
...
#ifdef TEST
    avio_wb32_ex(pb, 0); /* reserved (preview time) */
    avio_wb32_ex(pb, 0); /* reserved (preview duration) */
    avio_wb32_ex(pb, 0); /* reserved (poster time) */
    avio_wb32_ex(pb, 0); /* reserved (selection time) */
    avio_wb32_ex(pb, 0); /* reserved (selection duration) */
    avio_wb32_ex(pb, 0); /* reserved (current time) */
#else
    {
        uint8_t data[24] = {0};
        avio_write_ex(pb, data, sizeof(data));
...
#endif
    avio_wb32_ex(pb, max_track_id + 1); /* Next track id */
    return 0x6c;
}
                              ...
                              AVFormatContext *s)
{
    ...
    int64_t pos = avio_tell(pb);
#ifdef TEST
    avio_wb32_ex(pb, 0); /* size placeholder*/
...
{
    avio_wb32_ex(pb, 8);    // placeholder for extended size field (64 bit)
...
#if 1
        void* new_space = av_realloc_ex(trk->entry[table_id], new_size);
#else
        // for test
        void* new_space;

        if (trk->entry_allocated[table_id] == 0 || table_id == 0)
            new_space = av_realloc_ex(trk->entry[table_id], new_size);
        else
            new_space = 0;
#endif
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (s->flags & AVFMT_FLAG_CACHE_DATA_IN_FILE)
    {
        ...
        avio_write_ex(pb, pkt->data, size);
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    mov->tracks = av_mallocz_ex(mov->nb_streams*sizeof(*mov->tracks));
    if (!mov->tracks)
    {
        trac("");
...
//        AVDictionaryEntry *lang = av_dict_get(st->metadata, "language", NULL,0);
        ...
        track->enc = st->codec;
//        track->language = ff_mov_iso639_to_lang_ex(lang?lang->value:"und", mov->mode!=MODE_MOV);
//        if (track->language < 0)
//            track->language = 0;
        track->mode = mov->mode;
        ...
        avpriv_set_pts_info_ex(st, 64, 1, track->timescale);

        /* copy extradata if it exists */
        if (st->codec->extradata_size) {
            track->vos_len  = st->codec->extradata_size;
            track->vos_data = av_malloc_ex(track->vos_len);
...
static int mov_write_trailer(AVFormatContext *s)
{
    ...
    if (mov->mdat_size + 8 <= UINT32_MAX) {
        avio_seek_ex(pb, mov->mdat_pos, SEEK_SET);
        avio_wb32_ex(pb, mov->mdat_size + 8);
    } else {
        /* overwrite 'wide' placeholder atom */
        avio_seek_ex(pb, mov->mdat_pos - 8, SEEK_SET);
...
#endif
        avio_wb64_ex(pb, mov->mdat_size + 16);
    }
    avio_seek_ex(pb, moov_pos, SEEK_SET);
    mov_write_moov_tag(pb, mov, s);

    for (i=0; i<mov->nb_streams; i++) {
        MOVTrack *track = &mov->tracks[i];
        if (track->vos_len)
            av_free_ex(track->vos_data);
...
static uint64_t mov_write_size(AVFormatContext *s)
{
    ...
    write_size = avio_write_size_ex(pb);
    max_entry_size = 0;
    header_size = 0;
...
static int mov_init_format(AVFormatContext *s)
{
    ...
    if (s->flags & AVFMT_FLAG_CACHE_DATA_IN_FILE)
    {
        ...
        s->caches = (cache_t**)av_mallocz_ex(sizeof(*s->caches) * CACHE_FILE_COUNT);
        if (!s->caches)
            goto fail;
...
AVOutputFormat ff_mp4_muxer = {
    ...
    .codec_tag         = (const AVCodecTag* const []){ ff_mp4_obj_type_ex, 0 },
