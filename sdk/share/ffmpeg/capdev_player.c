    ...
    void *arg)
{
    ...
    if (capture0state >> 31)
    {
        if ((capture0state >> 8) & 0xF)
        {
            ...
            ithPrintf("1. ErrorCode = [%d],[%d],[0x%x],[0x%x]\n",(capture0state >> 8) & 0xF,(capture0state >> 4) & 0xF,capture0state,(capture0state >> 31));
            interrupt_status = 1;
        ...
        {
 //          ithPrintf("cap0_isr frame end = [%d],[%d],[0x%x],[0x%x]\n",(capture0state >> 8) & 0xF,(capture0state >> 4) & 0xF,capture0state,(capture0state >> 31));
            interrupt_status = 0;
            xQueueSendToBackFromISR(gCapInfoQueue, (void *)&interrupt_status, &gHigherPriorityTaskWoken);
    ...
    {
    	ithPrintf("cap0_isr frame end2 = [%d],[%d],[0x%x],[0x%x]\n",(capture0state >> 8) & 0xF,(capture0state >> 4) & 0xF,capture0state,(capture0state >> 31));
    }
...
static int _SIGNALCHECK_FIRE(CAPTURE_HANDLE *ptDev)
{
    ...
    while ((capture0state & 0xe) != 0xe)
    {
    	ithPrintf("_SIGNALCHECK_FIRE1 = [%d],[%d],[0x%x]\n",(capture0state >> 8) & 0xF,(capture0state >> 4) & 0xF,capture0state);
        if (++timeout > CAPDEVTIMEOUT)
            return 1;
        printf("Hsync or Vsync not stable! MPG[%d]\n",timeout);
        usleep(10 * 1000);
        capture0state = ithCapGetEngineErrorStatus(&gCapDev0, MMP_CAP_LANE0_STATUS);
    }
	ithPrintf("_SIGNALCHECK_FIRE2 = [%d],[%d],[0x%x]\n",(capture0state >> 8) & 0xF,(capture0state >> 4) & 0xF,capture0state);
    ithCapFire(ptDev, MMP_TRUE);
    printf("Capture Fire! (%d) MPG\n", ptDev->cap_id);
...
static int ithCapdevPlayer_select_file(const char *filename, int level)
{
    ...
    if(_SENSOR_SIGNALCHECK())
		pprop->is_memory_mode = true;
	else
		pprop->is_memory_mode = (bool)ithCapDeviceGetProperty(DEVICES_ISINTERLANCED);
    ...
    else
    {
		...
		printf("w = %d h = %d\n", isp_share.width, isp_share.height);
