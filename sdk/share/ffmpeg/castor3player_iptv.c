...
#define FIRST_VIDEO_WAIT_COUNT  4
#define RESYNC_VIDEO_UPPER_THRESHOLD  2
#define RESYNC_VIDEO_LOWER_THRESHOLD  2

#define RESYNC_VIDEO_UPPER_THRESHOLD_TS  6
#define RESYNC_VIDEO_LOWER_THRESHOLD_TS  8
...
extern bool gisLowDelay;
uint64_t    gCurVideoPts = 0;
static int gAudioSync = 0;

extern unsigned int I2S_DA32_GET_RP(void);
extern unsigned int I2S_DA32_GET_WP(void);
extern void i2s_frection_set(int data);
extern unsigned int PalGetClock(void);
...
static int video_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (ret > 0)
        {
            ...
            int threshold;
            threshold = (gisLowDelay) ? RESYNC_VIDEO_LOWER_THRESHOLD : RESYNC_VIDEO_LOWER_THRESHOLD_TS;

			if(is->videoq.nb_packets > threshold)
			{
				printf("Skip frame\n");
				continue;
			}
...
static int audio_thread(void *arg)
{
	...
	int dec_buf_size , dec_data_size = 0;
    unsigned long avail_buf_size = 0;
...
retry:
    ...
    pthread_create(&pprop->video_tid, &attr, (void *)video_thread, (void*)pprop);
	...
	param.sched_priority = sched_get_priority_max(SCHED_RR);
	pthread_attr_setschedparam(&attr, &param);
    pthread_create(&pprop->audio_tid, &attr, (void *)audio_thread, (void*)pprop);
#endif	
    ...
    for (;;)
    {
        ...
        if (pkt->stream_index == is->video_stream)
        {
            ...
            int threshold;
            threshold = (gisLowDelay) ? RESYNC_VIDEO_UPPER_THRESHOLD : RESYNC_VIDEO_UPPER_THRESHOLD_TS;
...
static int get_video_frame_ext(PlayerInstance *is, AVFrame *frame, int64_t *pts, AVPacket *pkt)
{
    int got_picture, i, result = 0;
	...
	//is->video_pts = (double)pkt->pts * av_q2d(is->video_st->time_base);
    avcodec_decode_video2(is->videoctx, frame, &got_picture, pkt);
...
static int video_thread_ext(void *arg)
{
    PlayerProps *pProp = (PlayerProps*) arg;
    AVFrame *frame = avcodec_alloc_frame();
    uint64_t pts_int = AV_NOPTS_VALUE, pos = -1;
    double pts;
    int ret;
    int pktSize = 0;
    int64_t curPts = 0;
    ...
    for (;;)
    {
        ...
        checkTime = itpGetTickCount();
        queueCount = is->videoq.nb_packets;
        ret = get_video_frame_ext(is, frame, NULL, &pkt);
        if (ret < 0)
        {
            goto the_end;
        }
        ...
        {
        	//printf(".");
            int prevGap = itpGetTickDuration(prevTime);
            if (is->is_video_sync)
            {
                gCurVideoPts = pts_int = (uint64_t)(pkt.timestamp * 1000);
            }

            if (prevGap > 60)
            {
                printf("video pts = %lld, cnt: %d, gap: %d, decode: %d, queue: %d\n", pts_int, is->videoq.nb_packets, itpGetTickDuration(prevTime), itpGetTickDuration(checkTime), queueCount);
            }
            
            if (prevGap > 1000)
            {
                printf("AV need resync........................................\n");
                is->is_video_sync = 0;
                gCurVideoPts = 0;
                itv_cleanup_screen();
            }
            
            prevTime = itpGetTickCount();
            av_free_packet(&pkt);
            if (!is->is_video_sync)
            {				
#if 1
                ...
                startThreshold = FIRST_VIDEO_WAIT_COUNT;
				printf("start nb count: %d, cur_q_cnt: %d\n", startThreshold, is->videoq.nb_packets);
#else
                usleep(VIDEO_WAIT_TIME * 1000);
#endif
                set_resync_state(is, NEED_RESYNC_NULL_STATE);
            ...
            if(is->videoq.nb_packets > threshold && skipcnt < 7)
            {
                bDropFrame = true;
                printf("(%d)Skip frame %d, queue count: %d, gap: %d ms\n", __LINE__, skipcnt, is->videoq.nb_packets, itpGetTickDuration(checkTime));
                checkTime = itpGetTickCount();
                skipcnt++;
                continue;
            }
			skipcnt = 0;
            render_frame(is->videoctx, frame, pts_int/*(uint64_t)(pts*90000)*/);
			//usleep(1000);
        }
		else
		{
...
static int audio_thread_ext(void *arg)
{
    PlayerProps *pProp = (PlayerProps*) arg;
    AVFrame *frame = avcodec_alloc_frame();
    int64_t pts_int = AV_NOPTS_VALUE, pos = -1;
...
get_next_audio_pkt:
        ...
        {
        	//double diff, audio_pts;
            int64_t timegap = (int64_t)(pkt.timestamp * 1000) - (int64_t)gCurVideoPts;
retry_audio:
        	if (!is->is_video_sync || !gAudioSync)
        	{
                if (timegap >= -150 && gCurVideoPts && is->audioq.nb_packets < 2)
                {
                    if (!gAudioSync)
                    {
                        printf("Audio sync time stamp: %lld, video time stamp: %lld, a_q: %d, v_q: %d\n",
                               (int64_t)(pkt.timestamp * 1000), gCurVideoPts, is->audioq.nb_packets, is->videoq.nb_packets);
                    }
                    gAudioSync = 1;
                    usleep(10*1000);
                    goto retry_audio;
                ...
                else
                {
                    if (gCurVideoPts)
                    {
                        printf("time gap: %lld, gCurVideo: %lld, _q: %d, v_q: %d\n", timegap, gCurVideoPts, is->audioq.nb_packets, is->videoq.nb_packets);
                    }
                    av_free_packet(&pkt);
                }
        	}
			else
			{
				...
				{	                
					...
					dec_data_size = dec_buf_size - avail_buf_size - 2;
					//printf("dec_buf_size = %d, avail_buf_size = %d\n", dec_buf_size, avail_buf_size);
                    if (pkt.size && (dec_data_size / pkt.size) > 4)
                    {
                        //Abnormal audio decode status. we try to purge audio data and resync again.
                        uint32_t timeoutTick = itpGetTickCount();
                        av_free_packet(&pkt);
                        while(1)
                        {
                            iteAudioGetAttrib(ITE_AUDIO_DIRVER_DECODE_BUFFER_LENGTH, &dec_buf_size);
                            ...
                            {
                                gAudioSync = 0;
                                printf("purge audio buffer and resync auido....\n");
                                goto get_next_audio_pkt;
                            }
                            else
                            {
                                usleep(20*1000);
                            }
                            if (itpGetTickDuration(timeoutTick) >= 1000)
                            {
                                printf("abnormal audio wait empty time out..\n");
                                gAudioSync = 0;
                                goto get_next_audio_pkt;
                            }
                        }
                    ...
                    if (is->audioq.nb_packets > 3)
                    {
                        printf("av_gap: %lld, audio: %lld, video: %lld, buf: %f s, a_q: %d, v_q: %d, dac: %d, dec: %d\n",
                               timegap, (int64_t)(pkt.timestamp * 1000), (int64_t)gCurVideoPts, audio_buf_time,
                               is->audioq.nb_packets, is->videoq.nb_packets,
                               dac_data_size, dec_data_size);
                        av_free_packet(&pkt);
                        goto get_next_audio_pkt;
                    }
					//diff = (audio_pts - audio_buf_time) - is->video_pts;
					//printf("diff = %f, audio_pts = %f, audio_buf_time = %f, video_pts = %f, init_gap = %f\n", diff, audio_pts, audio_buf_time, is->video_pts, is->init_av_gap);
					//printf("YC0: audio_buf_time = %f\n", audio_buf_time);
                    #if 0
                    if (itpGetTickDuration(curTime) > 2000)
                    {
                        printf("av_gap: %lld, audio: %lld, video: %lld, buf: %f s, a_q: %d, v_q: %d, dac: %d, dec: %d\n",
                               timegap, (int64_t)(pkt.timestamp * 1000), (int64_t)gCurVideoPts, audio_buf_time,
                               is->audioq.nb_packets, is->videoq.nb_packets, dac_data_size, dec_data_size);
                        curTime = itpGetTickCount();
                    }
                    #endif
					
					if(audio_buf_time > 0.15)
					{
						if(i2s_frection_value != 50)
	                    {
	                    	i2s_frection_value = 50;
                            printf("(%d), value: %d, buf: %f, dac: %d, dec: %d\n", __LINE__, i2s_frection_value, audio_buf_time, dac_data_size, dec_data_size);
	                    	i2s_frection_set(i2s_frection_value);
	                    }
					}
					else if(audio_buf_time < 0.05)
...
#endif				
				avcodec_decode_audio4(is->audioctx, frame, &got_frame, &pkt);
				//printf("audio pts = %lld\n", pkt.pts);
				av_free_packet(&pkt);
			}
        }
...
void ithMediaPlayer_InitAVDecodeEnv()
{
	...
	pthread_attr_setschedparam(&attr, &param);
    printf("init video thread\n");
    pthread_create(&pprop->video_tid, &attr, (void *)video_thread_ext, (void*)pprop);
    //pthread_create(&pprop->video_tid, NULL, video_thread, (void*)pprop);
    /* Create Audio thread */	
    printf("init audio thread\n");
	pthread_attr_init(&attr);
	...
	param.sched_priority = sched_get_priority_max(SCHED_RR);
	pthread_attr_setschedparam(&attr, &param);
	pthread_create(&pprop->audio_tid, &attr, (void *)audio_thread_ext, (void*)pprop);	
#endif
}

void ithMediaPlayer_InitH264DecodeEnv(int width, int height)
{
    PlayerProps    *pprop         = global_player_prop;
    PlayerInstance *is            = NULL;
    AVCodec        *videoCodec;
...
void ithMediaPlayer_InitAudioDecodeEnv(int samplerate, int num_channels)
{
    PlayerProps    *pprop         = global_player_prop;
    PlayerInstance *is            = NULL;
    AVCodec        *audioCodec;
...
int ithMediaPlayer_h264_decode(unsigned char *inputbuf, int inputbuf_size, double timestamp)
{
    PlayerProps    *pprop = global_player_prop;
    PlayerInstance *is;
    AVPacket       packet;
...
int ithMediaPlayer_audio_decode(unsigned char *inputbuf, int inputbuf_size, double timestamp)
{
    PlayerProps    *pprop = global_player_prop;
    PlayerInstance *is;
    AVPacket       packet;
...
ithMediaPlayer player1 = {
    ...
    ithMediaPlayer_h264_decode,
    ithMediaPlayer_audio_decode,
...
ithMediaPlayer player1 = {
    ...
    .h264_decode				 = ithMediaPlayer_h264_decode,
    .audio_decode				 = ithMediaPlayer_audio_decode,
