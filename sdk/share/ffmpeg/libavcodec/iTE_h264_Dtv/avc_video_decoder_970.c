...
{    
    ...
    //	return;
#if !defined(CFG_IPTV_RX) && !defined(CFG_IPTV_TX)    
...
#if defined(CFG_IPTV_RX) || defined(CFG_IPTV_TX)
    ithFlushDCacheRange((void*)buf+h->nal_length_size, buf_size);
#else
    ithFlushDCacheRange((void*)gp_video_buf_sys_addr, buf_size);
#endif
...
#if defined(CFG_IPTV_RX) || defined(CFG_IPTV_TX)
    decInput.pStream = buf+h->nal_length_size;
    decInput.streamBusAddress = buf+h->nal_length_size;
#else	
    ...
    do
    {
        ...
        case H264DEC_HDRS_RDY:
            ...
            if ((predecInfo.picWidth != decInfo.picWidth || predecInfo.picHeight != decInfo.picHeight || 
                predecInfo.interlacedSequence != decInfo.interlacedSequence) && predecInfo.picWidth != 0)
                gbChangeFormat = true;
            	
            predecInfo.picWidth = decInfo.picWidth;
            predecInfo.picHeight = decInfo.picHeight;
            predecInfo.interlacedSequence = decInfo.interlacedSequence;
            break;
        case H264DEC_PIC_DECODED:
            if (H264DecNextPicture(decInst, &decPicture, 0) == H264DEC_PIC_RDY)
            {
                ...
                pict->key_frame = !decPicture.picCodingType[0];
...
int iTE_h264_Dtv_decode_init(AVCodecContext *avctx)
{
    ...
    memset(&predecInfo, 0, sizeof(H264DecInfo));
    gbChangeFormat = false;

#if defined(CFG_DOORBELL_INDOOR) || defined(CFG_DOORBELL_ADMIN)
	if (H264DecInit(&decInst, 1, 0, 1, DEC_REF_FRM_RASTER_SCAN)!= H264DEC_OK)
#else
    ...
    {
#if defined(CFG_IPTV_RX) || defined(CFG_IPTV_TX)    	
        gp_video_buf_vram_addr = itpVmemAlignedAlloc(8, 2*1024*1024);
        gp_video_buf_sys_addr = (uint8_t*) ithMapVram(gp_video_buf_vram_addr, 2*1024*1024, /*ITH_VRAM_READ |*/ ITH_VRAM_WRITE); 
#else
