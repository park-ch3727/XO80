...
static void* ReadTask(void *arg)
{
    ...
    while (s->state)
    {
        ...
        {
            //printf("len:%d payload[0] = 0x%x, payload[188] = 0x%x, payload[376] = 0x%x, payload[564] = 0x%x, payload[752] = 0x%x, payload[940] = 0x%x, payload[1128] = 0x%x\n",
            //retv, payload[0], payload[188], payload[376], payload[564], payload[752], payload[940], payload[1128]);
            bEncrypted = 1;

            //printf("payload[376]: %x %x %x %x %x %x %x %x\n",
            //payload[376], payload[377],payload[378],payload[379], payload[380], payload[381],payload[382],payload[383]);
        }
#endif
        writtensize = 0;
        do
...
//  pthread_mutex_destroy(&s->mutex);
    memset(s, 0, sizeof(TSMulticastContext));
