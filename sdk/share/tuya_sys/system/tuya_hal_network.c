               ...
               OUT UNW_FD_SET_T *errorfds,IN const uint32_t ms_timeout)
{
    ...
    ret = select(maxfd, UNW_TO_SYS_FD_SET(readfds), UNW_TO_SYS_FD_SET(writefds), UNW_TO_SYS_FD_SET(errorfds), tmp);
    
    //return select(maxfd + 1, &fdsr, NULL, NULL, &timeout);
    //ret = select(maxfd + 1, UNW_TO_SYS_FD_SET(readfds), UNW_TO_SYS_FD_SET(writefds), UNW_TO_SYS_FD_SET(errorfds), &timeout);
  //  printf(">>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<111=%d, %d\n", ret, ms_timeout);
    return ret;
}
                              ...
                              IN const uint32_t cnt)
{
    ...
    ret1 = setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (void *)&keepalive , sizeof(keepalive));
    //printf("[Ctrl]setsocketopt set the new keepalive 001 value:%d \n",ret1);

    ret2 = setsockopt(fd, IPPROTO_TCP, TCP_KEEPIDLE, (void*)&keepidle , sizeof(keepidle));
    //printf("[Ctrl]setsocketopt set the new keepalive 002 value:%d \n",ret2);
    ret3 = setsockopt(fd, IPPROTO_TCP, TCP_KEEPINTVL, (void *)&keepinterval , sizeof(keepinterval));
    //printf("[Ctrl]setsocketopt set the new keepalive 003 value:%d \n",ret3);
    ret4 = setsockopt(fd, IPPROTO_TCP, TCP_KEEPCNT, (void *)&keepcount , sizeof(keepcount));
    //printf("[Ctrl]setsocketopt set the new keepalive value:%d \n",ret4);
...
int tuya_hal_net_res_init(void)
{
    return UNW_SUCCESS;
}
