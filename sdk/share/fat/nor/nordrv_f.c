...
int nor_readmultiplesector(F_DRIVER *driver, void *data, unsigned long sector, int cnt)
{
    ...
    if (!driver) return readSectors(sector, cnt, data) ? 0 : 1;
    ...
    {
        return readSectors(drvParam->partitionOffset + sector, cnt, data) ? 0 : 1;
    }
}
...
int nor_writemultiplesector(F_DRIVER *driver, void *data, unsigned long sector, int cnt)
{
    ...
    if (!driver) return writeSectors(sector, cnt, data) ? 0 : 1;
    ...
    {
        return writeSectors(drvParam->partitionOffset + sector, cnt, data) ? 0 : 1;
    }
}
