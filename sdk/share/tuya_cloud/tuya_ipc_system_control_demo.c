...
static mblk_ite *audio_m[16] = {0};
static uint8_t audio_qidx_r = 0, audio_qidx_w = 0;
static uint8_t start_calling = 0; 
...
{
    ithPrintf("rev audio cb len:%u sample:%d db:%d channel:%d\r\n", p_audio_frame->size, audio_sample, audio_databits, audio_channel);
    //PutAudioStreamPacketQueue(p_audio_frame->p_buf, p_audio_frame->size);
    //PCM-Format 8K 16Bit MONO
    //TODO
    /* Developers need to implement the operations of voice playback*/
    mblk_ite *m = allocb_ite(p_audio_frame->size);
    memcpy(m->b_wptr, p_audio_frame->p_buf, p_audio_frame->size);
    m->b_wptr += p_audio_frame->size;
    audio_m[audio_qidx_w] = m;
    audio_qidx_w++;
    audio_qidx_w = audio_qidx_w%16;
}

mblk_ite* get_TUYA_APP_audio(VOID)
{
    mblk_ite *m = audio_m[audio_qidx_r];
    if(m != NULL)
    {
        audio_m[audio_qidx_r] = NULL;
        audio_qidx_r++;
        audio_qidx_r = audio_qidx_r%16;
        return m;
    }else
        return NULL;
}

VOID TUYA_APP_Doorbell_Connect(VOID)
{
    ithPrintf("TUYA_APP_DOORBELL_CONNECT!\r\n");

    start_calling = 1;
}

VOID TUYA_APP_Doorbell_Stop(VOID)
{
    ithPrintf("TUYA_APP_DOORBELL_STOP!\r\n");

    start_calling = 0;
}

uint8_t get_TUYA_APP_call_state(VOID)
{
