...
VOID IPC_APP_upload_all_status(VOID)
{
    ...
    respone_dp_bool(TUYA_DP_CTRL_DOOR, IPC_APP_get_bootbell_ctrl_onoff() );
#endif
...
STATIC VOID handle_DP_SD_STORAGE_ONLY_GET(IN TY_OBJ_DP_S *p_obj_dp)
{
    ...
    snprintf(tmp_str, 100, "%u|%u|%u", total, used, empty);
    respone_dp_str(TUYA_DP_SD_STORAGE_ONLY_GET, tmp_str);
    printf("%s %d call sd card storage only get.\n",__FILE__,__LINE__);
}
#endif
...
#ifdef TUYA_DP_CTRL_DOOR
STATIC VOID handle_DP_CTRLDOORBELL(IN TY_OBJ_DP_S *p_obj_dp)
{
    BOOL_T m_ctrl_key_on_off = check_dp_bool_invalid(p_obj_dp);

    //当前处理开关锁.
    printf("[TUYA]Open Key --> %s .\n",(m_ctrl_key_on_off?"Open":"Close"));

    //只需要操作，不需要回馈.
    //respone_dp_bool(TUYA_DP_CTRL_DOOR, m_ctrl_key_on_off);
}
#endif
...
#endif
#ifdef TUYA_DP_CTRL_DOOR
    {TUYA_DP_CTRL_DOOR,           handle_DP_CTRLDOORBELL},
#endif
};
