...
STATIC INT_T __tuya_app_read_STR(CHAR_T *key, CHAR_T *value, INT_T value_size)
{
    ...
    value[strlen_tuya(value,__func__,__LINE__)] = 0;
    printf("readstr:%s \r\n", value);
    pclose(p_file);
    return 0;
}
...
VOID IPC_APP_set_alarm_zone_draw(cJSON * p_alarm_zone)
{
    ...
    INT_T i = 0;
    cJSON * pJson = ty_cJSON_Parse((CHAR_T *)p_alarm_zone);
    ...
    cJSON * tmp = ty_cJSON_GetObjectItem(pJson, "num");
    if (NULL == tmp){
        printf("%s %d step error\n",__FUNCTION__,__LINE__);
        ty_cJSON_Delete(pJson);
        //free(pResult);
        return ;
    }
    ...
    for (i = 0; i < strAlarmZoneInfo.iZoneNum; i++){
        ...
        cJSONRegion = ty_cJSON_GetObjectItem(pJson, region);
        if (NULL == cJSONRegion){
            printf("#####[%s][%d]error\n",__FUNCTION__,__LINE__);
            ty_cJSON_Delete(pJson);
            //free(pResult);
        ...
        strAlarmZoneInfo.alarmZone[i].pointX = ty_cJSON_GetObjectItem(cJSONRegion, "x")->valueint;
        strAlarmZoneInfo.alarmZone[i].pointY = ty_cJSON_GetObjectItem(cJSONRegion, "y")->valueint;
        strAlarmZoneInfo.alarmZone[i].width = ty_cJSON_GetObjectItem(cJSONRegion,  "xlen")->valueint;
        strAlarmZoneInfo.alarmZone[i].height = ty_cJSON_GetObjectItem(cJSONRegion, "ylen")->valueint;
...
void IPC_APP_set_patrol_time(cJSON * p_patrol_time)
{
    ...
    cJSON * pJson = ty_cJSON_Parse((CHAR_T *)p_patrol_time);
    if (NULL == pJson){
        TYWARN("----error---\n");

        return -1;
    }
    cJSON* t_start = ty_cJSON_GetObjectItem(pJson, "t_start");
    cJSON* t_end = ty_cJSON_GetObjectItem(pJson, "t_end");
...
void IPC_APP_set_preset(cJSON * p_preset_param)
{
...
#if 0
    cJSON * pJson = ty_cJSON_Parse((CHAR_T *)p_preset_param);
    if (NULL == pJson){
        TYERROR("null preset set input");
        return -1;
    }
    cJSON* type = ty_cJSON_GetObjectItem(pJson, "type");
    cJSON* data = ty_cJSON_GetObjectItem(pJson, "data");
    ...
    if(type->valueint == 1)
    {
        ...
        cJSON* name = ty_cJSON_GetObjectItem(data, "name");
        int name_len = 0;
        int error_num = 0;
    ...
    {
        cJSON* num = ty_cJSON_GetObjectItem(data, "num"); //can delete one or more
        cJSON* sets = ty_cJSON_GetObjectItem(data, "sets");
        char respond_del[128] = {0}; 
        cJSON* del_data;
        int del_num = num->valueint;
        for(i = 0; i < del_num; i++)
        {
            del_data = ty_cJSON_GetArrayItem(sets,i);
            cJSON* devId = ty_cJSON_GetObjectItem(del_data, "devId");  /*devid is the preset point number registered in the server*/
            cJSON* mpId = ty_cJSON_GetObjectItem(del_data, "mpId");  /*mpid is the preset point number managed on the device*/
    ...
    {
        cJSON* mpId = ty_cJSON_GetObjectItem(data, "mpId");
...
#ifdef TUYA_DP_CTRL_DOOR
BOOL_T IPC_APP_get_bootbell_ctrl_onoff(VOID)
{
    //get the current door-bell statue.
    return false;
}
#endif
