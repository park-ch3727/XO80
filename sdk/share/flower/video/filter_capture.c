...
#define FORCE_MEMMODE       0  //capture force mem mode.


CAPTURE_HANDLE   gCapDev0 = {0};
/*memory mode max : 720P*/
static int       gMaxWidth  = 1280;
static int       gMaxHeight = 720;
static unsigned short   gFramerate  = 0;
static unsigned short   ginterlaced = 0;
static bool             gIsMemMode  = false; //false : onfly mode true : memory mode
...
static void f_capture_process(IteFilter *f)
{
	...
	CAPTURE_SETTING onfly_analog_set   = {MMP_CAP_DEV_ANALOG_DECODER, MMP_TRUE,  MMP_FALSE, gMaxWidth, gMaxHeight};
	CAPTURE_SETTING onfly_digital_set  = {MMP_CAP_DEV_SENSOR, MMP_TRUE,  MMP_FALSE, gMaxWidth, gMaxHeight};
    ...
    if (gMaxWidth != 0 && gMaxHeight != 0)
    {
		...
		if (ithCapDeviceIsSignalStable())
			gIsMemMode = ithCapDeviceGetProperty(DEVICES_ISINTERLANCED);
		else
			gIsMemMode = true;//No signal
		...
		if(gIsMemMode == false)
		{
	    	if(ithCapDeviceGetProperty(DEVICES_IS_DECODER) == 1)
	        	ithCapConnect(&gCapDev0, onfly_analog_set);
			else
				ithCapConnect(&gCapDev0, onfly_digital_set);
		...
		if(gIsMemMode == false)
        {
            if (ithCapDeviceIsSignalStable())
            {
           		if (ithCapIsFire(&gCapDev0) == false)
        		{
	                ...
	                if(_SIGNALCHECK_FIRE(&gCapDev0) == 0)
	                {
	        ...
	        if((capturestate & 0x0F00) && (gIsError == false))
	        {
	        	gIsError = true;
	        	printf("[Error]ErrorCode = 0x%x\n",(capturestate >> 8) & 0xF);
	        }
...
IteFilterDes        FilterCaptureOnfly = {
