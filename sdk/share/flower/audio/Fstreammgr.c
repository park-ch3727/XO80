...
void streamQUninit(void){
    ...
    g.gcodecType=-1;
    if(g.qbuf) freemsg_ite(g.qbuf);
    iteAudioSetMusicCodecDump(0);
}

void streamQPut(unsigned char *inbuf,int size){
    //put data to Q ,fixed size=g.qbufsize:
    mblk_ite *buf=g.qbuf;
    int maxSize=g.qbufsize;
    int availSize=buf->b_wptr-buf->b_rptr;
    if(size==0) {//eof : put 0 to Q
        if(availSize){
            mblk_ite *om=allocb_ite(availSize);//size==0
            memcpy(om->b_wptr,buf->b_rptr,availSize);
            om->b_wptr+=availSize;
            pthread_mutex_lock(&g.mutex);
            putmblkq(&g.dataQ, om);
            pthread_mutex_unlock(&g.mutex);
        ...
        mblk_ite *om=allocb_ite(size);//size==0
        pthread_mutex_lock(&g.mutex);
        putmblkq(&g.dataQ, om);
        pthread_mutex_unlock(&g.mutex);
        printf("put Zeor Q\n");
    ...
    if(availSize+size<maxSize){
        memcpy(buf->b_wptr,inbuf,size);
        buf->b_wptr+=size;
    }else{
        unsigned int remain=maxSize-availSize;
        int residual=size-remain;
        int count=0;
        mblk_ite *om=allocb_ite(maxSize);
        memcpy(om->b_wptr,buf->b_rptr,availSize);
        om->b_wptr+=availSize;
        memcpy(om->b_wptr,inbuf,remain);
        om->b_wptr+=remain;        
        
        pthread_mutex_lock(&g.mutex);
        putmblkq(&g.dataQ, om);
        pthread_mutex_unlock(&g.mutex);
        buf->b_wptr-=availSize;
        
        while(residual>maxSize){
            unsigned int offset=count*maxSize+remain;
            mblk_ite *om=allocb_ite(maxSize);
            memcpy(om->b_wptr,inbuf+offset,maxSize);
            om->b_wptr+=maxSize;
        ...
        if(residual>0){
            unsigned int offset=count*maxSize+remain;
            memcpy(buf->b_wptr,inbuf+offset,residual);
            buf->b_wptr+=residual;
        }            
