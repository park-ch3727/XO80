...
_WifiMgr_Sta_Scan_Process(struct net_device_info *apInfo)
{
    ...
    while (1)
    {
        ...
        {
            // scan finish
            printf("[ WIFIMGR ] %s: Scan AP Finish(wifi_scan_done_hdl)\r\n", __FUNCTION__);
            break;
        }
...
_WifiMgr_Sta_HostAP_Switch_Thread(void *arg)
{
    ...
    if (gWifiMgrVar.WIFI_Mode == WIFIMGR_MODE_SOFTAP){
		...
		sleep(1);
		wifi_on(RTW_MODE_AP);
		printf("[ WIFIMGR ] WifiMgr_Sta_HostAP_Switch deinit done  \n");
