...
retry_scan:
    ...
    for (int i = 0; i < nAPCount; i++) {
        ...
        if (!strcmp(ssid, pList[i].ssidName)) {
            is_ssid_match = true;
            printf("[%s] Wanna connect to [%s], list is matched [%s(0x%X)] nChannel %d \n",
                __FUNCTION__, ssid, pList[i].ssidName, pList[i].securityMode,pList[i].channelId);
            if (pList[i].securityMode != 0){
                security_type = pList[i].securityMode;
                nChannel = pList[i].channelId;
            } else {
...
retry:
    ...
    if (is_ssid_match) {
        if (cRet != 0  && retry_connect_cnt < WIFIMGR_RECONNTECT_TIME){
            /* If SSID can be found but connection was ERROR or TIMEOUT, try again. */
        retry_connect_cnt++;

        WifiMgr_Sta_Disconnect();
        usleep(500*1000);
//            printf("[%s] wifi connect retry (%d/%d) times - %s\n", __FUNCTION__, retry_connect_cnt, WIFIMGR_RECONNTECT_TIME, (cRet == RTW_ERROR) ? "SSID can be found" : "SSID was connected timeout");

        goto retry;
        }
    } else {
        if (cRet != 0 /*RTW_TIMEOUT*/ && retry_connect_cnt < WIFIMGR_RECONNTECT_TIME) {
            /* If SSID can NOT be found and connection was TIMEOUT, try SCAN and CONNECT again. */
        retry_connect_cnt++;

            printf("[%s] wifi connect retry (%d/%d) times - Maybe it were AP issues\n", __FUNCTION__, retry_connect_cnt, WIFIMGR_RECONNTECT_TIME);
            gWifiMgrVar.Pre_Scan = false; //Renew the scan list,  avoid security changing suddenly.
            usleep(1000*1000);

            goto retry_scan;
        } else if (cRet != 0 /*RTW_ERROR*/ && retry_connect_cnt < WIFIMGR_RECONNTECT_TIME) {
    ...
    if (cRet != 0 /*(RTW_ERROR | RTW_TIMEOUT)*/ && retry_connect_cnt >= WIFIMGR_RECONNTECT_TIME) {
