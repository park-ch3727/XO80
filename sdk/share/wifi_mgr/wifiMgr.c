...
static WIFI_MGR_SCANAP_LIST             gWifiMgrTempApList[WIFI_SCAN_LIST_NUMBER] = {0};
        ...
        if (nCheckCnt == 0) {
            ...
            if (wifi_mode_now == WIFIMGR_MODE_SOFTAP) {
                ...
                if (gWifiMgrVar.Is_First_Connect) {
                    //Avoid the main thread run beyond 'complete of WifiMgr_Init'
                    while(!gWifiMgrVar.WIFI_Init_Ready){
                        usleep(100 * 1000);
                    }
                ...
                if (connstate == WIFIMGR_CONNSTATE_SETTING ||
                    ...
                    if (nWiFiConnState == WIFIMGR_CONNSTATE_STOP) {
                        ...
                        if (nWiFiConnEcode == WIFIMGR_ECODE_OK) {
                            if (WifiMgr_Get_Sta_Connect_Complete() < 1) {
