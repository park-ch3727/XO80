...
/****************************************************************************
 * MACRO define of ATMXT640U
 ****************************************************************************/
#define TOUCH_DEVICE_ID1      (0x4A)
#define TOUCH_DEVICE_ID2      (0x4B)
...
uint8_t I2C_application_address = TP_I2C_DEVICE_ID; //Need change to 640UD address
uint8_t ID_information[7] = {0xA6, 0x17, 0x10, 0xE1, 0x20, 0x14, 0x28}; 
uint8_t Txseq_num = 0;
...
static void _tpDoPowerOnSeq_vendor(void)
{
	//for Atmxt640u Linux  power on methods.
	    ...
	    */
		...
		switch (Object_table[0])
		{
			...
			case MXT_TOUCH_MULTITOUCHSCREEN_T100:
				...
				//printf("i=%d, T100_reportid_min = %x, %x\n",i, T100_reportid_min, T100_reportid_max);
				break;
		}
	}
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
	...
	//printf("mxtRdMsg: reportid_tmp = %x, T100_reportid_min = %x, T100_reportid_max = %x\n",reportid_tmp, T100_reportid_min, T100_reportid_max);
	if((reportid_tmp >= T100_reportid_min) && (reportid_tmp <= T100_reportid_max))
...
static int _tpParseRawPxy_vendor(struct ts_sample *smp, unsigned char *buf)
{
	...
	else if(touchscreen_reportid > (T100_reportid_min + 1))
	{
		...
		if(gTpSpec.tpMaxRawY > position_y)
			position_y = gTpSpec.tpMaxRawY - position_y;
		else
			position_y = 0;
 ...
 */ 
static int atmxt640u_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops atmxt640u_ops =
{
	atmxt640u_read,
};

TSAPI struct tslib_module_info *atmxt640u_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(atmxt640u_mod_init);
#endif
