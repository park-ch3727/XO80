...
//#define	ENABLE_GT911_PATCH_TOUCH_UP_ISSUE
...
static uint32_t	gtTchUpErrCnt = 0;
 ...
 ******************************************************************************/
bool MainDispMode_PW = 1; // 0:Multi Finger , 1:Single Finger  // JPHong 2024-11-28
	
bool XO_ituButtonSetMode_TP(bool SetMode)	// JPHong 2023-07-28
{
#if 0
	if(SetMode==1)
		gTpSpec.tpSampleNum         = 1;       //from this driver setting
	else
		gTpSpec.tpSampleNum         = 2;       //from this driver setting	
#endif		
	MainDispMode_PW = SetMode;
	
	
	printf("XO_ituButtonSetMode_TP : %d,%d\n",gTpSpec.tpSampleNum,MainDispMode_PW);
...
#define	TOUCH_ORG_SRC



void _tpDoPowerOnSeq_vendor1(char ResetPin,char IntPin)
{
	//for Gt911 Linux  power on methods.
		...
		return;
	#endif
	gTpSpec.tpResetPin = ResetPin;
	gTpSpec.tpIntPin = IntPin;
    printf("DO POWER-ON 1 sequence, reset pin:%d,int pin:%d\n",gTpSpec.tpResetPin,gTpSpec.tpIntPin);
...
void _tpDoPowerOnSeq_vendor2(char ResetPin,char IntPin)
{
	//for Gt911 Linux  power on methods.
	#ifdef TOUCH_ORG_SRC
		return;
	...
	gTpSpec.tpResetPin = ResetPin;
	gTpSpec.tpIntPin = IntPin;
    printf("DO POWER-ON 2 sequence, reset pin:%d,int pin:%d\n",gTpSpec.tpResetPin,gTpSpec.tpIntPin);
	if(gTpSpec.tpResetPin == (char)-1) return;
...
void _tpDoPowerOnSeq_vendor3(char ResetPin,char IntPin)
{
	//for Gt911 Linux  power on methods.
	#ifdef TOUCH_ORG_SRC
		return;
	...
	gTpSpec.tpResetPin = ResetPin;
	gTpSpec.tpIntPin = IntPin;
    printf("DO POWER-ON 3 sequence, reset pin:%d,int pin:%d\n",gTpSpec.tpResetPin,gTpSpec.tpIntPin);
    if(gTpSpec.tpResetPin == (char)-1) return;
...
static void _tpDoPowerOnSeq_vendor(void)
{
	...
	printf("DO POWER-ON ORG sequence, reset pin:%d,int pin:%d\n",gTpSpec.tpResetPin,gTpSpec.tpIntPin);

	#ifndef TOUCH_ORG_SRC
		return;
	#endif
#if 1
    printf("DO POWER-ON 1 sequence, reset pin:%d\n",gTpSpec.tpResetPin);	
    if(gTpSpec.tpResetPin == (char)-1) return;
...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
    ...
    if((tmpBuf[0] == 0x0) && (lastTmp >= 0x81) )
    {
        ...
        //It does not have to send the "exit-work" command
        #ifdef	ENABLE_TOUCH_PANEL_DBG_MSG
    ...
    {
    	#ifdef	ENABLE_GT911_PATCH_TOUCH_UP_ISSUE
    	if(gTpInfo.tpStatus == TOUCH_DOWN)
    	{
    		...
    		{
        		recv = false;
        		//printf("rdBfrErr1 (%x, %x, %x)\n",touch_num, GTP_MAX_TOUCH, tmpBuf[0]);    
        		goto exit_work_func;
    		}
    		else
    		{
        		//printf("skip TP this time2: %x, %x\n", gTpInfo.tpStatus, gtTchUpErrCnt);     
        		return 1;
    		}
    	}
    	else
    	{
        	recv = false;
        	//printf("rdBfrErr2 (%x, %x, %x)\n",touch_num, GTP_MAX_TOUCH, tmpBuf[0]);    
        	gtTchUpErrCnt = 0;    
        	goto exit_work_func;
    	}
    	#else
        recv = false;
        //printf("rdBfrErr0 (%x, %x, %x)\n",touch_num, GTP_MAX_TOUCH, tmpBuf[0]);       
        goto exit_work_func;
        #endif
    }
    ...
    #ifdef  TP_MULTI_FINGER_ENABLE
	    if (MainDispMode_PW==1)		//Main_Disp_Mode = MAIN_MODE_PW;if (MainDispMode_PW==1)		//Main_Disp_Mode = MAIN_MODE_PW;
	...
	while(real_nr++<nr) 
	{
	    ...
	    switch(res)
	    {
	        ...
	        default:    //error, send touch-up
	        	/*
	        	  To skip some GT911's meaningless INT signal when TP event is touch-up(TOUCH_NO_CONTACT)
	        	*/
	        	if(gTpInfo.tpStatus == TOUCH_NO_CONTACT)	gTpInfo.tpCurrINT = 0;
...
static void _tpUpdateLastXY(struct ts_sample *smp)
{
	...
	else
    {
		...
		if( smpChgFlag )
		{
			static int lastFnr = 0;
			int cFnr = (int)smp->finger;
			
			if(smp->finger < lastFnr)	cFnr = lastFnr;

			memcpy((void *)&g_sample[0] ,(void *)smp, sizeof(struct ts_sample)*cFnr);
			lastFnr = (int)smp->finger;
			SendQueCnt++;
			//printf("inQ1:%d\n",SendQueCnt);
...
static int gt911_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
	...
	{
		static int lastFnr = 0;
		int currFnr = samp->finger;
		printf("    dQ1:[%x] f=%d, id=%d (%d,%d,%d)r=%d\n", samp, samp->finger, samp->id, samp->pressure, samp->x, samp->y, ret );
		
		if( (lastFnr==2) && (samp->finger==1) )		currFnr = lastFnr;		
		if(currFnr>1)
		{
			struct ts_sample *tsp = (struct ts_sample *)samp->next;
			printf("    dQ2:[%x] f=%d, id=%d (%d,%d,%d)\n", tsp, tsp->finger, tsp->id, tsp->pressure, tsp->x, tsp->y);
		}
