...
void AudioThreadFunc()
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            {
                usleep(10000);//wait 10 ms for WP register set;
...
stop:
        ...
        { 
            ...
            if (smtkAudioMgrGetInterruptStatus() == 1 && smtkAudioMgrGetInterruptOverwriteStatus() == 0)
            {
                ...
                if (audioMgr.playing /*i2s_get_DA_running()*/ && mtal_pb_check_fileplayer_with_audio() == 0)
                {
...
static int audio_special_case_play()
{
    ...
    if (i2s_get_DA_running() && audioMgr.ptNetwork.nSpecialCase == 1)
    {
        ...
        int             copySize    = 0;
        unsigned int    pcmsize     = audioMgr.ptNetwork.nLocalFileSize - SMTK_AUDIO_WAV_HEADER;
        unsigned char   *pcmptr     = audioMgr.sampleBuf + SMTK_AUDIO_WAV_HEADER;

        I2S_DA32_SET_WP(I2S_DA32_GET_RP());
        gOutReadPointer     = I2S_DA32_GET_RP();
        gOutWritePointer    = I2S_DA32_GET_WP();
        dstBuffer           = gOutBuffer + gOutWritePointer;
        i2sNewWP            = gOutWritePointer;

        if (gOutWritePointer + pcmsize > nBufferLength)
        ...
        if (gOutWritePointer + pcmsize <= nBufferLength)
        {
            ...
            copySize    = pcmsize;
            i2sNewWP    = i2sNewWP + copySize;
        }
        else if (gOutWritePointer + pcmsize > nBufferLength)
        {
            //dstBuffer   = gOutBuffer + i2sNewWP;
            copySize    = nBufferLength - i2sNewWP;
            memcpy(dstBuffer, pcmptr, copySize);

            nOffset     = copySize;
            dstBuffer   = gOutBuffer;
            copySize    = pcmsize - nOffset;
            i2sNewWP    = copySize;
        }
        else
        {
        ...
        if (copySize > 0)
        {
            memcpy(dstBuffer, pcmptr + nOffset, copySize);
        #if CFG_CPU_WB
