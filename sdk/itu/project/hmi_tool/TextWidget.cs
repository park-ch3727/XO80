...
namespace GUIDesigner
{
    class TextWidget : Label, IWidget
    {
        ...
        {
            if (ITU.fontFace[this.FontIndex] != null)
            {
                uint fontWidth = (uint)Math.Round(this.Font.Size);
                int boldSize = base.Font.Bold ? this.BoldSize : 0;
                ITU.DrawText(this.FontIndex, fontWidth, this.ClientSize, this.Text, this.TextAlign, boldSize, this.ForeColor, e.Graphics);
            }
            else
                base.OnPaint(e);
        }
        ...
        public ITUWidget CreateITUWidget()
        {
            ...
            //fix bug
            //when user had modified Text1 ~ Text31 with empty string
            //stringSet count should not be +1
            //solution is try to force make these string address null
