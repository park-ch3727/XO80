...
#ifdef HUD
static GFXSurface* tempImg = NULL;
//static GFXSurface* Lcdsuf;
#endif
...
static void M2dHWFlip(ITUSurface *surf)
{
    ...
    {
#ifdef HUD
        //uint32_t Start = ithTimerGetCounter(ITH_TIMER4);
        //uint32_t End = 0;
        ...
        if (gfxGetHUDTable() != 3)
        {
            ...
            //printf("LcdAddrA():0x%x LcdAddrB():0x%x LcdAddrC():0x%x\n", ithLcdGetBaseAddrA(), ithLcdGetBaseAddrB(), ithLcdGetBaseAddrC());
            ...
            src.srcX = 0;
            src.srcY = 0;
            src.srcWidth = ithLcdGetWidth(); //800;
            src.srcHeight = ithLcdGetHeight(); //480;
            gfxSurfaceBitblt(tempImg, 0, 0, &src);

            //gfxwaitEngineIdle();

            surface.dstSurface = srcSurf->m2dSurf;
            surface.dstX = 0;
            surface.dstY = 0;
            ...
            surface.dstSurface = srcSurf->m2dSurf; //Lcdsuf;// 
            surface.dstX = 0;
            surface.dstY = 0;
            surface.dstWidth = ithLcdGetWidth(); //800;
            surface.dstHeight = ithLcdGetHeight(); //480;

            src.srcSurface = tempImg;
            src.srcX = 0;
            src.srcY = 0;
            src.srcWidth = Block_W;
            src.srcHeight = Block_H;

            //printf("srcSurf->m2dSurf->imageData:0x%x\n", srcSurf->m2dSurf->imageData);
            gfxUpdateSrcAddr(tempImg->imageData, srcSurf->m2dSurf->imageData);
            ...
            //src.srcSurface = Lcdsuf;
            //src.srcX = 0;
            //src.srcY = 0;
            //src.srcWidth = 800;
            //src.srcHeight = 480;
            //gfxSurfaceBitblt(srcSurf->m2dSurf, 0, 0, &src);
            //gfxwaitEngineIdle();

        }
        //End = ithTimerGetCounter(ITH_TIMER4);
        //printf("doing 2D time:%d \n ", End - Start);
#endif
        ...
        //gfxDestroySurface(tempImg);
        //gfxDestroySurface(Lcdsuf);
    }
...
#ifdef HUD
    dstSurf = (M2dSurface *)lcdSurf;

    //ithTimerCtrlEnable(ITH_TIMER4, ITH_TIMER_UPCOUNT);
    //ithTimerCtrlEnable(ITH_TIMER4, ITH_TIMER_PERIODIC);
    //ithTimerSetCounter(ITH_TIMER4, 0);
    //ithTimerSetMatch(ITH_TIMER4, ithGetBusClock());
    //ithTimerEnable(ITH_TIMER4);

    tempImg = gfxCreateSurface(ithLcdGetWidth(), ithLcdGetHeight(), ithLcdGetWidth() * 2, GFX_COLOR_RGB565);
    //Lcdsuf = gfxCreateSurface(1024, 600, 1024 * 2, GFX_COLOR_RGB565);

    gfxUpdateSrcAddr(tempImg->imageData, dstSurf->m2dSurf->imageData);
#endif
