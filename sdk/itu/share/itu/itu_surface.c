...
ITUSurface *ituSurfaceDecompress(ITUSurface *surf)
{
	...
	if (surf->flags & ITU_JPEG)
	{
		...
		if (ituScene->surfpl)
		{
			uint8_t* pbuf = ithMapVram(ituScene->vPoolBuf, ituScene->poolSize, ITH_VRAM_READ);
			ituDestroySurface(ituScene->surfpl);
			ituScene->surfpl = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format, pbuf, ITU_STATIC);
			ithUnmapVram(pbuf, ituScene->poolSize);
			//printf("[SurfacePool] %d x %d [jpeg format: %d]\n", surf->width, surf->height, surf->format);
		}
		#endif
		...
		if (surf->format == ITU_ARGB8888)
		{
			...
			ituJpegStoreAlphaPlane(vmem, buf, alphaSize);
#endif
#endif
		}
		else //JPEG_565
		{
			...
			#else
			ituJpegLoad(surf->width, surf->height, (uint8_t *)surf->addr, surf->size, 0);
			#endif
		}
	}
	else
	{
		...
		if (ituScene->surfpl)
		{
			...
			ituScene->surfpl = ituCreateSurface(surf->width, surf->height, surf->pitch, surf->format, pbuf, ITU_STATIC);
			ithUnmapVram(pbuf, ituScene->poolSize);
			buf = ithMapVram(ituScene->surfpl->addr, bufSize, ITH_VRAM_WRITE);
			//printf("[SurfacePool] %d x %d [format: %d] [size %d]\n", surf->width, surf->height, surf->format, bufSize);
		}
#endif
		...
		//else
		//	printf("---[%d]---bufsize[%d]--compressedSize[%d]\n", __LINE__, bufSize, compressedSize);
...
#ifdef CFG_ITU_DECOMPRESS_SURFACE_INPLACE
	if ((ituScene->surfpl) && (retSurf == NULL))
	{
		//printf("heap size: %d\n", itpGetFreeHeapSize());
		return ituScene->surfpl;
	}
#endif
...
void ituSurfaceRelease(ITUSurface *surf)
{
	...
	{
		//printf("[SurfacePool] release bypass!\n");
		return;
	}
#endif
