...
void ituIconLoadExternalDataNULL(ITUIcon* icon)
{
        ...
        else if (desta == 255)
        {
            ...
            else
            {
                ...
                else
                {
					...
					if ((icon->surf->width != rect->width) || (icon->surf->width != rect->width))
					{
						int fitSide = (icon->surf->width > icon->surf->height) ? (icon->surf->width) : (icon->surf->height);
						...
						ituBitBlt(tmpSurf, (fitSide - icon->surf->width) / 2, (fitSide - icon->surf->height) / 2, icon->surf->width, icon->surf->height, icon->surf, 0, 0);
						ituRotate(dest, destx - (fitSide - rect->width) / 2, desty - (fitSide - rect->height) / 2, tmpSurf, tmpSurf->width / 2, tmpSurf->height / 2, (float)widget->angle, 1.0f, 1.0f);
...
#ifdef CFG_ITU_DECOMPRESS_SURFACE_INPLACE
	if (!icon->staticSurf || (widget->flags & ITU_EXTERNAL) || (widget->flags & ITU_EXTERNAL_IMAGE))
		return;
#else
