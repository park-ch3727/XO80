...
static bool MainDispMode_PW_BT = 1; // 0:Multi Finger , 1:Single Finger  // JPHong 2024-11-28
	
bool XO_ituButtonSetMode_Bt(bool SetMode)	// JPHong 2023-07-28
{
	MainDispMode_PW_BT = SetMode;
	return true;
}			
	...
	else if (ev == ITU_EVENT_MOUSEMOVE)
	{
		if (ituWidgetIsEnabled(widget))
		{
			...
			if (((btn->press_alpha_delay_count < 255) && (arg1 == PRESS_ALPHA_DELAY_ARG)) || bWidgetInSideCheck)
			{
				...
				{
					//printf("unpress %s\n", widget->name);
					if (MainDispMode_PW_BT==1)		//Main_Disp_Mode = MAIN_MODE_PW;
					{
						printf("unpress %s\n", widget->name);
//						ituButtonSetPressed(btn, false);		// JPHong 2023-07-28
					}
				}
				btn->press_alpha_delay_count = 0;
			}
...
void ituButtonDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
	...
	if (widget->flags & ITU_EXTERNAL)
		ituIconLoadExternalDataNULL(&(btn->bg.icon));
	else
		ituIconLoadStaticData(&(btn->bg.icon));
