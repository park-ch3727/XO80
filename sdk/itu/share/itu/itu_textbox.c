...
void ituTextBoxDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    ...
    else if (textbox->textboxFlags & ITU_TEXTBOX_MULTILINE)
    {
		...
		int maxW = (rect->width > dest->width) ? (dest->width) : (rect->width);
		int maxH = (rect->height > dest->height) ? (dest->height) : (rect->height);
        uint8_t desta, destbga;
        ITURectangle prevClip;
        char *pch, *saveptr;
        ...
        if (destbga == 255)
        {
			...
			//printf("[%d]textbox colorfill on dest(%d/%d) from (%d/%d) to (%d/%d)\n", __LINE__, dest->width, dest->height, minx, miny, minx + mdw, miny + mdh);
        }
        else if (destbga > 0)
        {
			//try to fix error blt when textbox widget size too large
            ...
            {
                ituColorFill(surf, 0, 0, rect->width, rect->height, &text->bgColor);
				//printf("[%d]textbox alphablend on dest(%d/%d) from (%d/%d) to (%d/%d)\n", __LINE__, dest->width, dest->height, minx, miny, minx + mdw, miny + mdh);
        ...
        else if (desta > 0)
        {
			...
			{
				int yy = 0;
				int startX = (destx < 0) ? (0) : ((destx > dest->width) ? (dest->width) : (destx));
				int startY = (desty < 0) ? (0) : ((desty > dest->height) ? (dest->height) : (desty));
				int endX = (destx + rect->width) > (dest->width) ? (dest->width) : (destx + rect->width);
				int endY = (desty + rect->height) > (dest->height) ? (dest->height) : (desty + rect->height);
				int validW = endX - startX;
				int validH = endY - startY;
				//try to fix error blt when textbox widget size too large
				if ((rect->width > dest->width) || (rect->height > dest->height))
				{
					ITUColor emptycolor = { 0, 0, 0, 0 };
					int surfX = (destx > 0) ? (0) : (-destx);
					int surfY = (desty > 0) ? (0) : (-desty);
					ituColorFill(surf, 0, 0, rect->width, rect->height, &emptycolor);
					ituBitBlt(surf, surfX, surfY, validW, validH, dest, startX, startY);
					printf("[%d]textbox bitblt dest(%d/%d) (%d,%d)/(%d,%d) to surf(%d/%d)\n", __LINE__, dest->width, dest->height, startX, startY, validW, validH, surfX, surfY);
				}
				else
				{
					ituBitBlt(surf, 0, 0, rect->width, rect->height, dest, destx, desty);
					printf("[%d]textbox bitblt dest(%d/%d) (%d,%d)/(%d,%d)\n", __LINE__, dest->width, dest->height, destx, desty, rect->width, rect->height);
				}
				ituSetColor(&surf->fgColor, desta, widget->color.red, widget->color.green, widget->color.blue);
				if (text->fontHeight > 0)
					ituFtSetFontSize(text->fontWidth, text->fontHeight);
				...
				if (surf)
				{
					...
					int rew = rex - vdx;
					int reh = rey - vdy;
					//printf("[%d]alphablend surf(%d/%d) (%d,%d)/(%d,%d) to dest\n", __LINE__, surf->width, surf->height, 0, 0, rect->width, rect->height);
					//ituAlphaBlend(dest, destx, desty, rect->width, rect->height, surf, 0, 0, desta);

					printf("[%d]textbox alphablend surf(%d/%d) (%d,%d)/(%d,%d) to dest(%d/%d)\n", __LINE__, surf->width, surf->height, modx, mody, rew, reh, vdx, vdy);
