...
//printf("ituIconListBoxDraw========>>>chpark=======\n\n");
    ...
    for (node = widget->tree.child; node; node = node->sibling)
    {
        ...
        if (childbga == 255)
        {
            ...
            //printf("ituIconListBoxDraw:childbga==chpark==>index=%d  i=%d count=%d  childbga=%d \n\n",index,i,count,childbga);
        ...
        if (childa > 0)
        {
            ...
            {
                int index = atoi(str);
                //printf("ituIconListBoxDraw========>>>chpark=======index=%d\n\n",index);
                
                if (index < ITU_ICON_LISTBOX_TYPE_COUNT)
                {
                    //if (i == (count + ilistbox->listbox.focusIndex) && ilistbox->focusSurfArray[index] )//chpark.06.21
                    if (i == (count + ilistbox->listbox.focusIndex) && ilistbox->focusSurfArray[index] && ilistbox->listbox.focusIndex!=-1)//chpark.06.09
                    {
                        /*    
                        if(ilistbox->focusSurfArray[index])
                            printf("Image YES\n");
                        else
                            printf("Image NO\n");
                        */
                        surf = ilistbox->focusSurfArray[index];
                        //printf("ituIconListBoxDraw:Focus==chpark==>index=%d  i=%d count=%d  focusIndex=%d \n\n",index,i,count,ilistbox->listbox.focusIndex);
                    }
                    else
                    {
                        //if(ilistbox->focusSurfArray[index])
                        ...
                        //printf("ituIconListBoxDraw:Back==chpark==>index=%d  i=%d count=%d  focusIndex=%d \n\n",index,i,count,ilistbox->listbox.focusIndex);
                        surf = ilistbox->surfArray[index];
                        if (surf)
                        {
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf=%d\n",i,index,surf);

                        }
                        else
                        {
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf0=%d\n",i,index,ilistbox->surfArray[0]);
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf1=%d\n",i,index,ilistbox->surfArray[1]);
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf2=%d\n",i,index,ilistbox->surfArray[2]);
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf3=%d\n",i,index,ilistbox->surfArray[3]);    
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf4=%d\n",i,index,ilistbox->surfArray[4]);
                            //printf("ituIconListBoxDraw:Back==chpark==>No Error: i=%d index=%d  surf5=%d\n",i,index,ilistbox->surfArray[5]);
                ...
                {
                    //printf("ituIconListBoxDraw:Back==chpark==>Error: %d index=%d\n",i,index);
    ...
    if (!(widget->flags & ITU_EXTERNAL))
    {
        ...
        for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
        {
            if (ilistbox->staticSurfArray[i])
			{
                ...
                //printf("ituIconListBoxLoad=[YES surfArray] staticSurfArray:%d %d\n",ilistbox->staticSurfArray[i],i);
            }
            else
            {
                //printf("ituIconListBoxLoad=[NO surfArray:d]\n",i);
            }
        }
...
void ituIconListBoxLoadStaticData(ITUIconListBox* ilistbox)
{
      ...
      //printf("\n\n\n  == ituIconListBoxLoadStaticData ==chpark==== \n\n\n");
    ...
    for (i = 0; i < ITU_ICON_LISTBOX_TYPE_COUNT; i++)
    {
        ITUSurface* surf;
        //chpark.06.21
        //When running the list for the first time, it is modified to reload even if the surface is held.
        //============================================================================
        //if (!ilistbox->staticSurfArray[i] || ilistbox->surfArray[i])
        //    break;
        if(ilistbox->surfArray[i])
        {
            //printf("ituIconListBoxLoadStaticData=[YES surfArray] LOAD:%d \n",i);
            //break;//chpark.06.21
        }

        if(!ilistbox->staticSurfArray[i])
        {
            // printf("ituIconListBoxLoadStaticData=[NO staticSurfArray] LOAD:%d \n",i);
            break;
        }
        //============================================================================
