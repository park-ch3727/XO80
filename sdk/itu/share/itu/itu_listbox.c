            ...
            if (ituWidgetIsInside(widget, x, y))
            {
                ...
                for (node = widget->tree.child; node; node = node->sibling)
                {
                    ...
                    if (ituWidgetIsInside(item, x1, y1))
                    {
                        ...
                        {
                        	printf("ituListBoxUpdate : ITU_EVENT_MOUSEDOWN\n");
...
void ituListBoxSelect(ITUListBox* listbox, int index)
{
    ...
    if ((listbox->focusIndex >= 0)||((listbox->focusIndex<0)&&(index<0)))
    {
...
ITUWidget* ituListBoxGetFocusItem(ITUListBox* listbox)
{
    ...
    ITU_ASSERT_THREAD();
#ifdef DBG_MSG
	printf("ituListBoxGetFocusItem : %d\n",listbox->focusIndex);
#endif
    if (listbox->focusIndex >= 0)
    {
        ITUWidget* item;
...
ITUWidget* ituListBoxGetItem(ITUListBox* listbox,int ItemIndex)		// JPHong 2023-05-02
{
    ...
    if (ItemIndex >= 0)
    {
        ...
        item = (ITUWidget*)itcTreeGetChildAt(listbox, count + ItemIndex);
        return item;
    }
    return NULL;
}
...
void ituListBoxSetItemRead(ITUListBox* listbox, ITUWidget* item, bool read)
{
    ...
    ITU_ASSERT_THREAD();
#ifdef DBG_MSG
	printf("ituListBoxSetItemRead : %d\n",read);
#endif
    if (item)
    {
