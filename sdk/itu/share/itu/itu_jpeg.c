...
PktQueue gAlphaInputQueue;
static bool bAlphaQueueInit = false;
#endif
//=============================================================================
//				  Private Function Definition
//=============================================================================
static pfPktReleaseCb
_alphaInputPktRelease(
    void* pkt)
{
    AlphaInputPkt *ptAlphaInputPkt = (AlphaInputPkt*)pkt;
    if (ptAlphaInputPkt && ptAlphaInputPkt->pInputBuffer && ptAlphaInputPkt->bufferSize)
    {
        ithUnmapVram(ptAlphaInputPkt->pInputBuffer, ptAlphaInputPkt->bufferSize);
		itpVmemFree(ptAlphaInputPkt->addr);
    }
    free(pkt);
}
...
#ifdef CFG_ITU_DECOMPRESS_SURFACE_INPLACE
void ituJpegStoreAlphaPlane(uint32_t vmem, uint8_t *buf, int alphaSize)
{
	AlphaInputPkt *alphaPkt = (AlphaInputPkt*) malloc(sizeof(AlphaInputPkt));
    if (alphaPkt)
    {
    	alphaPkt->addr = vmem;
        alphaPkt->pInputBuffer = buf;
		alphaPkt->bufferSize = alphaSize;
		//printf("input: vmem = 0x%x\n", vmem);
        _packetQueuePut(&gAlphaInputQueue, alphaPkt);
    }
}
#endif
...
ITUSurface *ituJpegAlphaLoad(int width, int height, uint8_t *alpha, uint8_t *data, int size)
{
    ...
    else
    {
		...
		if(!bAlphaQueueInit)
		{
			bAlphaQueueInit = true;
			_packetQueueInit(&gAlphaInputQueue, _alphaInputPktRelease, 10);
		}

		{
			AlphaInputPkt *ptAlphaPkt = NULL;
			while(gAlphaInputQueue.numPackets >= 3)
			{
				if (_packetQueueGet(&gAlphaInputQueue, (void**) &ptAlphaPkt, 0) > 0)
                {
                    _alphaInputPktRelease(ptAlphaPkt);
                }
			}
		}		
#endif
