    ...
    else if (ev == ITU_EVENT_MOUSEDOWN)
    {
        ...
        if (ituWidgetIsEnabled(widget))
        {
            ...
            if (ituWidgetIsInside(widget, x, y))
            {
...

                {
					...
					if (ituWidgetIsInside(widget, x, y) && ((dist < ITU_DRAG_DISTANCE) || !(widget->flags & ITU_DRAGGABLE)))
					{
					    ...
					    for (; node; node = node->sibling)
					    {
					        ...
					        {
					            {
					            	printf("ITU_EVENT_MOUSEDOWN 2 : %d , %d\n",i,listbox->focusIndex);
					                ituListBoxSelect(listbox, i);
								//	listbox->focusIndex = i;
                ...
                result = true;
                printf("ITU_EVENT_MOUSEDOWN 1 ==%d\n");
                //check scroll touch.
                ituExecActions(widget, listbox->actions, ITU_EVENT_CUSTOM+9, i);//chpark.07.18
            }
        ...
        if (ituWidgetIsEnabled(widget))
        {
            ...
            if (ituWidgetIsInside(widget, x, y) && ((dist < ITU_DRAG_DISTANCE) || !(widget->flags & ITU_DRAGGABLE)))
            {
                ...
                for (; node; node = node->sibling)
                {
                   ...
                   //             ituListBoxSelect(listbox, i);
                   //             ituExecActions(widget, listbox->actions, ITU_EVENT_CUSTOM, i);
                  // listbox->focusIndex = i;
                                if(dist < ITU_DRAG_DISTANCE)
                                {
                                    printf("[ITU_EVENT_MOUSEUP 2][in ITU_DRAG_DISTANCE]: %d , %d\n",i,listbox->focusIndex);
                                    printf("[ITU_EVENT_MOUSEUP 2][send: custom8]: dist=%d , ITU_DRAG_DISTANCE=%d\n",dist,ITU_DRAG_DISTANCE);
                                    ituExecActions(widget, listbox->actions, ITU_EVENT_CUSTOM+8, i);
                                }
                                else
                                {
                                     printf("[ITU_EVENT_MOUSEUP 2][Over ITU_DRAG_DISTANCE]: %d , %d\n",i,listbox->focusIndex);
                                     printf("[ITU_EVENT_MOUSEUP 2][NOT send: custom8]: dist=%d , ITU_DRAG_DISTANCE=%d\n",dist,ITU_DRAG_DISTANCE);
            ...
            else
            {
                //=========================================
                //chpark.06.20
                //Added to catch a signal when the mouse is right-clicked out of the list area
                //리스트의 영역을 벗어나서 오른쪽 마우스 업이 되었을 때 신호를 캐치하기 위해서 추가함
                if(ituWidgetIsVisible(widget))
                {
                    if (ituWidgetIsInside(widget, x, y))
                    {
                        printf("ITU_EVENT_MOUSEUP 4 Inside================\n");
                    }
                    else
                    {
                        printf("ITU_EVENT_MOUSEUP 5 Outside================\n");
                    }
                    printf("[ITU_EVENT_MOUSEUP 6][send: custom8 + 10002] dist=%d,ITU_DRAG_DISTANCE=%d\n",dist,ITU_DRAG_DISTANCE);
                    
                    if(dist<ITU_DRAG_DISTANCE)
                    {
                        //ituExecActions(widget, listbox->actions, ITU_EVENT_CUSTOM+8, (int)124);
                    }
                    else
                    {
                        //can find out outside touchup .
                        //for dragging catch
                        ituExecActions(widget, listbox->actions, ITU_EVENT_CUSTOM+10, (int)124);
                    }
