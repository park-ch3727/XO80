...
void *_hid_read_thread_func(void *arg)
{
    ...
    {
        int len = ioctl(ITP_DEVICE_USBDHID, ITP_IOCTL_USBD_HIDG_READ, &usbData);
        if(len > 0)
        {
            int i;
...
int HidTransparent_AddConfig(HID_TRANSPARENT_CONFIG *ptConf)
{
    ...
    if(ioctl(ITP_DEVICE_USBDHID, ITP_IOCTL_USBD_HIDG_SET_CONFIG, pFc))
    {
        USB_COMPOSITE_OVERWRITE covr = _regen_usbd_id();
        ioctl(ITP_DEVICE_USBDHID, ITP_IOCTL_USBD_HIDG_OVERWRITE_VIDPID, &covr);
        ioctl(ITP_DEVICE_USBDHID, ITP_IOCTL_USBD_HIDG_REFRESH_CONFIG, NULL);
    }
...
int HidTransparent_Write(struct hid_device *hid, HIDG_FUNC_CMD_DATA *usbData)
{
    ...
    list_for_each_entry(phid_info, &hid_info_mapping_list, list) {
        if(hid == phid_info->hid) {
            ...
            ret = ioctl(ITP_DEVICE_USBDHID, ITP_IOCTL_USBD_HIDG_WRITE, ptData);
            free(ptData->buffer);
            free(ptData);
            break;
        }
