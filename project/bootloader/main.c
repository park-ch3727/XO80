    ...
    if (usbInited)
    {
        ...
        while (!found)
        {
            ...
            else
            {
                ...
                if (timeout <= 0)
                {
                    LOG_INFO "0 USB device not found.\n" LOG_END
#ifdef CFG_UPGRADE_FILE_FOR_NAND_PROGRAMMER
                    gDoReMapFlow = 0;
#endif
                    return;
...
static void InitFileSystem1(void)
{

	LOG_INFO "\nStart InitFileSystem1\n\n" LOG_END
    // init card device
#if  !defined(CFG_UPGRADE_USB_DEVICE) && !defined(_WIN32) && (defined(CFG_SD0_ENABLE) || defined(CFG_SD1_ENABLE) || defined(CFG_MSC_ENABLE) || defined(CFG_USBH_CD_MST) || defined(CFG_RAMDISK_ENABLE))
    itpRegisterDevice(ITP_DEVICE_CARD, &itpDeviceCard);
    ioctl(ITP_DEVICE_CARD, ITP_IOCTL_INIT, NULL);
...
static void InitFileSystem2(void)
{

	LOG_INFO "\n Start InitFileSystem2\n\n" LOG_END
    ...
    if (usbInited)
    {
        ...
        while (!found)
        {
            ...
            else
            {
                ...
                if (timeout <= 0)
                {
                    LOG_INFO "2 USB device not found.\n" LOG_END
#ifdef CFG_UPGRADE_FILE_FOR_NAND_PROGRAMMER
                    gDoReMapFlow = 0;
#endif
                    return;
...
static void DoUpgrade(void)
{
    ...
    LOG_INFO "Do Upgrade...1\r\n" LOG_END
    LOG_INFO "Do Upgrade...2\r\n" LOG_END
    

    upgradeFile = OpenUpgradePackage();
    
    LOG_INFO "Do Upgrade...3...%d\r\n",upgradeFile LOG_END
    if (upgradeFile)
    {
        int ret = 0;
		int ret2 = 0;
        ...
        {
            LOG_ERR "DoUpgrade : Upgrade failed.1\n" LOG_END
            ShowUpgradeFail();
            while (1)
                sleep(10);
        }
        ...
        ret2 = DeleteUpgradePackage();
    #endif

    #if defined(CFG_NOR_ENABLE) && CFG_NOR_CACHE_SIZE > 0
        LOG_INFO "DoUpgrade : Flushing NOR cache...\n" LOG_END
        ioctl(ITP_DEVICE_NOR, ITP_IOCTL_FLUSH, NULL);
    #endif

        if (ret)
        {
            LOG_INFO "DoUpgrade : Upgrade failed.2\n" LOG_END
            ShowUpgradeFail();
            while (1)
                sleep(10);
        }
        ...
        {
            stop_led = true;
            LOG_INFO "DoUpgrade : Upgrade finished.\n" LOG_END
#if 1	// Test   ORG
            if (ret2)
            {
	            ShowRemoveUSB();
	            LOG_INFO "\n\n\n\n\n\nDoUpgrade : Flushing NOR cache...\n" LOG_END
	            LOG_INFO "DoUpgrade : Upgrade finished.\n" LOG_END    
...
{
	LOG_INFO "backlightResetTask Start\r\n" LOG_END
 //   usleep(100000);
    ioctl(ITP_DEVICE_BACKLIGHT, ITP_IOCTL_RESET, NULL);
	LOG_INFO "backlightResetTask End\r\n" LOG_END
    return NULL;
}
#endif
...
#ifdef CFG_DASHBOARD
    ithGpioSetOut(ITH_GPIO_PIN59);//BL ENABLE
    ithGpioSet(ITH_GPIO_PIN59);
#endif

#ifdef CFG_BL_SHOW_LOGO
	LOG_INFO "Start CFG_BL_SHOW_LOGO\n" LOG_END
	
	#if 0	//(LCD_MODULE_SEL==ITS_LCD)	// JPHong ORG
	#else
	    ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
	    ShowLogo();
    #endif
	LOG_INFO "End CFG_BL_SHOW_LOGO\n" LOG_END
...
#endif // CFG_UPGRADE_USB_DEVICE
LOG_INFO "CFG_BACKLIGHT_ENABLE : Start\n" LOG_END
	#if 0	//(LCD_MODULE_SEL==ITS_LCD)	// JPHong ORG
	#else
		#ifdef CFG_BACKLIGHT_ENABLE
		    {
		...
		//		ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_RESUME, NULL);
		    ...
		    blLcdOn = true;
	#endif  
 LOG_INFO "CFG_BACKLIGHT_ENABLE : End\n" LOG_END     
#if defined(CFG_UPGRADE_PRESSKEY) || defined(CFG_UPGRADE_RESET_FACTORY) || defined(CFG_UPGRADE_RECOVERY)
    ioctl(ITP_DEVICE_KEYPAD, ITP_IOCTL_PROBE, NULL);
    if (read(ITP_DEVICE_KEYPAD, &ev, sizeof (ITPKeypadEvent)) == sizeof (ITPKeypadEvent))
    {
...
bbbb        // init fat
    #ifdef CFG_FS_FAT
        itpRegisterDevice(ITP_DEVICE_FAT, &itpFSDeviceFat.dev);
 cccc       ioctl(ITP_DEVICE_FAT, ITP_IOCTL_INIT, NULL);
        ioctl(ITP_DEVICE_FAT, ITP_IOCTL_ENABLE, NULL);
    #endif // CFG_FS_FAT
        ...
        RestorePackage();
LOG_INFO "Upgrade finished. HJP2\n" LOG_END
#else
    ShowLastUpgradeFail();
    sleep(10);
	...
	#if 0//(LCD_MODULE_SEL==ITS_LCD)   // JPHong last
		ioctl(ITP_DEVICE_SCREEN, ITP_IOCTL_POST_RESET, NULL);
		ShowLogo();
