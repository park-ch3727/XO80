...
static void link_info_setting(void)
{
	...
	snprintf(ap_info[0].secumode, WIFI_SECUMODE_MAXLEN, "7");

	snprintf(ap_info[1].ssid, WIFI_SSID_MAXLEN, SSID_2);
	snprintf(ap_info[1].password, WIFI_PASSWORD_MAXLEN, PW_2);
	snprintf(ap_info[1].secumode, WIFI_SECUMODE_MAXLEN, "7");
};
...
static void link_differnet_ap(void)
{
	...
	WifiMgr_Sta_Connect(ap_info[link_loop_time%2].ssid, ap_info[link_loop_time%2].password, ap_info[link_loop_time%2].secumode);
...
{
	WifiMgr_Sta_Switch(wifi_on_off);

#if network_ap_layer
    printf("====>init network\n");
    NetworkInit();

    while (!ioctl(ITP_DEVICE_WIFI, ITP_IOCTL_WIFI_GET_NET_DEVICE, NULL)){
        usleep(100*1000);
    }
    printf("====>init wifi\n");
