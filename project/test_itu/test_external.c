...
static bool LoadTheImage(ITUWidget* widget, char* param)
{
	ITUIcon* icon = (ITUIcon*)ituSceneFindWidget(&scene, "Icon4");
	if (icon)
	{
		ituIconLoadPngFile(icon, CFG_PRIVATE_DRIVE ":/png/Icon_117x116.png");
	}
	return true;
}
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU Icon External Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/external.itu");
	...
	// load external_image
	ituSceneUpdate(&scene, ITU_EVENT_LOAD_IMAGE, (int)CFG_PRIVATE_DRIVE ":/", 0, 0);
