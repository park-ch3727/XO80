...
#define URL_LEN 256
//#define SAVEPKGTOD
//#define PKGBUFFER_SIZE 1024 * 1024 * 10;
...
static ITCArrayStream EspStream;
static char pkgFilePath[PATH_MAX];

#ifdef SAVEPKGTOD
#else
//unsigned char pkgbuffer[1024 * 1024 * 10] = { 0 };
//unsigned char *pkgbuffer;
//int pkgSizeCount = 0;
extern void UpgradeGetPkgPointer(char**, int*);
#endif
...
static int UpgradeEspPackage(void)
{
	printf("In %s: %d", __FUNCTION__, __LINE__);
	sleep(2);
	int ret = 0, size;
	char *pkgbuffer;
	ITCArrayStream EspStream;
	ITCStream* fwStream = NULL;

#ifdef SAVEPKGTOD
	FILE *test = fopen("D:/ITEPKG03.PKG", "rb");
	...
	int charsTransferred = fread(buffer, 1, size, test);
	printf("charsTransferred = %d, size = %d\n", charsTransferred, size);

	fclose(test);

	printf("Start to upgrade FW\n");
	itcArrayStreamOpen(&EspStream, buffer, size);
#else
	UpgradeGetPkgPointer(&pkgbuffer, &size);
	printf("Start to upgrade FW\n");

	itcArrayStreamOpen(&EspStream, pkgbuffer, size);
#endif
	fwStream = &EspStream.stream;
	if (!fwStream) {
...
#ifdef CFG_ESP32_SDIO_AT
	else if(code == QUIT_UPGRADE_ESP)
	{
		printf("QUIT_UPGRADE_ESP\n");
		UpgradeLcdConsoleEnable();
		ret = UpgradeEspPackage();
	}
#endif
