...
static ITUContainer* mainPlayStopContainer;
static ITUSprite* mainWarnSprite;
static ITUPopupButton *mainStopPopupButton;
static ITUPopupButton *mainPlayPopupButton;
static int checkPopupEnd = -1;
...
static float EvalVideoSpeed(float speed)
{
    if(speed < 0.5)    
        ...
        return 1.0;
    else if(speed >= 5.0 && speed < 5.5)    
        return 1.1;
    else if(speed >= 5.5 && speed < 6.0)
        return 1.2;
        ...
        return 2.4;
    else if(speed >= 16.0 && speed < 17.0)
        return 2.6;
    else if(speed >= 17.0 && speed < 18.0)
        return 2.8;
    else if(speed >= 18.0)
        return 3.0;
}
...
static int MainAudioPlayCallback(int state)
{
    ...
    case AUDIOMGR_STATE_CALLBACK_PLAYING_FINISH:
        ...
        case SOUNDPLAY_STATUS_PULSE_HUNDRED_NUMBER:
            ...
            break;
		case SOUNDPLAY_STATUS_PULSE_HUNDRED:
			//value = mainStatusPulse / 100;
...
static MainProcessExternalWarn(ExternalEvent* ev)
{
    ...
    else
    {
        ...
        case EXTERNAL_WARN_SAFETY:
			...
			ituWidgetSetVisible(mainPlayStopContainer, false);
...
bool MainOnTimer(ITUWidget* widget, char* param)
{
    ...
    else if (mainState == MAIN_WARNED)
    {
        if (itpGetTickDuration(mainWarnSoundTick) >= 5000)
        {
            ...
            else
            {
                ...
                if (AudioPlay(CFG_PUBLIC_DRIVE ":/" CFG_RES_PATH "/Voice/safety.wav", MainAudioPlayCallback) < 0)
                    soundPlayState = SOUNDPLAY_NONE;
                else
                    soundPlayState = SOUNDPLAY_WARN;
            }
	...
	if (checkPopupEnd > 0)
	{
		switch (checkPopupEnd)
		{
		case 1:
			if (mainPlayPopupButton->frame == 0)
			{
				ituWidgetSetVisible(mainOverlay2Background, false);
				ituWidgetEnable(mainOverlay1Background);
				//ituWidgetSetVisible(mainOverlay2Background, true);
			...
			break;
		case 2:
			if (mainStopPopupButton->frame == 0)
			{
				AudioStop();
...
bool MainOnEnter(ITUWidget* widget, char* param)
{
    if (!mainBackground)
    {
		...
		mainPlayStopContainer = ituSceneFindWidget(&theScene, "mainPlayStopContainer");
		assert(mainPlayStopContainer);

        mainWarnSprite = ituSceneFindWidget(&theScene, "mainWarnSprite");
        assert(mainWarnSprite);

		mainPlayPopupButton = ituSceneFindWidget(&theScene, "mainPlayPopupButton");
		assert(mainPlayPopupButton);

		mainStopPopupButton = ituSceneFindWidget(&theScene, "mainStopPopupButton");
		assert(mainStopPopupButton);
	...
	ITUText* text = (ITUText*)ituWheelGetFocusItem((ITUWheel*)mainSpeedWheel);
	float speed = (float)atof(ituTextGetString(text));
	ituVideoSpeedUpDown(mainVideo, EvalVideoSpeed(speed));
