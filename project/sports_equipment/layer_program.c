...
static ITUSprite* programDistanceUnitSprite;
...
bool ProgramOnEnter(ITUWidget* widget, char* param)
{
    if (!programType1IconRadioBox)
    {
		...
		programDistanceUnitSprite = ituSceneFindWidget(&theScene, "programDistanceUnitSprite");
		assert(programDistanceUnitSprite);
    }
	...
	if (theProgram.distance < 1000)
		ituWheelGoto(programDistanceWheel, 0); // for pace 200
	else
	...
	if (theConfig.unit_mile)
		ituSpriteGoto(programDistanceUnitSprite, 1);
	else
		ituSpriteGoto(programDistanceUnitSprite, 0);
