...
static bool speed_reverse = false;
static bool fuel_reverse = false;
static bool radiator_reverse = false;

static int rgbIndex = 0, demoIndex = 0, curType = 0, stallIndex = 1;
...
static void MainFuelDemo2(int, int);
...
void MainRadiatorDemo(int type, int tacho)
{
	if (!radiator_reverse)
	{
		radiator_m += tachoGap;
		if (radiator_m >= 100)
			radiator_reverse = true;
	}
	else
	{
		radiator_m -= tachoGap;
		if (radiator_m <= 0)
			radiator_reverse = false;
	}
...
void MainFuelDemo2(int type, int diff)
{
	if (!fuel_reverse)
	{
		fuel_m += tachoGap;
		if (fuel_m >= 100) fuel_reverse = true;
	}
	else
	{
		fuel_m -= tachoGap;
		if (fuel_m <= 0) fuel_reverse = false;
	}
	...
	{
	case 0:
		ituSpriteGoto(mainTypeAFuelSprite, (fuel_m / 6));
		break;
	case 1:
		mainTypeDFuelProgressBar->value = fuel_m;
		break;
	case 2:
		ituMeterSetValue(mainTypeCMeter[2], fuel_m);
		break;
