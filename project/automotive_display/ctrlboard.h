...
#define FILE_NUM_MAX 80
#define PAGE_NUM_MAX 10
#define MIN_RESERVED_SIZE 300*1024*1024 // 300MB 
...
typedef struct
{
    int  FileCount;
	int  PageCount;
	char VideoFilePathArray[FILE_NUM_MAX][PATH_MAX];
	char VideoFileNameArray[FILE_NUM_MAX][PATH_MAX];
	char ThumbnailPathArray[FILE_NUM_MAX][PATH_MAX];
    char SubDir[PATH_MAX];//Current Video Floder 
    char SubJpgDir[PATH_MAX];//Current JPG Floder 
    bool NOSDcard;
} VideoProcessHandle;

typedef struct{
	int  Satus;             //0 : NONE 1: CAMERA 2: VIDEO
	int  CamID;             //0 : LEFT 1: REAR
	int  PageIndex;         //playback menu page_index 
	int  LoopRecordMode[2]; //Loop recording time 
	bool LoadText;		    //load text enable
	bool LoadImage;         //load image enable
	bool ImageLoaded;       //image loaded flag
	int  ImageLoadedCount;  //image file loaded counter      <range:0~7>
    int  ImageLoadIndex;    //currnt image file loaded index <range:0~FILE_NUM_MAX-1>
	uint8_t* ImageData;
	int ImageDataSize;
}LayerControl;
...
typedef void (*VideoFileLoadCallback)(VideoProcessHandle* vhandle);
void VideoFileInit(void);
int VideoFileLoad(VideoFileLoadCallback callback);
