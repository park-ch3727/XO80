...
void AudioPlaySound(SoundType sound)
{
    ...
    if (!AudioIsMuted())
    {
        ...
        if (sound == SOUND_KEY || audioKeySoundPaused )
            return;        
        
        sprintf(path, CFG_PRIVATE_DRIVE ":/sound/%s", soundFileNames[sound]);
        if(theConfig.sound_enable) AudioPlay(path, AudioSoundPlayCallback);
    }
    else
    {
        audioNextSound = SOUND_KEY;
...
void AudioWaitPlayingEof(int sec){
#ifdef __OPENRTOS__
    smtkAudioWaitAudioStop(sec);
#endif
}