...
static ITUBackground *mainWindTrackTipBackground;
...
bool MainOnEnter(ITUWidget* widget, char* param)
{
    if (!mainLayer)
    {
		...
		mainWindTrackTipBackground = ituSceneFindWidget(&theScene, "mainWindTrackTipBackground");
		assert(mainWindTrackTipBackground);
...
bool MainWindTrackButtonOnMouseUp(ITUWidget* widget, char* param)
{
	int wind = theConfig.wind;
	if (wind == 0)
		AudioPlaySound(SOUND_WIND_AUTO);
	else if (wind == 100)
		AudioPlaySound(SOUND_WIND_MAX);
...
bool MainWindTrackBarOnChange(ITUWidget* widget, char* param)
{
	ituWidgetSetVisible(mainWindTrackTipBackground, true);
	return true;
}