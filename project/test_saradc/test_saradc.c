...
int testSARADC(int channel)
{
    ...
    result = mmpSARSetAVGDetectRule(channel, 0xe00, 0xd00);
    if (result)
        printf("mmpSARSetAVGDetectRule() error (0x%x) !!\n", result);

    result = mmpSARSetAVGTriggerRule(channel, SARADC_TRIG_AVG_LEVEL, 0x500, 0x300);
    if (result)
    ...
    ithWriteRegA(0xd08000AC,0x00000505); // Rising Falling High_level Low_level
    ithWriteRegA(0xd08000A8,0x82800200); // En BIST mode
    ...
    ithWriteRegA(0xd08000A8,0x02800200); // Dis BIST mode
...
void* TestFunc(void* arg)
{
    ...
    result = mmpSARInitialize(SARADC_MODE_AVG_ENABLE, SARADC_MODE_STORE_AVG_ENABLE, SARADC_AMPLIFY_1X, SARADC_CLK_DIV_9);
    if (result)
        printf("mmpSARInitialize() error (0x%x) !!\n", result);
