...
bool MainOnTimer(ITUWidget* widget, char* param)
{
	...
	if (!activeMainInfo1Animation && ituWidgetIsVisible(mainInfo1Background) && ituWidgetIsVisible(mainInfo1TextMediaFileListBox) && ituWidgetIsVisible(mainInfo1FontMediaFileListBox))
    {
		...
		ituTextResize(mainInfo15Text);
		mainInfo111Text->widget.rect.x = -mainInfo15Text->widget.rect.width;
    }
...
static void MainInfo1AnimationOnStop(ITUAnimation* animation)
{
    ...
    if (ok)
    {
        ...
        ituTextSetString(text, infoBuf);
		ituTextResize(text);
		if (mainInfo1Sprite->frame == 0)
			mainInfo111Text->widget.rect.x = -text->widget.rect.width;// to right
		else
			mainInfo122Text->widget.rect.x = -text->widget.rect.width;// to left
    }
    ituAnimationPlay(activeMainInfo1Animation, 0);
}
...
bool MainOnEnter(ITUWidget* widget, char* param)
{
    if (!mainLayer)
    {
		...
		//ituTextResize(text);
		//mainInfo111Text->widget.rect.x = -text->widget.rect.width;
        ...
        ituAnimationSetOnStop(mainInfo21Animation, MainInfo1AnimationOnStop);
